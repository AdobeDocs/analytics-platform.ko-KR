...
>title="데이터 보기"
>abstract="Content Analytics을 구성하려면 하나 이상의 데이터 보기를 선택해야 합니다. 따라서 Content Analytics 데이터를 다른 데이터와 병합할 수 있습니다."

>[!CONTEXTUALHELP]
>id="aca_onboarding_dataviews_header"
>title="데이터 보기"
>abstract="Content Analytics 데이터를 병합하려는 Customer Journey Analytics에서 기존 데이터 보기를 하나 이상 선택합니다."

>[!CONTEXTUALHELP]
>id="aca_onboarding_dataviews_header_alt"
>title="데이터 보기"
>abstract="Customer Journey Analytics에서 Content Analytics 데이터를 병합할 기존 데이터 보기를 하나 이상 선택하십시오.<br/>"

>[!CONTEXTUALHELP]
>id="aca_onboarding_dataviews_new_dialog"
>title="선택한 데이터 보기"
>abstract="이 구성에 대해 선택한 데이터 보기를 수정했습니다. 선택한 데이터 보기가 Content Analytics 지표 및 차원을 포함하도록 업데이트됩니다. 이러한 지표 및 차원은 더 이상 선택되지 않은 이전에 선택한 데이터 보기에서 제거됩니다.<br/><br/>다른 연결이 선택한 데이터 보기에 연결되어 있으면 연결이 Content Analytics 데이터 집합을 포함하도록 업데이트됩니다. Content Analytics 데이터 세트는 원래 선택된 연결에서 제거되지 않습니다.<br/><br/>선택한 모든 데이터 보기는 이 구성에 속하는 채널을 상속합니다."
...
>title="채널"
>abstract="구성에 대해 하나 이상의 채널을 활성화하고 구성합니다. 구성에 포함된 모든 데이터 보기는 활성화된 채널을 상속합니다."
...
1. 웹 사이트에서 콘텐츠가 렌더링되는 방식에 대한 매개변수를 지정합니다. 매개변수는 0개 이상의 **[!UICONTROL 도메인 정규 표현식]**&#x200B;과 **[!UICONTROL 쿼리 매개변수]** 조합으로 구성됩니다. 쿼리 매개변수는 어떤 매개변수가 페이지의 콘텐츠에 영향을 미치는지 나타냅니다. 이 입력을 통해 Content Analytics는 고유한 경험을 정의할 때 페이지의 콘텐츠에 영향을 미치지 않는 모든 매개변수를 무시할 수 있습니다.
...
* ![편집](/help/assets/icons/Edit.svg) **[!UICONTROL 편집]**&#x200B;을 선택하여 Content Analytics의 경험을 위한 데이터 수집 구성을 추가로 편집합니다. 현재 구성과 연결된 태그 속성의 [Adobe Content Analytics 확장 기능](https://experienceleague.adobe.com/ko/docs/experience-platform/tags/extensions/client/content-analytics/overview#configure-event-segmenting)으로 리디렉션됩니다.


### 데이터 수집 {#onboarding-data-collection}

이 섹션에서는 Content Analytics 데이터를 수집하는 방법을 구성합니다.
...
>abstract="**포함/제외할 경험 위치**"

>[!CONTEXTUALHELP]
>id="aca_onboarding_datacollection_mobile_experience_locations_header"
>title="모바일 경험 위치 데이터 수집"
>abstract="Content Analytics에 대한 데이터를 수집할 때 **포함** 또는 **제외**&#x200B;해야 하는 경험 위치를 지정하십시오."

>[!CONTEXTUALHELP]
>id="aca_onboarding_datacollection_mobile_asset_locations_boldheader"
>title="모바일 자산 위치 데이터 수집"
>abstract="**포함/제외할 자산 위치**"

>[!CONTEXTUALHELP]
>id="aca_onboarding_datacollection_mobile_asset_locations_header"
>title="모바일 자산 위치 데이터 수집"
>abstract="Content Analytics에 대한 데이터를 수집할 때 자산 위치를 **포함** 또는 **제외**&#x200B;해야 함을 나타냅니다."

>[!CONTEXTUALHELP]
>id="aca_onboarding_datacollection_mobile_asset_urls_boldheader"
>title="모바일 자산 URL 데이터 수집"
>abstract="**포함/제외할 자산 URL**"

>[!CONTEXTUALHELP]
>id="aca_onboarding_datacollection_mobile_asset_urls_header"
>title="모바일 자산 URL 데이터 수집"
>abstract="Content Analytics에 대한 데이터를 수집할 때 **포함** 또는 **제외**&#x200B;해야 하는 자산 URL을 나타냅니다."
...
* 기존 태그 속성을 사용하는 방법은 다음과 같습니다.
  ...
  ![Content Analytics 데이터 수집 기존 태그](../assets/aca-configuration-datacollection-existingtag.png)
   ...
   2. **[!UICONTROL 태그 속성]** 드롭다운 메뉴에서 기존 속성을 선택합니다. 속성을 입력하여 검색하고 사용 가능한 옵션을 제한할 수 있습니다. 이미 구현된 다른 Content Analytics 구성에서 사용 중인 태그 속성은 선택할 수 없습니다.


* 새 태그 속성을 만드는 방법은 다음과 같습니다.

  ![Content Analytics 데이터 수집 새 태그](../assets/aca-configuration-datacollection-newtag.png)
...
Content Analytics 구성에 대한 데이터 수집을 시작하려면 **[!UICONTROL 구현]**&#x200B;을 선택한 후 생성되는 태그 속성을 [수동으로](manual.md) 게시해야 합니다.
