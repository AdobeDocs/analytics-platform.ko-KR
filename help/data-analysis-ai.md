...
| **명확한 질문** | 질문을 명확히 하는 것은 구성 요소 및 차원 항목으로 제한됩니다. 데이터 인사이트 에이전트에서는 데이터 보기, 시각화, 데이터 세부기간, 비교 및 범위와 같은 항목을 명확하게 할 수 없습니다. 명확한 질문을 사용할 수 없는 경우 도우미는 사용자가 요청할 가능성이 가장 높은 것으로 기본 설정됩니다. 예기치 않은 시각화 또는 데이터 세부기간을 반환하는 경우 다중 전환/업데이트 기능을 사용하여 시각화 및 데이터를 조정할 수 있습니다. |
...
## AI Assistant에서 액세스 및 데이터 인사이트 에이전트
...
**응답:** AI Assistant의 Data Insights 에이전트는 지표 및 구성 요소를 포함하여 데이터 보기의 데이터를 통해 인사이트를 수집합니다. 프롬프트가 데이터 범위 내에서 올바른 차원 및 지표로 변환됩니다.
...
**응답:** AI Assistant의 Data Insights 에이전트는 &quot;지역&quot;별로 &quot;고객 지역&quot;을 의미한다는 것을 지능적으로 이해합니다. 지역별 매출을 가장 잘 보여주는 막대 차트를 생성합니다.
...
**응답:** AI Assistant의 Data Insights 에이전트는 가장 적절한 시각화(이 경우 **[!UICONTROL 도넛]** 시각화)를 선택하여 질문에 답합니다.
...
다음 표의 AI Assistant의 데이터 인사이트 에이전트를 사용하여 프롬프트에 사용할 수 있는 예시 용어와 구문을 예상할 수 있는 응답 유형과 함께 검토하십시오.

이러한 예제는 특정 단어나 구조가 AI Assistant의 출력에 어떻게 영향을 미칠 수 있는지 잘 알고 보다 정확하고 가치 있는 통찰력을 확보할 수 있도록 설계되었습니다. AI Assistant의 Data Insights 에이전트는 생성 AI를 사용하므로 시각화 또는 선택한 데이터가 유사한 프롬프트에 따라 약간 달라질 수 있습니다.
...
AI Assistant에서 Data Insights 에이전트에서 응답을 평가할 때는 다음 사항을 고려하십시오.
