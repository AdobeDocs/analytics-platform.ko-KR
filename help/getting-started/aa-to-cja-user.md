...
연결을 통해 Analytics 관리자는 의 데이터 세트를 통합할 수 있습니다 [!DNL Adobe Experience Platform] 변환 [!UICONTROL 작업 공간]. [!DNL Experience Platform] 데이터 세트에 대해 보고하려면 먼저 [!DNL Experience Platform]과 [!UICONTROL 작업 영역]의 데이터 세트 간에 연결을 설정해야 합니다.
...
조직이 여전히 Adobe Analytics 플랫폼에 있고 CJA/AEP를 추가하는 경우 Adobe Analytics 소스 커넥터 또는 웹 SDK를 통해 보고서 세트 데이터를 Experience Platform으로 가져올 수 있습니다. 일반적으로 Analytics 스키마를 사용하여 보고서 세트별로 지정된 데이터 세트를 소스를 만듭니다.

하지만 보고서 세트는 더 이상 CJA에서 보고의 기반이 아닙니다. [데이터 보기](/help/data-views/data-views.md) 입니다. 데이터 보기에 대한 자세한 내용은 아래 섹션을 참조하십시오.
...
조회 데이터 세트는 이벤트 또는 프로필 데이터에 있는 값 또는 키를 찾는 데 사용됩니다. 예를 들어 이벤트 데이터의 숫자 ID를 제품 이름에 매핑하는 조회 데이터를 업로드할 수 있습니다. 예제는 [본 사용 사례](/help/use-cases/b2b.md)를 참고하십시오.

## 이제 고객 속성이 &#39;프로필 데이터 세트&#39;입니다

프로필 데이터 세트에는 [!UICONTROL 이벤트] 데이터. 예를 들어 고객에 대한 CRM 데이터를 업로드할 수 있습니다. 포함할 개인 ID를 선택할 수 있습니다. [!DNL Experience Platform]에 정의된 각 데이터 세트에는 쿠키 ID, 결합된 ID, 사용자 ID, 추적 코드 등과 같은 개인 ID가 하나 이상 정의된 고유한 ID 세트가 있습니다.

## ID

CJA는 ECID 이상의 ID의 개념을 확장하여 고객 ID, 쿠키 ID, 결합된 ID, 사용자 ID, 추적 코드 등을 포함하여 사용할 ID를 포함합니다. 데이터 세트에서 공통 네임스페이스 ID 사용 또는 [크로스 채널 분석](/help/connections/cca/overview.md) 은 서로 다른 데이터 세트에서 사람들을 서로 연결하는 데 도움이 됩니다. CJA에서 작업 공간 프로젝트를 설정하는 모든 사용자는 데이터 세트에서 사용되는 ID를 이해해야 합니다.

다음은 Customer Journey Analytics에서 ID의 사용을 강조 표시하는 비디오입니다.
