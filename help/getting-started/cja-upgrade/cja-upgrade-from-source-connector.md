---
title: Customer Journey Analytics을 위해 Analytics 소스 커넥터에서 웹 SDK로 전환
description: Customer Journey Analytics으로 업그레이드할 때 Analytics 소스 커넥터에서 웹 SDK로 전환하는 방법에 대해 알아봅니다
role: Admin
...
# Customer Journey Analytics을 위해 Analytics 소스 커넥터에서 웹 SDK로 전환
...
조직에서 이미 Analytics 소스 커넥터 구현만 사용하여 Customer Journey Analytics으로 업그레이드한 경우, 지속적인 데이터 수집을 위해 Web SDK의 새 구현과 함께 Analytics 소스 커넥터(이전 데이터를 가져오기 위해)를 사용하는 구현으로 전환하는 것을 고려해야 합니다.
