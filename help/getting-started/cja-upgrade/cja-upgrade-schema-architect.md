...
| 변수를 &quot;사전 해결&quot; 보고로 디자인 | 팩트를 캡처할 수 있도록 스키마를 디자인하고 데이터 보기를 사용하여 보고 의미 체계를 해결합니다 |
...
이 패턴은 팀이 불필요한 필드를 채우지 않고 단일 조직 스키마 전략을 지원합니다.
...
Adobe Analytics에서 많은 팀은 `events` 변수를 유일한 지표 추적 수단으로 취급합니다. Customer Journey Analytics에서는 계산해야 하는 항목과 지표를 해석하는 방법에 따라 여러 가지 방법으로 지표를 추적할 수 있습니다.
...
>[!TIP]
>
>나중에 스키마의 전용 필드와 파생 필드의 여부를 결정할 때 유용하고 안정적인 스키마 팩트를 캡처하는 것이 좋습니다. 수집 후 파생 필드를 사용하여 데이터를 수정하거나 모양을 변경할 수 있습니다.
