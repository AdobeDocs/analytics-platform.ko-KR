...
| 스트리밍 Media Analytics | 미디어 데이터는 미디어 동시 뷰어 패널의 일부로 Analytics 데이터 커넥터를 사용하고 작업 공간의 미디어 재생 시간 패널을 사용하여 사용할 수 있습니다. |
...
| 기능 | 참고 |
| --- | --- |
| 대상 게시(세그먼트 게시) | Adobe의 고객 데이터 플랫폼 또는 Journey Optimizer 제품에서 라이선스가 부여된 경우 지원됩니다. [대상자 게시](/help/components/audiences/audiences-overview.md)는 Experience Platform의 실시간 고객 프로필로 대상자를 보냅니다. |
...
| 분류 규칙 빌더 | 지원 대상 [하위 문자열](/help/data-views/component-settings/substring.md) 참조하십시오. 조회 데이터 세트가 아니라 보고서 시간에 문자열 조킹을 사용합니다. |
...
| 지표 중복 제거 | 이제 데이터 보기 내의 지표에 대해 구성됩니다. 지표 중복 제거는 데이터 세트, 데이터 보기 또는 연결 수준이 아닌 개인 또는 세션 수준에서 수행됩니다. |
...
| 새 세션 보고와 반복 세션 보고 비교 | 이전에 방문 번호 차원을 사용하여 수행되었습니다. 새 세션과 반복 세션이 지원됩니다 [13개월 전환 확인 기간 사용](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-dataviews/data-views-usecases.html?lang=ko-KR?lang=ko#new-repeat). |
...
| Report Builder (Excel 플러그인) | Excel용 새 Office 365 플러그인과 함께 지원됩니다. |
| 사용자 권한/데이터 액세스 제어 | CJA의 구분 [Adobe Admin Console](https://experienceleague.adobe.com/docs/core-services/interface/administration/admin-getting-started.html?lang=ko-KR) 제품 관리자, 제품 프로필 관리자 및 사용자. 제품 관리자만 다른 사용자가 만든 연결, 프로젝트, 필터 또는 계산된 지표를 생성/업데이트/삭제할 수 있고 제품 관리자 및 제품 프로필 관리자는 데이터 보기를 편집할 수 있습니다. 계산된 지표, 필터 또는 주석 만들기와 같은 추가적인 사용자 권한을 사용할 수 있습니다. |
| 처리 규칙, VISTA 규칙, 마케팅 채널 처리 규칙 | Analytics 데이터 커넥터의 데이터와 WebSDK 기반 데이터 세트 모두에 Adobe Experience Platform 데이터 준비 기능을 사용하여 지원됩니다. |
...
| 기능 | 참고 |
| --- | --- |
| 마케팅 채널 | 마케팅 채널 데이터가 Analytics 소스 커넥터를 통해 CJA로 전송됩니다. 마케팅 채널 규칙은 계속 기존의 Adobe Analytics에서 구성해야 하며 일부 규칙은 지원되지 않습니다. 자세한 내용은 [CJA 마케팅 채널 설명서](https://experienceleague.adobe.com/docs/analytics-platform/using/cja-usecases/marketing-channels.html?lang=ko-KR#cja-usecases)를 참조하십시오. 또한 WebSDK 구현의 경우 마케팅 채널 클라이언트측을 정의하는 데 플러그인을 사용할 수 있습니다. 보고서 시간 마케팅 채널 처리 규칙에 대한 향후 지원이 계획되어 있습니다. |
| 크로스 디바이스/크로스 채널 결합 | ID 정보가 직접 포함된 데이터 세트(&quot;필드 기반&quot; 결합이라고도 함) 지원 그래프 기반 결합은 아직 지원되지 않지만, 가능합니다. [크로스 채널 분석](/help/connections/cca/overview.md)을 참조하십시오. |
...
| 디바이스, 브라우저, 레퍼러, 기술 차원 | 지원 대상 [Adobe Analytics 소스 커넥터](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/analytics.html)데이터 세트 기반. [ADC를 통해 지원되는 Analytics 변수에 대한 설명서](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/mapping/analytics.html?lang=ko-KR)를 참조하십시오.<p>Adobe Source Connector를 사용하여 Adobe Analytics의 데이터를 CJA로 채우지 않고 대신 Experience Platform Web SDK 데이터 수집을 사용하는 경우 현재 디바이스 조회를 기반으로 하는 디바이스 및 차원이 지원되지 않습니다. 향후 지원 계획이 있습니다. |
...
| A4T | 부분 지원은 [Analytics 소스 커넥터](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/analytics.html). Target 활동 및 경험에 대한 A4T 친숙한 이름 지원이 계획되어 있습니다. |
...
| 통화 전환 | 지원이 예정되어 있습니다. |
| 데이터 피드 | AEP 대상을 통해 지원이 예정되어 있습니다. |
