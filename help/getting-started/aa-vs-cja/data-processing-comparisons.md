...
반면 CJA(Customer Journey Analytics)는 데이터를 구성하고 저장하기 전에 미리 수집하는 시간을 최소화하도록 설계되었습니다. CJA의 기본 아키텍처는 보고서 시간에 저장된 데이터로 작업하도록 보다 디자인되었으며 Workspace에서뿐만 아니라 의 정의를 통해 보다 중요한 강력한 보고서 시간 처리 기능을 제공합니다. [구성 요소](/help/data-views/component-settings/overview.md) 및 [파생 필드](/help/data-views/derived-fields/derived-fields.md) 데이터 보기에서 다음을 수행합니다.

![CJA 보고서 시간 처리](../assets/cja-processing.png)
...
| 세그먼트(필터) 논리 | 이벤트/방문/방문자(이벤트/세션/개인) 세그먼트(필터) 규칙의 평가입니다. | 예: 빨간 신발을 구입한 사람. |
...
| 사용자 정의 필드 | 논리는 데이터 보기에서 구성 요소를 정의하는 과정의 일부로 스키마 또는 표준 필드에 적용됩니다. | 예: 새 마케팅 채널 차원 만들기 |
