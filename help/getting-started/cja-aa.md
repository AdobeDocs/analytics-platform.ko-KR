...
| 디바이스, 브라우저, 기술 규격 | 이러한 차원은 AEP 데이터 세트에 특정 XDM 스키마 필드가 포함되고 XDM 경험 이벤트 클래스를 준수하는 경우에 자동으로 포함됩니다. |
| 시작, 종료, 시간 간격 차원 및 지표 | 지원됨(시작 및 종료는 이제 세션 시작 및 세션 종료)이라고 하며 약간 다른 방식으로 계산됩니다. |
