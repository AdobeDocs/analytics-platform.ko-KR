...
| 사용자 정의 세션 | 모바일 배경 조회 수를 제외한 모든 사용자 정의 세션 기능을 지원합니다. |
| 고객 특성 | 이제 “프로필 데이터 세트”라고 하며, Experience Cloud에서 자동으로 가져오지 않습니다. CJA에서 사용하려면 AEP로 업로드해야 합니다. |
| [!UICONTROL 장치], [!UICONTROL 브라우저], [!UICONTROL 레퍼러], [!UICONTROL 기술] 차원 | 이러한 차원은 AEP 데이터 세트에 특정 XDM 스키마 필드가 포함되고 XDM 경험 이벤트 클래스를 준수하는 경우에 자동으로 포함됩니다. 다음 문서를 참조하십시오 [analytics 소스 커넥터를 통해 지원되는 Analytics 변수에 대한 설명서입니다.](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/mapping/analytics.html?lang=en). 소스 커넥터를 사용하여 Adobe Analytics의 데이터를 CJA로 채우지 않고 대신 AEP 웹 SDK 데이터 수집을 사용하는 CJA 고객의 경우, [!UICONTROL 장치] 장치 조회를 기반으로 하는 및 차원은 현재 지원되지 않지만, 가까운 미래에 제공될 예정입니다. |
...
| 지리 특성 - 차원 | Adobe Analytics에 수집된 모든 지리 특성/지리적 위치는 [Analytics 소스 커넥터](https://experienceleague.adobe.com/docs/experience-platform/sources/connectors/adobe-applications/analytics.html?lang=en). 디지털 데이터 수집에 AEP Web SDK를 사용하는 것과 같이 Analytics 소스 커넥터를 사용하지 않는 구현은 자동으로 전체 지리적 조회 기간을 수행하지 않습니다. 국가 및 미국 주가 지원되며, 도시 및 우편 번호는 지원되지 않습니다. 현재 미국 외부의 주/지역에 대한 지원이 제한되거나 지원되지 않습니다. |
...
| --- | --- |
| 경고 | 지원이 예정되어 있습니다. |
| 달력 이벤트 | 작업 영역에서는 “Annotations”라고 합니다. |
