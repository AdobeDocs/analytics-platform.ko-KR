...
>[!NOTE]
>
>안내식 분석과 Analysis Workspace 간의 용어(세그먼트, 필터 등) 차이에 대한 자세한 내용은 [FAQ](faq.md)를 참조하세요.
...
가이드 분석을 위한 인터페이스는 질문과 답변 형식을 따릅니다. 쿼리 레일에서 질문을 작성하면 글로 작성된 인사이트, 차트 및 표를 통해 답변을 얻을 수 있습니다. 그런 다음 분석 및 시각화 설정을 사용하여 다음 질문을 할 수 있습니다.
...
| ![Menu](assets/menu.png){style="border:1px solid gray"} | **[!UICONTROL 메뉴]**<br/>안내식 분석 프로젝트에서 사용 가능 | 분석을 위해 중요한 작업을 제공하는 안내가 있는 분석 프로젝트의 오른쪽 상단에 있는 명령입니다.<ul><li>![데이터](/help/assets/icons/Data.svg) ***데이터 보기의 이름***: 분석에서 사용하는 데이터 보기를 변경합니다. 데이터 보기를 변경하면 쿼리 레일에서 사용 가능한 구성 요소도 변경됩니다.</li><li>![링크](/help/assets/icons/Link.svg) **링크 복사**: 분석에 대한 링크를 클립보드에 복사합니다. 공유하기 전에 저장하라는 메시지가 표시됩니다.</li><li>**공유**: 개별 사용자 또는 그룹과 공유하기 위한 추가 옵션이 포함된 공유 모드를 엽니다. 분석을 다른 사용자와 공유하거나, 링크를 생성하여 모두와 공유할 수 있습니다.</li><li>**저장**: 분석을 저장합니다. 새 분석을 저장하는 경우 이름과 설명을 요청하는 **[!UICONTROL 분석 저장]** 대화 상자가 나타납니다. 저장되면 **[!UICONTROL 분석 저장]** 대화 상자를 통해 분석을 공유할 수 있습니다.</li></ul>다음과 같은 추가 작업을 보려면 ![자세히](/help/assets/icons/More.svg)를 선택하십시오.<ul><li>**다른 이름으로 저장**: 현재 분석과 별개로 분석을 저장하여 사본을 만듭니다. 새 이름과 설명을 요청하는 대화 상자가 나타납니다.</li><li>**Workspace으로 내보내기**: Analysis Workspace에서 현재 안내식 분석 쿼리를 다시 만듭니다. Workspace 프로젝트는 새 탭에서 만들어지므로 가이드 분석에서 작업하는 도중에 중단이 발생하지 않습니다. 이는 분석의 사본이며, 연 후에는 원본 가이드 분석과 동기화된 상태로 유지되지 않습니다. 분석가 팀에 전달하거나 가이드 분석에서 가능한 것보다 더 심층적으로 데이터를 분석하려는 경우 이 명령을 사용합니다.</li><li>**클립보드에 차트 복사**: 다른 응용 프로그램에 붙여넣을 차트 그래픽을 클립보드에 복사합니다. 쿼리 레일과 테이블은 그래픽에 포함되지 않습니다.</li><li>**PNG 다운로드**: 차트 그래픽을 `.png`로 다운로드합니다. 쿼리 레일과 테이블은 그래픽에 포함되지 않습니다.</li><li>**CSV 다운로드**: 테이블 데이터를 `.csv`로 다운로드합니다. 쿼리 레일과 차트는 파일에 포함되지 않습니다.</li></ul> |
| ![메뉴 시각화](assets/menu-visualization.png) | **메뉴**<br/> Analysis Workspace의 안내식 분석 시각화에서 사용할 수 있습니다. | Analysis Workspace의 안내식 분석 시각화에서 사용할 수 있는 명령.<ul><li>![GraphScatter](/help/assets/icons/GraphScatter.svg) **[!UICONTROL 차트]**: 분석 차트만 표시합니다.</li><li>![테이블](/help/assets/icons/Table.svg) **[!UICONTROL 테이블]**: 분석 테이블만 표시합니다.</li><li>![TableAndChart](/help/assets/icons/TableAndChart.svg) **[!UICONTROL 모두]**: 분석의 차트와 테이블을 표시합니다.</li><li>![편집](/help/assets/icons/Edit.svg) **[!UICONTROL 편집]**: 분석의 구성을 편집하려면</li><li>![일정](/help/assets/icons/Calendar.svg) **[!UICONTROL *날짜 범위&#x200B;*]**: 분석에 사용할 데이터 범위를 구성합니다.</li></ul> |
