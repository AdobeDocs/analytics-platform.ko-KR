...
* **고객 충성도**: 이벤트를 주문으로, 지표를 사용자로 설정합니다. 이 분석을 통해 지정된 날짜 범위 내에서 사이트에서 구매한 횟수에 따라 사용자를 그룹화할 수 있습니다.
...
이벤트가 그룹(버킷)으로 분류되는 방식을 결정합니다. 트렌드 테이블 보기에서는 총 사용 빈도와 각 간격을 기준으로 사용자가 버킷에 분류됩니다. 즉, 한 명의 사용자가 서로 다른 간격에서 서로 다른 버킷에 포함될 수 있습니다.
