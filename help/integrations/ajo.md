...
Journey Optimizer는 보고 엔진으로 Customer Journey Analytics 사용을 지원합니다. Journey Optimizer 설명서에서 [새 보고 인터페이스 시작](https://experienceleague.adobe.com/ko/docs/journey-optimizer/using/reporting/channel-report/report-gs-cja)을 참조하시기 바랍니다.
...
연결에 **[!UICONTROL AJO 사용 연결(*샌드박스 이름*)]** 이름이 있으며 구성 및 데이터 세트에 대해 다음과 같은 기본 값이 있습니다.
...
| [!UICONTROL AJO 메시지 피드백 이벤트 데이터 세트 - 프로필이 아님] <br/>(아래 [높은 처리량 추가 기능 데이터 세트](#high-throughput-add-on-datasets) 섹션 참조) | [!UICONTROL AJO 메시지 피드백 이벤트 스키마] | [!UICONTROL 이벤트] | [!UICONTROL 기타] | [!UICONTROL IdentityMap(\&lt;primary\>)] | - | - | ![Status Green](assets/../../connections/assets/status-green.svg) 켜짐 | ![Status Gray](assets/../../connections/assets/status-gray.svg) 꺼짐 |
| [!UICONTROL AJO 전자 메일 추적 경험 이벤트 데이터 세트 - 프로필이 아님] <br/>(아래 [높은 처리량 추가 데이터 세트](#high-throughput-add-on-datasets) 섹션 참조) | [!UICONTROL AJO 이메일 추적 경험 이벤트 스키마] | [!UICONTROL 이벤트] | [!UICONTROL 기타] | [!UICONTROL IdentityMap(\&lt;primary\>)] | - | - | ![Status Green](assets/../../connections/assets/status-green.svg) 켜짐 | ![Status Gray](assets/../../connections/assets/status-gray.svg) 꺼짐 |

#### 높은 처리량 추가 데이터 세트

IMS Org에 대해 처리량 높음 트랜잭션 메시지 추가 기능을 활성화하면 두 개의 비프로필 시스템 생성 데이터 세트가 연결에 포함됩니다.

* AJO 메시지 피드백 이벤트 데이터 세트 - 비프로필

* AJO 이메일 추적 경험 이벤트 데이터 세트 - 비프로필

처리량이 많은 트랜잭션 메시지 추가 기능이 활성화되면, 글로벌(샌드박스) 수준의 Journey Optimizer 보고에서 두 개의 새로운 위젯을 사용할 수 있습니다(캠페인별 수준에서는 사용할 수 없음).
...
| AJO 이메일 추적 경험 이벤트 데이터 세트 | 이벤트 | 개인 ID: `IdentityMap` | “[!UICONTROL 열람수]”, “[!UICONTROL 클릭수]” 및 “[!UICONTROL 구독 취소]”와 같은 이메일 추적 이벤트를 포함합니다. |
...
| AJO 메시지 피드백 이벤트 데이터 세트 - 비프로필 | 이벤트 | 개인 ID: `IdentityMap` | 프로필이 아닌 메시지 게재 피드백 이벤트를 포함합니다. [높은 처리량 트랜잭션 메시지 추가 기능](#high-throughput-add-on-datasets)을(를) 사용하도록 설정한 경우에만 사용할 수 있습니다. |
| AJO 이메일 추적 경험 이벤트 데이터 세트 - 비프로필 | 이벤트 | 개인 ID: `IdentityMap` | 비프로필 이메일 추적 경험 이벤트를 포함합니다. [높은 처리량 트랜잭션 메시지 추가 기능](#high-throughput-add-on-datasets)을(를) 사용하도록 설정한 경우에만 사용할 수 있습니다. |
...
| 메시지 실패 카테고리 (AJO) | 실패 카테고리 | AJO 메시지 피드백 이벤트 데이터 세트 | ` _experience.customerJourneyManagement.`<br/>`messageDeliveryfeedback.`<br/>`messageFailure.category` | 구성 요소 유형: 차원 |
| 메시지 실패 이유 (AJO) | 실패 이유 | AJO 메시지 피드백 이벤트 데이터 세트 | `_experience.customerJourneyManagement.`<br/>`messageDeliveryfeedback.`<br/>`messageFailure.reason` | 구성 요소 유형: 차원 |
...
| 메시지 재시도 (AJO) | 재시도 횟수 | AJO 메시지 피드백 이벤트 데이터 세트 | `_experience.customerJourneyManagement.`<br/>`messageDeliveryfeedback.retryCount` | 구성 요소 유형: 차원 |
...
| 푸시 플랫폼 (AJO) | 푸시 공급자 서비스(예: apns 또는 fcm)입니다. | AJO 이메일 추적 경험 이벤트 데이터 세트, AJO 메시지 피드백 이벤트 데이터 세트, AJO 푸시 추적 경험 이벤트 데이터 세트 | `_experience.customerJourneyManagement.`<br/>`pushChannelContext.platform` | 구성 요소 유형: 차원 |
...
| 처리 이름 (AJO) | 실험을 위해 선택된 처리의 이름입니다. | AJO 엔티티 데이터 세트 | `_experience.customerJourneyManagement.`<br/>`entities.experiment.treatmentName` | 구성 요소 유형: 차원 |
...
| URL 레이블 (AJO) | 사람이 인식할 수 있는 URL 레이블입니다. | AJO 이메일 추적 경험 이벤트 데이터 세트 | `_experience.customerJourneyManagement.`<br/>`messageInteraction.label` | 구성 요소 유형: 차원 |
...
| 디스플레이 (AJO) | 이 수는 AJO 메시지를 표시합니다. 여기에는 이메일 열기, 웹 디스플레이 및 인앱 디스플레이가 포함됩니다. 모바일 플랫폼은 SMS 및 푸시 메시지 표시를 보고하지 않으므로 카운트되지 않습니다. | AJO 푸시 추적 경험 이벤트 데이터 세트, 여정 단계 이벤트, AJO 이메일 추적 경험 이벤트 데이터 세트, AJO 메시지 피드백 이벤트 데이터 세트 | 파생 필드 | 구성 요소 유형: 지표 (파생 필드) |
...
| 타기팅됨 (AJO) | 제안이 개인에게 타겟팅된 횟수입니다. 사람에게 표시하기 위해 제안이 고려된 횟수입니다. | AJO 푸시 추적 경험 이벤트 데이터 세트, 여정 단계 이벤트, AJO 메시지 피드백 이벤트 데이터 세트, AJO 이메일 추적 경험 이벤트 데이터 세트 | 파생 필드 | 구성 요소 유형: 지표 (파생 필드) |
| 트리거됨 (AJO) | 제안 은 Adobe SDK에 의해 표시되도록 선택되었습니다. 다른 요인으로 인해 실제로 표시되지 않을 수 있습니다. | AJO 푸시 추적 경험 이벤트 데이터 세트, 여정 단계 이벤트, AJO 메시지 피드백 이벤트 데이터 세트, AJO 이메일 추적 경험 이벤트 데이터 세트 | `_experience.decisioning.`<br/>`propositionEventType.trigger` | 구성 요소 유형: 지표 |
