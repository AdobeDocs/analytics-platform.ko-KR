...
1. Customer Journey Analytics에서 [!UICONTROL **프로젝트**] 랜딩 페이지를 선택한 다음 [!UICONTROL **환경 설정 편집**].
   ...
   제품 관리자는 로 이동하여 IMS 조직 환경 설정을 업데이트할 수 있습니다. [!UICONTROL **구성 요소**] 탭, 선택 [!UICONTROL **환경 설정**].
...
| | 예측 표시 | 생성하는 시계열 자유 형식 테이블의 첫 번째 지표 열에 대해 예측 값이 자동으로 표시되는지 여부를 결정합니다. |
...
| | 예측을 Y축으로 확장하도록 허용 | 과거 값의 상한 및 하한 범위를 벗어나는 예측 값이 있는 경우 이러한 예측 값에 대해 y축이 자동으로 조정되지 않습니다. 이 옵션을 켜면 예측된 값에 대한 y축의 크기가 적절히 조정됩니다. |
...
| | 컨테이너 | 방문과 방문자 간을 변환하여 방문자 이동 경로를 분석할 수 있도록 합니다. 기본값은 방문자입니다. 이러한 설정을 통해 세션 간 개인 수준에서 개인 참여를 이해하거나 분석을 단일 세션으로 제한할 수 있습니다. <p>다음 옵션을 사용할 수 있습니다.</p> <ul><li>방문</li><li>방문자</li></ul> |
...
| | 계산 방법 | 다음 선택 사항 중 하나를 선택합니다. <ul><li>히트</li><li>방문</li><li>방문자</li></ul> <p>예를 들어 페이지 보기 수와 함께 사용할 경우, 사람당 페이지 보기 수, 방문용 페이지 보기 수 또는 이벤트당 페이지 보기 수를 선택할 수 있습니다. 히트의 경우 “발생 횟수”는 자유형 테이블에서 Y축 지표로 사용됩니다.</p> |
