...
만든 모든 새 프로젝트 또는 패널에 대해 Analysis Workspace 및 관련 구성 요소의 사용자 설정이나 환경 설정을 관리할 수 있습니다. 기존 프로젝트 및 패널은 영향을 받지 않습니다.
...
- Customer Journey Analytics의 상단 메뉴 모음에서 **[!UICONTROL 구성 요소]** > **[!UICONTROL 환경 설정]**&#x200B;을 선택합니다(제품 관리자만 사용 가능).
...
| | 합계 표시 | 이 총계 수는 일반적으로 [!UICONTROL 총계]와 같거나 그 하위 집합입니다. [!UICONTROL 없음 포함] 옵션을 포함하여 자유 형식 테이블 내에 적용된 테이블 세그먼트를 반영합니다. |
| | 총계 표시 | 이 총 횟수는 수집된 모든 이벤트를 나타내며 경우에 따라 *데이터 보기 합계*&#x200B;라고도 합니다. 세그먼트가 패널 수준에서 또는 자유 형식 테이블 내에서 적용되면 이 합계는 세그먼트 기준과 일치하는 모든 이벤트를 반영하도록 조정됩니다. [정적 행](/help/analysis-workspace/visualizations/freeform-table/workspace-totals.md)이 있는 테이블 또는 분류에서는 총 합계가 지원되지 않습니다. |
...
| | 비율 | 셀에 지표에 대한 퍼센트 값을 표시할지 또는 숨길지를 결정합니다. 예를 들어 지표가 페이지 보기 횟수이면, 퍼센트 값은 행 항목에 대한 페이지 보기 횟수를 해당 열에 대한 총 페이지 보기 횟수로 나눈 수입니다.  참고: 정확하게 말하자면 100%보다 큰 백분율을 표시할 수 있습니다. 열 너비를 너무 크게 늘릴 수 있도록 상한을 1,000%로 이동할 수도 있습니다. |
...
| | 0을 값없음으로 해석 | 값이 0인 셀에 대해, 0을 표시할지 아니면 빈 셀을 표시할지를 결정합니다. 달의 각 날에 대한 데이터를 보고 일부 날은 아직 일어나지 않은 경우 유용합니다.  차후의 날짜에 대해 0을 표시하는 대신 빈 셀을 표시할 수 있습니다. 차트는 이 설정도 준수합니다(예: 이 설정을 선택한 경우 장바구니에 값이 0인 줄 또는 막대가 표시되지 않음). |
...
| | 최대 항목 수 제한 | 모든 시각화에 대한 X축의 항목 수를 줄입니다. 이 환경 설정은 큰 데이터 세트가 있는 경우 유용할 수 있습니다. |
...
| | Y축을 0에 고정 | 차트에 그려진 모든 값이 0을 상당히 초과하는 경우 차트 기본값은 y축의 하단을 0이 아닌 값으로 업데이트합니다. 이 상자를 선택하면 y축이 0이 됩니다(그리고 차트를 다시 그립니다). |
| | 예외 항목을 고정하여 Y축의 크기 조절 | Y축은 예외 항목 값을 사용하여 크기가 조정됩니다. |
| **[라인](/help/analysis-workspace/visualizations/line.md)** | | |
| | 백분율 | 라인 시각화 값을 백분율로 표시합니다. |
| | 범례 표시 | 라인 시각화에 대한 자세한 범례 텍스트를 숨길 수 있습니다. |
| | 최대 항목 수 제한 | 라인 시각화에서 X축의 항목 수를 줄입니다. 이 환경 설정은 큰 데이터 세트가 있는 경우 유용할 수 있습니다. |
| | 이중 축 표시 (해당되는 경우) | 지표가 두 개일 경우에만 적용됩니다. 왼쪽(한 지표에 대해)과 오른쪽(다른 지표에 대해)에 Y축을 놓을 수 있습니다. 이 환경 설정은 그려진 지표의 크기가 매우 다른 경우에 유용합니다. |
| | 표준화 (해당되는 경우) | 지표를 등분 비례에 강제 적용합니다. 이 환경 설정은 그려진 지표의 크기가 매우 다른 경우에 유용합니다. |
| | X축 표시 | 선 차트에 X축을 표시합니다. |
| | Y축 표시 | 선 차트에 Y축을 표시합니다. |
| | Y축 고정 | 차트에 그려진 모든 값이 0보다 상당히 높은 경우 차트 기본값은 y축의 하단을 0이 아닌 상태로 렌더링합니다. 이 상자를 선택하면 y축이 0이 됩니다(그리고 차트를 다시 그리기). |
| | 예외 항목으로 Y축의 크기 조절 | 차트에 여러 개의 지표가 있을 경우에는 사용자는 마우스를 각 예외 항목의 위에 놓아 해당 지표에 대한 신뢰 대역을 확인해야 합니다. 예외 항목 탐지 신뢰 구간은 시각화를 읽기 쉽게 만들기 위해 Y축 크기를 자동으로 조절하지 않습니다. 이 설정을 사용하면 신뢰 구간에서 시각화의 크기를 조정할 수 있습니다. <p>자세한 내용은 [Analysis Workspace에서 예외 항목 보기](/help/analysis-workspace/c-anomaly-detection/view-anomalies.md)를 참조하십시오.</p> |
| | 예측을 Y축으로 확장하도록 허용 | 과거 값의 상한 및 하한 범위를 벗어나는 예측 값이 있는 경우 y축은 이러한 예측된 값의 크기를 자동으로 조정하지 않습니다. 이 옵션을 켜면 예측된 값에 대한 y축의 배율이 제대로 조정되지 않습니다. |
| | 최소 표시 | 최소값 레이블을 오버레이하여 지표의 주기를 빠르게 강조 표시합니다. 참고: 최소값은 차원 내의 전체 값 집합이 아니라 시각화에 표시되는 데이터 포인트에서 파생됩니다. |
...
| | 계산 방법 | 다음 선택 사항 중 하나를 선택합니다. <ul><li>히트</li><li>세션</li><li>개인</li></ul> <p>예를 들어 페이지 보기 수와 함께 사용할 경우, 사람당 페이지 보기 수, 방문용 페이지 보기 수 또는 이벤트당 페이지 보기 수를 선택할 수 있습니다. 히트의 경우 “발생 횟수”는 자유형 테이블에서 Y축 지표로 사용됩니다.</p> |
...
| | 최대 항목 수 제한 | 트리맵 시각화에서 X축의 항목 수를 줄입니다. 이 환경 설정은 큰 데이터 세트가 있는 경우 유용할 수 있습니다. |
...
| | 최대 항목 수 제한 | 분산형 시각화에서 X축의 항목 수를 줄입니다.  이 환경 설정은 큰 데이터 세트가 있는 경우 유용할 수 있습니다. |
| | Y축을 0에 고정 | 차트에 그려진 모든 값이 0보다 상당히 높은 경우 차트 기본값은 y축의 하단을 0이 아닌 상태로 렌더링합니다. 이 상자를 선택하면 y축이 0이 됩니다(그리고 차트를 다시 그립니다). |

## 기본 환경 설정 복원

모든 사용자 환경 설정을 시스템 기본값으로 복원할 수 있습니다. 이 환경 설정은 회사 탭의 관리자 환경 설정에는 영향을 주지 않습니다.

이 작업은 실행 취소할 수 없습니다.

1. Customer Journey Analytics의 상단 메뉴에서 [!UICONTROL **구성 요소**] **>** [!UICONTROL **환경 설정**]&#x200B;을 선택합니다. 또는 Workspace 메뉴에서 **[!UICONTROL 프로젝트]** > **[!UICONTROL 사용자 설정]**&#x200B;을 선택합니다.
...
1. **[!UICONTROL 어두운 테마]**&#x200B;를 사용하도록 설정합니다.
