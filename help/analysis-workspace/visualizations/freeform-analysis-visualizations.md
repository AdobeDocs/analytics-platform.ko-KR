...
어떤 시각화를 선택할지 확실하지 않은 경우 테이블 행에서 **[!UICONTROL 시각화 만들기]** 아이콘을 클릭합니다(마우스 오버 시 사용 가능). 이는 시각화를 추가하는 가장 빠른 방법입니다. 이 아이콘을 클릭하면 기존 학습을 토대로 사용자 데이터에 가장 적합한 시각화가 추측됩니다. 예를 들어 1개의 행을 선택한 경우 추세선 그래프가 생성됩니다. 3개의 필터 행을 선택한 경우 벤 다이어그램이 생성됩니다.
