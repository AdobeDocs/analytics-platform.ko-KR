...
Journey Optimizer와 여정 캔버스 간의 동기화를 이해하려면 다음 동작을 살펴보십시오.

* **데이터 동기화는 단방향으로만 수행됩니다.**
...
* **여정 캔버스에서 여정을 수정하면 동기화가 중지됩니다.**

  Journey Optimizer에서 여정을 변경한 내용은 [여정 캔버스에서 여정이 크게 수정되지 않은 경우에만](#differences-after-modifying-a-journey-in-journey-canvas) 동기화됩니다. 여정 캔버스에서 여정을 수정한 후에는 Journey Optimizer에서 여정에 적용한 변경 사항이 여정 캔버스에 반영되지 않습니다. 여정 캔버스에 반영된 변경 사항을 보기 위해 [여정 캔버스에서 여정을 삭제하고 다시 만들 수 있습니다](/help/analysis-workspace/visualizations/journey-canvas/configure-journey-canvas.md).
...
여정 캔버스에서 Journey Optimizer 여정을 수정한 후 [!UICONTROL **화살표 설정**] 드롭다운 필드에서 사용할 수 있는 옵션은 수정 사항에 따라 변경됩니다. 자세한 내용은 [설정 구성](/help/analysis-workspace/visualizations/journey-canvas/configure-journey-canvas.md)을 참조하십시오.
