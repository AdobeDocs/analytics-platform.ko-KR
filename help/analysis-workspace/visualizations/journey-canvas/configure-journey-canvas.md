...
1. 시각화 상단에 표시되는 다음 설정 중 하나를 구성합니다.
   ...
   | [!UICONTROL **폴아웃 표시**] | 폴아웃 데이터는 여정의 각 노드에서 떨어지는 백분율과 수를 보여줍니다. 폴아웃 데이터는 여정의 컨테이너 설정과 연결된 지표를 기반으로 합니다. 기본 또는 보조 지표를 기반으로 하지 않습니다.<p>기본적으로 컨테이너는 _사람_&#x200B;이므로 폴아웃 데이터에 사용되는 지표는 _사람_&#x200B;입니다. 컨테이너를 _세션_(으)로 변경하면 폴아웃 데이터에 사용되는 지표는 _세션_&#x200B;입니다.</p><p>예를 들어 컨테이너 설정으로 _Person_&#x200B;을(를) 사용하는 폴아웃은 여정의 각 노드에서 바로 다음 노드에 도달하지 못한 사람의 비율과 수를 표시합니다. 사이트에서 다른 작업을 수행했을 수 있지만 바로 다음에 오는 노드에 의해 정의된 기준을 충족하지 않았습니다.</p> <p>여정 캔버스 컨테이너 설정에 대한 자세한 내용은 [여정 캔버스 시각화 작성 시작](#begin-building-a-journey-canvas-visualization)을 참조하십시오. |
