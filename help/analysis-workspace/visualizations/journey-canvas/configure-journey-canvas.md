...
1. 프로젝트에 빈 패널을 추가하고 왼쪽 레일에서 [!UICONTROL **시각화**] 아이콘을 선택한 다음 ![GraphPathing](/help/assets/icons/Branch3.svg) [!UICONTROL **여정 캔버스**] 시각화를 패널로 드래그합니다.
   ...
   [시각화 개요](/help/analysis-workspace/visualizations/freeform-analysis-visualizations.md#add-visualizations-to-a-panel)의 [패널에 시각화 추가](/help/analysis-workspace/visualizations/freeform-analysis-visualizations.md) 섹션에 설명된 방법으로 여정 캔버스 시각화를 추가하십시오.
...
1. 여정 캔버스를 구성하려면 다음 기본 정보를 지정하십시오.
   ...
   | [!UICONTROL **Journey Optimizer 여정**]<!-- name? --> | 여정 캔버스에서 분석의 기준으로 사용할 Journey Optimizer 여정을 선택합니다. 라이브, 정지됨 또는 완료됨 상태의 여정을 사용할 수 있습니다 <p>또는 Analysis Workspace 내에서 분석을 빌드할 빈 캔버스를 원하는 경우 이 옵션을 비워 둘 수 있습니다.</p> <p>여정 캔버스에서 Journey Optimizer 여정을 분석할 때 여정은 Journey Optimizer에서와 동일한 순서, 시퀀스 및 구조로 표시됩니다. 자세한 내용은 [여정 개요](/help/analysis-workspace/visualizations/journey-canvas/journey-canvas.md#analyze-journey-optimizer-journeys)에서 [Journey Optimizer 여정 분석](/help/analysis-workspace/visualizations/journey-canvas/journey-canvas.md)을 참조하십시오.</p><p>**참고**: 이 옵션은 시각화를 추가하고 있는 Analysis Workspace 패널에서 선택한 동일한 데이터 보기에서 Journey Optimizer 데이터가 검색되는 경우에만 표시됩니다. Analysis Workspace의 패널에서 데이터 보기를 변경하는 방법에 대한 자세한 내용은 [Analysis Workspace 개요](/help/analysis-workspace/home.md)를 참조하십시오.</p> |
...
1. 지표, 차원, 차원 항목, 세그먼트 또는 날짜 범위를 왼쪽 레일에서 캔버스로 드래그합니다. [파생 필드](/help/data-views/derived-fields/derived-fields.md)을(를) 기반으로 하는 지표가 지원됩니다. 그러나 [요약 데이터 세트](/help/data-views/summary-data.md)를 기반으로 하는 모든 지표 또는 차원과 계산된 지표는 지원되지 않습니다.
   ...
   | 세그먼트 | 캔버스의 빈 영역 | 노드는 다른 노드와 연결되지 않은 채 구성 요소가 드롭된 위치를 표시합니다.<p>노드에 표시되는 수와 백분율에는 선택한 세그먼트로 세그먼트화된 기본 지표의 합계가 포함됩니다.</p> <p>예를 들어 인물 이 여정에 대한 기본 지표로 선택된 경우, 캔버스의 빈 영역에 오늘의 세그먼트를 추가하면 오늘 이벤트가 있었던 모든 사람이 표시됩니다.</p> |
   | 세그먼트 | 기존 노드 | 기존 노드에 세그먼트를 적용합니다. |
   | 세그먼트 | 노드 2개를 연결하는 화살표 | 노드는 구성 요소가 삭제된 두 기존 노드 사이에 표시되며 두 기존 노드 모두에 연결됩니다. 자세한 내용은 [노드 연결](#connect-nodes)을 참조하십시오.</p><p>구성 요소가 드롭된 패스의 점에 세그먼트를 적용합니다.</p> |
   | 날짜 범위 | 캔버스의 빈 영역 | 노드는 다른 노드와 연결되지 않은 채 구성 요소가 삭제된 위치를 표시합니다.<p>노드에 표시되는 수와 백분율에는 선택한 날짜 범위로 세그먼트화된 기본 지표의 합계가 포함됩니다.</p> <p>예를 들어 인물 이 여정에 대한 기본 지표로 선택된 경우, 이번 달의 날짜 범위를 캔버스의 빈 영역에 추가하면 현재 달 동안 이벤트를 가졌던 모든 사람이 표시됩니다.</p> |
...
여정 캔버스의 여정은 이벤트, 차원 항목 및 세그먼트의 조합을 나타내는 노드와 화살표의 유연한 그래프로 구성됩니다.
...
>[!TIP]
>
>노드를 마우스 오른쪽 단추로 클릭한 다음 [!UICONTROL **노드에서 세그먼트 만들기**]&#x200B;를 선택하여 결합된 노드의 논리를 볼 수 있습니다. 논리는 [!UICONTROL **정의**] 섹션에 표시됩니다.
...
| 날짜 범위 + 지표, 세그먼트 또는 Dimension | AND로 결합됨 |
| 세그먼트 + 지표, 날짜 범위 또는 Dimension | AND로 결합됨 |
...
노드를 마우스 오른쪽 단추로 클릭한 다음 [!UICONTROL **노드에서 세그먼트 만들기**]&#x200B;를 선택하여 연결된 노드의 논리를 볼 수 있습니다. 논리는 [!UICONTROL **정의**] 섹션에 표시됩니다.
