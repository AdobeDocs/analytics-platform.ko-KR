...
| 요소 | 설명 |
| --- | --- |
| 조건부 서식 | 셀에는 선택한 미리 구성된 색상 세트를 적용합니다. 선택한 4가지 사용 가능한 색상 구성표 중 어느 것에 따라 높은 값, 중간점 값 및 낮은 값에 다른 색상이 할당됩니다. <br> 테이블에서 차원을 바꾸면 조건부 서식 제한이 재설정됩니다. 지표를 바꾸면 해당 열에 대한 제한이 재계산됩니다 (지표는 X축에 있고 차원은 Y축에 있음). |
...
| 조건부 서식 팔레트 | 조건부 서식에 사용할 사용 가능한 4가지 색상 구성표 중 하나를 선택합니다. |

## 비기본 속성 모델 사용 {#attribution}

Analysis Workspace은 거의 모든 지표에 대한 [속성](/help/analysis-workspace/attribution/overview.md)을 지원합니다.

1. 자유 형식 테이블 열 옆의 설정(기어) 아이콘을 클릭합니다.

   ![속성 확인란](assets/attribution-checkbox.png)

1. **[!UICONTROL 데이터 설정]**&#x200B;에서 **[!UICONTROL 기본값이 아닌 속성 모델 사용]**&#x200B;을 선택합니다. 다른 속성 모델에 대한 자세한 내용은 [속성 모델](/help/analysis-workspace/attribution/models.md)을 참조하십시오.
