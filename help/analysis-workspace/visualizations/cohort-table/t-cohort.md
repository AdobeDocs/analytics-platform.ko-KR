...
1. (선택 사항) 선택 항목에서 필터를 만듭니다.

   셀(연속 또는 비연속)을 선택한 다음 > 마우스 오른쪽 단추를 클릭합니다. **[!UICONTROL 선택 항목에서 필터 만들기]**.

1. 에서 [필터 빌더](/help/components/filters/manage-filters.md)를 클릭하고 필터를 추가로 편집한 다음 를 클릭합니다. **[!UICONTROL 저장]**.

   저장된 필터는 [!UICONTROL 필터] 패널 [!UICONTROL Analysis Workspace].
