...
데모 비디오는 ![VideoCheckedOut](/help/assets/icons/VideoCheckedOut.svg) [맵 시각화](https://experienceleague.adobe.com/ko/docs/customer-journey-analytics-learn/tutorials/analysis-workspace/visualizations/configure-and-use-the-map-visualization){target="_blank"}를 참조하십시오.
...
1. (조건부) 웹 SDK을 사용하고 있고 위도와 경도가 데이터 스트림에 채워지도록 구성한 경우 또는 Analytics Source 커넥터를 사용하여 이벤트 데이터를 채우는 경우, 위도 및 경도 필드를 스키마에서 이미 사용할 수 있고 올바른 컨텍스트 레이블로 채워져 있어야 합니다.

   이 **[!UICONTROL 위도]** 및 **[!UICONTROL 경도]** 스키마 필드(**[!UICONTROL 이벤트 데이터 세트]** > **[!UICONTROL placeContext]** > **[!UICONTROL 지역]** > **[!UICONTROL _schema]**&#x200B;에서)를 찾아 아직 없는 경우 차원으로 데이터 보기에 끌어서 놓습니다.

   이러한 스키마 필드가 데이터 보기에 차원으로 존재하면 해당 컨텍스트 레이블이 자동으로 적용되고 맵 시각화는 추가 구성 없이 이 필드를 사용합니다.

   ![위도 및 경도 스키마 필드를 데이터 보기에 추가](assets/dataview-lat-long-default.png)

1. (조건부) 위도 및 경도 데이터에 사용할 사용자 지정 차원이 있는 경우 사용자 지정 필드에 컨텍스트 레이블을 구성할 수 있습니다.

   1. **[!UICONTROL 차원]** 섹션에서 경도 데이터가 포함된 차원을 선택합니다.
   ...
   1. (선택 사항) 기본적으로 이러한 차원은 맵 시각화의 도시 또는 우편 번호 수준에 정확하며 Workspace 보고서에 소수점 이하 두 자리를 표시합니다. 맵 시각화에서 1미터 이내로 정확하게 조정하고 Workspace 보고서에 소수점 5자리를 표시하도록 조정할 수 있습니다. 정밀도 수준을 조정하는 방법에 대한 자세한 내용은 [차원의 정확한 위치 구성](#configure-precise-locations-for-dimensions)을 참조하십시오.
...
1. (조건부) 웹 SDK을 사용하고 있고 데이터 스트림에 지리 필드를 채우도록 구성했거나 Analytics Source 커넥터를 사용하여 이벤트 데이터를 채우는 경우, 지리 필드를 스키마에서 이미 사용할 수 있고 올바른 컨텍스트 레이블로 채워야 합니다.
   ...
   **[!UICONTROL 구/군/시]**, **[!UICONTROL 우편 번호]**, **[!UICONTROL 시/도]**(**[!UICONTROL 이벤트 데이터 세트]** > **[!UICONTROL placeContext]** > **[!UICONTROL 지역]**&#x200B;에서)와 같은 적절한 스키마 필드를 찾은 다음 아직 없는 경우 차원으로 데이터 보기에 끌어서 놓습니다.
   ...
   ![데이터 보기에 지역 스키마 필드 추가](assets/dataview-geo-default.png)

1. (조건부) 지역 데이터에 사용할 사용자 지정 차원이 있는 경우 사용자 지정 필드에 컨텍스트 레이블을 구성할 수 있습니다.

   1. 국가 데이터가 포함된 차원을 선택합니다.
...
1. 왼쪽 레일에서 [!UICONTROL **시각화**] 아이콘을 선택한 다음 **[!UICONTROL 맵]** 시각화 ![맵](/help/assets/icons/Globe.svg)을(를) 자유 형식 테이블이 포함된 패널로 드래그합니다.
   ...
   [시각화 개요](/help/analysis-workspace/visualizations/freeform-analysis-visualizations.md)의 [패널에 시각화 추가](/help/analysis-workspace/visualizations/freeform-analysis-visualizations.md#add-visualizations-to-a-panel) 섹션에 설명된 방법으로 맵 시각화를 추가하십시오.
...
1. 구성할 위도 및 경도에 사용할 차원을 선택합니다. 사용 중인 차원에 대한 자세한 내용은 [맵 시각화의 위도와 경도에 필요한 컨텍스트 레이블](#required-context-labels-for-latitude-and-longitude-in-the-map-visualization)을 참조하십시오.
...
1. 대상 빌더를 사용하여 새 대상을 정의합니다. 자세한 내용은 [대상 만들기 및 게시](/help/components/audiences/publish.md)의 [대상 빌더](/help/components/audiences/publish.md#audience-builder)를 참조하십시오.
