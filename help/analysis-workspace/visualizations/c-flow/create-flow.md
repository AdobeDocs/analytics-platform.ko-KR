...
플로우 시각화는 웹 사이트 또는 앱의 특정 전환 이벤트에서 시작되거나 이어지는 여정을 이해하는 데 도움이 됩니다. 차원(및 차원 항목) 또는 지표를 통해 경로를 추적합니다.
...
1. 다음 옵션 중 하나를 사용하여 플로우 시각화를 고정합니다.
   ...
   * 차원 또는 지표의 이름을 입력한 다음 드롭다운 목록에 표시될 때 선택합니다.
...
1. (선택 사항) 선택 **[!UICONTROL 고급 설정 표시]** 다음 옵션 중 하나를 구성하려면:
   ...
   | **[!UICONTROL 처음/마지막 발생으로 제한]** | 차원/항목/지표의 처음/마지막 발생으로 시작/종료하는 경로로 경로를 제한합니다. 아래 섹션 참조, [&#39;처음/마지막 발생으로 제한&#39;에 대한 예제 시나리오](#example-scenario-for-limit-to-firstlast-occurrence)를 참조하십시오. |
   | **[!UICONTROL 열 수]** | 흐름 다이어그램에 사용할 열의 수입니다. 최대 5개의 열을 지정할 수 있습니다. |
   | **[!UICONTROL 열당 항목 확장됨]** | 각 열에 원하는 항목의 수입니다. 열당 최대 10개의 항목을 확장하도록 지정할 수 있습니다. |
   ...
   >[!IMPORTANT]
   >
   >의 조합 **[!UICONTROL 열 수]** 및 **[!UICONTROL 열당 항목 확장됨]** 플로우 시각화를 만드는 데 필요한 기본 요청 수를 결정합니다. 숫자가 높을수록 시각화를 렌더링하는 데 더 오래 걸립니다.
...
>[!INFO]
>
>**예:** 사용자가 사이트에서 가장 방문 빈도가 높은 페이지로 이동한 경로를 추적한다고 가정해 봅시다.
>
>이렇게 하려면 다음을 수행합니다
>
>1. 위에 설명된 대로 플로우 시각화 만들기를 시작합니다.
>1. 드래그 [!UICONTROL **페이지**] 차원 입력 **[!UICONTROL 다음 포함]** 필드, 선택 [!UICONTROL **빌드**].
>1. 플로우 시각화는 시각화 중앙에 있는 포커스 노드에 표시되는 가장 많이 본 페이지로 빌드됩니다. 또한 해당 페이지(포커스 노드 왼쪽)로 이어지는 상위 페이지와 해당 포커스 페이지(포커스 노드 오른쪽)로 이어지는 상위 페이지도 표시됩니다.
>1. 에 설명된 대로 플로우에서 데이터 분석 [흐름 출력 보기 및 변경](#view-and-change-the-flow-output).
...
* 사용된 시리즈는 컨테이너에 따라 다릅니다. 을 사용하는 경우 **[!UICONTROL 방문]** 컨테이너인 경우 일련의 이벤트가 세션이 됩니다. 을 사용하는 경우 **[!UICONTROL 방문자]** 컨테이너인 경우 일련의 이벤트는 제공된 날짜 범위에서 지정된 사용자의 모든 이벤트가 됩니다.
...
&quot;처음/마지막 발생으로 제한&quot;이 비활성화된 경우 이 단일 연속 이벤트 수는 &quot;장바구니에 추가&quot; 2회 발생으로 카운트됩니다.
예상 흐름 출력:
“장바구니에 추가” (2) —> “제품” (1)
-> “청구” (1)

그러나 &quot;처음/마지막 발생으로 제한&quot;이 활성화된 경우 &quot;장바구니에 추가&quot;의 처음 발생만 분석에 포함됩니다.
예상 흐름 출력:
“장바구니에 추가” (1) —> “제품” (1)

### 동일한 일련의 이벤트를 고려하되 다음 설정을 사용하십시오.

* [!UICONTROL 장바구니에 추가] (차원 항목)로 끝남
* [!UICONTROL 페이지] 경로 지정 차원
* [!UICONTROL 방문] 컨테이너

If **[!UICONTROL 처음/마지막 발생으로 제한]** 은(는) *비활성화됨*, 그런 다음 이 단일 연속 이벤트 수는 &quot;장바구니에 추가&quot; 2회 발생으로 카운트됩니다.
예상 흐름 출력:
“제품” (2) &lt;— “장바구니에 추가” (2)
