...
1. 다음 세 가지 옵션 중 하나를 사용하여 플로우 시각화를 고정합니다.
   ...
   * [!UICONTROL 다음으로 끝남] (지표, 차원 또는 항목)
   ...
   이러한 각각의 범주는 화면에 &quot;놓기 영역&quot;으로 표시됩니다. 다음 세 가지 방법으로 드롭 영역을 채울 수 있습니다.

   * 드롭다운 메뉴를 사용하여 지표나 차원을 선택합니다.
   * 차원 또는 지표 목록에서 항목을 드래그합니다.
   * 검색 을 사용하여 찾고 있는 지표 또는 차원을 찾습니다.
...
1. (선택 사항) **[!UICONTROL 고급 설정 표시]** 고급 설정을 구성하려면:
   ...
   | **[!UICONTROL 처음/마지막 발생으로 제한]** | 차원/항목/지표의 첫 번째/마지막 발생으로 시작/종료되는 경로로 경로를 제한합니다. 자세한 내용은 &#39;첫 번째/마지막 발생 횟수로 제한&#39;에 대한 아래의 &quot;시나리오 예&quot; 섹션을 참조하십시오. |
...
각 열 위에 마우스를 가져가면 필터가 나타납니다. 필터를 클릭하면 오늘 자유 형식 테이블에 있는 것과 동일한 필터 대화 상자가 표시됩니다. 이 필터는 자유 형식 테이블에서와 동일하게 작동합니다.

* 고급 설정을 사용하여 연산자 목록과 함께 특정 기준을 포함하거나 제외합니다.
* 목록에서 항목을 필터링하면 해당 특정 열이 필터링을 반영합니다. 필터는 필터에 허용되는 항목만 표시하도록 축소하거나 필터에 원하는 항목 하나를 제외한 모든 항목을 제거합니다.
* 나머지 노드로 데이터가 유입되는 한 모든 다운스트림 및 업스트림 열은 유지해야 합니다.
* 적용되면 필터링 중인 열 위에 필터 아이콘이 파란색으로 표시됩니다.
* 필터를 제거하려면 필터 아이콘을 클릭하여 필터 메뉴를 엽니다. 적용된 필터를 제거한 다음 **[!UICONTROL 저장]**. 흐름은 필터링되지 않은 이전 상태로 반환되어야 합니다.
...
| [!UICONTROL 항목 제외]/[!UICONTROL 제외된 항목 복원] | 열에서 특정 노드를 제거하고 열 맨 위에 필터로 자동으로 만듭니다. 제외된 항목을 복원하려면 다시 마우스 오른쪽 단추를 클릭하고 **[!UICONTROL 제외된 항목 복원]**. 열 상단에 있는 필터를 열고 방금 제외된 항목이 있는 필상자를 제거할 수도 있습니다. |

## &#39;첫 번째/마지막 발생 횟수로 제한&#39;에 대한 예제 시나리오

이 옵션을 사용할 때는 다음 사항에 유의하십시오.

* **[!UICONTROL 첫 번째/마지막 항목으로 제한]** 는 시리즈의 첫 번째/마지막 항목만 계산합니다. 기타 모든 발생 횟수 **[!UICONTROL 다음으로 시작]** 또는 **[!UICONTROL 다음으로 끝남]** 기준은 무시됩니다.
* 와 함께 사용하는 경우 **[!UICONTROL 다음으로 시작]** 흐름, 시작 기준과 일치하는 첫 번째 발생 항목만 포함됩니다.
* 와 함께 사용하는 경우 **[!UICONTROL 다음으로 끝남]** 흐름, 종료 기준과 일치하는 마지막 항목만 포함됩니다.
* 사용된 시리즈는 컨테이너에 따라 다릅니다. 를 사용하는 경우 **[!UICONTROL 방문]** 컨테이너 를 사용하는 경우 일련의 히트가 세션이 됩니다. 를 사용하는 경우 **[!UICONTROL 방문자]** 컨테이너: 일련의 히트가 제공된 날짜 범위에서 주어진 사용자에 대한 모든 히트가 됩니다.
...
* 다음으로 시작[!UICONTROL  장바구니에 추가] (Dimension 항목)
* [!UICONTROL 페이지] 경로 지정 차원
* [!UICONTROL 방문] 컨테이너

첫 번째/마지막 발생으로 제한 이 비활성화되어 있으면 이 일련의 단일 히트는 &quot;장바구니에 추가&quot;의 2개 항목을 계산합니다.
예상 흐름 출력: &quot;장바구니에 추가&quot; (2) —> &quot;Products&quot; (1) -> &quot;Billing&quot; (1)

하지만 &quot;첫 번째/마지막 발생 횟수로 제한&quot;이 활성화되어 있으면 &quot;장바구니에 추가&quot;의 첫 번째 발생만 분석에 포함됩니다.
예상 흐름 출력: &quot;장바구니에 추가&quot; (1) —> &quot;Products&quot; (1)

### 동일한 일련의 히트를 고려하지만 다음 설정을 사용합니다.

* 다음으로 끝남 [!UICONTROL 장바구니에 추가] (Dimension 항목)
* [!UICONTROL 페이지] 경로 지정 차원
* [!UICONTROL 방문] 컨테이너
...
그러나, **[!UICONTROL 첫 번째/마지막 항목으로 제한]** is *활성화됨*, 의 마지막 발생 항목만 [!UICONTROL 장바구니에 추가] 이 분석에 포함됩니다.
