...
1. [!UICONTROL **템플릿**] 섹션에서 다음 탭 중 하나를 선택합니다.
   ...
   * **[!UICONTROL _login_company_name _템플릿]**: 조직에 대해 만들어진 모든 회사 템플릿을 표시합니다.
     ...
     관리자만 회사 템플릿을 만들 수 있습니다. 회사 서식 파일을 만드는 방법에 대한 자세한 내용은 [서식 파일 만들기 및 관리](/help/analysis-workspace/templates/create-templates.md)를 참조하십시오.
...
1. (선택 사항) 데이터 보기에서 사용할 수 없는 구성 요소가 포함된 템플릿을 볼 수 있습니다. (기본적으로 템플릿은 데이터 보기에서 사용할 수 있는 구성 요소를 사용하는 경우에만 표시됩니다.)
   ...
   1. 추가 구성 요소가 필요한 템플릿을 표시하려면 **[!UICONTROL 사용할 준비가 되지 않음]**&#x200B;을(를) 선택하십시오.

      ![구성 요소가 없는 템플릿 사용](assets/template-not-ready.png)

1. 선택한 템플릿을 기반으로 보고서를 만들려면 템플릿을 선택합니다.

1. (조건부) 템플릿에 데이터 보기에서 사용할 수 없는 구성 요소가 포함되어 있는 경우 데이터 보기가 템플릿과 호환되지 않으며 누락된 구성 요소를 표시하는 호환되지 않는 데이터 보기 대화 상자가 표시됩니다.

   다음 중 하나를 수행합니다.

   * **[!UICONTROL 데이터 보기 변경]** 드롭다운 메뉴에서 다른 데이터 보기를 선택하십시오.

   * 누락된 구성 요소가 있는 템플릿을 보려면 **[!UICONTROL 계속]**&#x200B;을 선택하십시오.
...
1. (선택 사항) [!UICONTROL **프로젝트**] > [!UICONTROL **저장**]&#x200B;을 선택하여 템플릿을 프로젝트로 저장합니다.
   ...
   템플릿은 새 프로젝트로 저장됩니다. 기존 템플릿은 수정되지 않습니다. 프로젝트 저장에 대한 자세한 내용은 [프로젝트 저장](/help/analysis-workspace/build-workspace-project/save-projects.md)을 참조하세요.
...
>title="특정 페이지를 방문한 후 바로 사람들이 방문하는 가장 일반적인 위치를 봅니다."
>abstract="**특정 페이지를 방문한 후 사용자 동작을 더 잘 이해하는 데 도움이 됩니다**.<br/>**학습한 내용에 따라** 페이지 디자인이나 레이아웃을 최적화하여 구매할 페이지나 리뷰를 남길 페이지와 같이 더 바람직한 페이지로 안내할 수 있는지 여부를 평가하는 등 여러 가지 작업을 수행할 수 있습니다.<br/>이 템플릿은 페이지 차원과 이벤트 지표를 사용합니다."
...
>title="사람들이 특정 페이지를 방문하기 전에 바로 방문하는 가장 일반적인 위치를 봅니다."
>abstract="**이를 통해**&#x200B;특정 페이지로 가장 많은 트래픽을 보내는 페이지를 더 잘 이해할 수 있습니다.<br/>**학습한 내용에 따라** 이전 페이지로 표시되지 않는 페이지에 현재 페이지로 연결되는 더 눈에 띄는 링크가 필요한지 여부를 평가하는 등 여러 가지 작업을 수행할 수 있습니다."
...
>title="모든 주문 내에서 구매된 모든 제품의 금액을 조회합니다. 데이터는 일정 기간 동안 표시되며 이전 기간과 비교됩니다."
...
>abstract="**시간이 지남에 따라 제품 및 서비스에 대한 관심이 어떻게 증가 또는 감소하는지 이해하는 데 도움이 됩니다**. 세그먼트를 적용하여 가장 많은 주문을 하는 고객 또는 지역과 시간 경과에 따른 주문 트렌딩을 파악할 수 있습니다.<br/>**학습한 내용을 바탕으로** 캠페인이 시작되기 전과 후의 주문을 비교하여 최근에 시작된 마케팅 캠페인의 효과를 평가하는 등 다양한 작업을 수행할 수 있습니다. 또는 전년 대비 휴일 주문을 비교할 수도 있습니다.<br/>이 템플릿은 일 차원과 주문 지표를 사용합니다."
...
| [!UICONTROL **페이지**] | <!--duplicated in Engagement section--> 방문 빈도가 가장 높은 페이지와 가장 낮은 페이지를 파악합니다. <p>**이를 통해** 대상자와 대상자가 가장 관심을 갖는 정보의 유형을 더 잘 이해할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 조회수가 적은 페이지의 가시성을 높이기 위해 페이지 메타데이터를 조정하거나 가장 많이 조회된 페이지의 콘텐츠를 향상시키는 데 시간을 할애하는 등 다양한 작업을 수행할 수 있습니다.</p><p>이 템플릿은 페이지 차원 및 페이지 보기 횟수 지표를 사용합니다.</p> |
...
| [!UICONTROL **다음 페이지**] | 특정 페이지를 방문한 후 바로 사람들이 방문하는 가장 일반적인 위치를 봅니다. <p>**특정 페이지를 방문한 후 사용자 동작을 더 잘 이해하는 데 도움이 됩니다**.</p><p>**학습한 내용에 따라** 페이지 디자인이나 레이아웃을 최적화하여 구매할 페이지나 리뷰를 남길 페이지와 같이 더 바람직한 페이지로 안내할 수 있는지 여부를 평가하는 등 여러 가지 작업을 수행할 수 있습니다.</p> <p>이 템플릿은 페이지 차원과 이벤트 지표를 사용합니다.</p> |
| [!UICONTROL **이전 페이지**] | 사람들이 특정 페이지를 방문하기 전에 바로 방문하는 가장 일반적인 위치를 봅니다. <p>**이를 통해**&#x200B;특정 페이지로 가장 많은 트래픽을 보내는 페이지를 더 잘 이해할 수 있습니다.</p><p>**학습한 내용에 따라** 이전 페이지로 표시되지 않는 페이지에 현재 페이지로 연결되는 더 눈에 띄는 링크가 필요한지 여부를 평가하는 등 여러 가지 작업을 수행할 수 있습니다.</p><p>이 템플릿은 페이지 차원과 이벤트 지표를 사용합니다.</p> |
...
| [!UICONTROL **주문**] | <!--duplicated in Web Conversion section-->총 구매 이벤트 수를 확인할 수 있습니다. 데이터는 일정 기간 동안 표시되며 이전 기간과 비교됩니다. <p>**시간이 지남에 따라 제품 및 서비스에 대한 관심이 어떻게 증가 또는 감소하는지 이해하는 데 도움이 됩니다**. 세그먼트를 적용하여 가장 많은 주문을 하는 고객 또는 지역과 시간 경과에 따른 주문 트렌딩을 파악할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 캠페인이 시작되기 전과 후의 주문을 비교하여 최근에 시작된 마케팅 캠페인의 효과를 평가하는 등 다양한 작업을 수행할 수 있습니다. 또는 전년 대비 휴일 주문을 비교할 수도 있습니다.</p><p>이 템플릿은 일 차원과 주문 지표를 사용합니다.</p> |
...
>title="사이트에서 현재 수집 중인 차원 및 지표를 확인합니다."
>abstract="**사이트 트렌드를 이해하는 데 도움이 됩니다**.<br/>**학습한 내용에 따라** 현재 마케팅 콘텐츠 및 캠페인에 응답하고 능동적으로 성과를 관리하는 등 여러 가지 작업을 수행할 수 있습니다."
...
>title="모든 디지털 기기에서 미디어 오디오 사용의 트렌드와 주요 지표를 볼 수 있습니다."
...
>title="모든 디지털 기기에서 미디어 사용에 대한 경향과 최상위 지표를 볼 수 있습니다."
>abstract="**이를 통해** 일반 사용자가 사이트를 방문하는 횟수, 사람들이 사이트에 재방문하는 빈도 및 재방문 사이의 일 수를 더 잘 이해할 수 있습니다.<br/>**학습한 내용을 바탕으로** 사용자를 사이트를 재방문하도록 하는데 가장 효과적인 콘텐츠가 무엇인지 분석하는 등 다양한 작업을 수행할 수 있습니다.<br/>이 템플릿은 방문 횟수 지표와 고유 방문자 수 지표를 사용합니다."
...
>abstract="**특정 페이지에서 방문자에게 페이지를 다시 로드하라는 메시지가 표시될 수 있는 문제를 식별하는 데 도움이 될 수 있습니다**.<br/>**학습한 내용을 바탕으로** 어떤 페이지에 해결해야 할 문제가 있는지 평가하는 등 다양한 작업을 수행할 수 있습니다.<br/>이 템플릿은 다시 로드 지표를 사용합니다."
...
>title="Adobe Experience Manager Forms에 대한 성능 데이터를 봅니다."
>abstract="**이를 통해** Adobe Experience Manager의 가치 실현을 더 잘 이해할 수 있습니다.<br/>**학습한 내용을 바탕으로** Experience Manager 설정을 최적화하는 등 다양한 작업을 수행할 수 있습니다."
...
>abstract="**이를 통해** 사용자가 특정 페이지와 상호 작용하는 방식을 더 잘 이해하는 데 도움이 될 수 있습니다.<br/>**학습한 내용을 바탕으로** 일정 기간 동안 페이지의 성능을 분석하거나 페이지로 트래픽을 유도하는 요인을 더 잘 이해하는 등 다양한 작업을 수행할 수 있습니다.<br/>이 템플릿은 페이지 보기 수 지표를 사용합니다. 또한 선 시각화 및 흐름 시각화도 사용합니다."
...
| [!UICONTROL **페이지**] | <!--duplicated in Most popular section-->방문 빈도가 가장 높은 페이지와 가장 낮은 페이지를 파악합니다. <p>**이를 통해** 대상자와 대상자가 가장 관심을 갖는 정보의 유형을 더 잘 이해할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 조회수가 적은 페이지의 가시성을 높이기 위해 페이지 메타데이터를 조정하거나 가장 많이 조회된 페이지의 콘텐츠를 향상시키는 데 시간을 할애하는 등 다양한 작업을 수행할 수 있습니다.</p><p>이 템플릿은 페이지 차원 및 페이지 보기 횟수 지표를 사용합니다.</p> |
...
| [!UICONTROL **웹 콘텐츠 사용량**] | 어떤 웹 콘텐츠가 가장 많이 소비되고 사용자의 관심을 끌었는지 확인할 수 있습니다.<p>**이를 통해** 사람들이 사이트에 처음 들어오는 위치, 사이트에서 사람들이 가장 많이 방문하는 섹션, 사이트에서 사람들을 멀어지게 할 가능성이 가장 높은 페이지를 더 잘 이해할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 사이트에서 가장 중요한 페이지로 이동하는 경로와 사이트에서 사람들을 멀어지게 할 가능성이 높은 페이지를 평가하는 등 다양한 작업을 수행할 수 있습니다.</p> <p>이 템플릿은 페이지 차원 및 페이지 보기 수 지표, 방문 횟수 지표, 고유 방문자 수 지표, 시작 비율 지표, 바운스 비율 지표, 종료 비율 지표 및 컨텐츠 속도 지표를 사용합니다. 또한 진입, 종료 및 상위 섹션에 흐름 시각화를 사용합니다.</p> |
...
>title="사이트에서 각 제품 카테고리와 연결된 방문 횟수를 봅니다. 이 기능은 제품 변수를 사용하고 제품 카테고리에 대한 지표를 보려는 구현에 유용합니다. 사이트에 제품이 없는 경우 이 템플릿을 채우는 차원이 의도적으로 비어 있을 수 있습니다."
>abstract="**이를 통해 최다 판매 제품이나 가장 많이 본 제품을 더 잘 이해할 수 있습니다**. &lt;/br/>**학습한 내용에 따라, 특정 제품에 대한 마케팅 캠페인의 효과를 측정하는 것과 같이**&#x200B;할 수 있습니다.<br/>이 서식 파일은 범주 차원과 방문 횟수 지표를 사용합니다. "
...
>abstract="**이를 통해**&#x200B;상거래 활동이 매출액 증가에 어떻게 기여하고 있는지 더 잘 이해할 수 있습니다.<br/>**학습한 내용에 따라, 가장 ROI가 높은 활동에 예산을 조정하는 등**&#x200B;다양한 작업을 수행할 수 있습니다."
...
>title="장바구니, 체크아웃 및 주문을 표시하는 단계 시각화에서 제품 전환을 봅니다. 전환율, 매출 평균, 단위 평균 및 주문 평균도 볼 수 있습니다."
>abstract="**전환 프로세스 중에 사람들이 진행 및 중단하는 방법을 이해하는 데 도움이 됩니다**.<br/>**학습한 내용에 따라** 웹 사이트를 개선하여 보다 원활한 체크아웃 프로세스를 용이하게 하는 등 여러 가지 작업을 수행할 수 있습니다."
...
>abstract="**이를 통해** 제품에 대한 고객의 관심이 장바구니에 추가할 만큼 높은 전환 단계 부분을 더 잘 이해하는 데 도움이 될 수 있습니다.<br/>**학습한 내용을 바탕으로** 모든 고객을 위한 제품 추천 개선과 같은 다양한 작업을 수행할 수 있습니다. 이는 동일한 장바구니에 추가될 빈번한 제품을 분석하고, 장바구니에 이미 있는 항목을 기반으로 관련 제품을 제안하여 수행할 수 있습니다."
...
>title="세션, 장바구니 및 주문을 표시하는 단계 시각화에서 구매 전환을 봅니다. 전환율, 매출 평균, 단위 평균 및 주문 평균도 볼 수 있습니다."
>abstract="**전환 프로세스 중에 사람들이 진행 및 중단하는 방법을 이해하는 데 도움이 됩니다**.<br/>**학습한 내용에 따라** 웹 사이트를 개선하여 보다 원활한 체크아웃 프로세스를 용이하게 하는 등 여러 가지 작업을 수행할 수 있습니다."
...
| 템플릿 이름 | 이 템플릿 <!-- What do you do with it? What can it help you learn? and What are the potential actions? -->을(를) 사용하는 이유 |
| --- | --- | 
| [!UICONTROL **제품 전환 단계**] | 장바구니, 체크아웃 및 주문을 표시하는 단계 시각화에서 제품 전환을 봅니다. 전환율, 매출 평균, 단위 평균 및 주문 평균도 볼 수 있습니다.<p>**전환 프로세스 중에 사람들이 진행 및 중단하는 방법을 이해하는 데 도움이 됩니다**.</p><p>**학습한 내용에 따라** 웹 사이트를 개선하여 보다 원활한 체크아웃 프로세스를 용이하게 하는 등 여러 가지 작업을 수행할 수 있습니다.</p> |
...
| **장바구니 보기 수** | 사람들이 장바구니를 조회한 횟수를 확인할 수 있습니다. <p>**이를 통해** 장바구니 포기율을 낮추기 위해 체크아웃 경험을 더 잘 이해하거나 다양한 제품 간의 장바구니 추가와 체크아웃 사이의 시간을 분석할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 장바구니에 가장 오래 보관되어 포기 위험이 가장 큰 제품에 대한 프로모션을 제공하는 등 다양한 작업을 수행할 수 있습니다.</p><p>이 템플릿은 장바구니 보기 수 지표를 사용합니다. |
...
| **구매 전환 단계** | 세션, 장바구니 및 주문을 표시하는 단계 시각화에서 구매 전환을 봅니다. 전환율, 매출 평균, 단위 평균 및 주문 평균도 볼 수 있습니다.<p>**전환 프로세스 중에 사람들이 진행 및 중단하는 방법을 이해하는 데 도움이 됩니다**.</p><p>**학습한 내용에 따라** 웹 사이트를 개선하여 보다 원활한 체크아웃 프로세스를 용이하게 하는 등 여러 가지 작업을 수행할 수 있습니다.</p> |
...
>[!CONTEXTUALHELP]
>id="template--people"
>title="내 브랜드와 상호 작용하는 사람 수를 봅니다."
>abstract="**이를 통해** 사이트의 사용 트렌드를 더 잘 이해할 수 있습니다.<br/>**학습한 내용에 따라** 사이트에 대한 새 방문자를 생성하는 데 있어 최근 마케팅 노력의 효과를 측정하는 것과 같이 여러 가지 작업을 수행할 수 있습니다."
...
>[!CONTEXTUALHELP]
>id="template--bots"
>title="사이트의 보트 트래픽에 대한 페이지 보기 수 및 트렌드를 봅니다."
>abstract="**이렇게 하면**&#x200B;사용자가 구성한 보트 규칙에 따라 보고에서 필터링되는 보트 트래픽의 양을 더 잘 이해하는 데 도움이 됩니다.<br/>**학습한 내용에 따라** 새로운 패턴을 식별하기 위해 보트 활동을 계속 모니터링하는 등 여러 가지 작업을 수행할 수 있습니다."
...
>title="처음 방문자와 반복 방문자를 비교하여 봅니다."
>abstract="**이렇게 하면 고객의 충성도를 유지하는 사이트의 효율성 또는 새 고객을 획득하는 속도를 더 잘 이해하는 데 도움이 됩니다**.<br/>**학습한 내용에 따라 나중에 처음 방문하는 사람에게 나중에 구입하도록 인센티브를 제공하는 등 여러 가지 작업을 수행할 수 있습니다**."
...
>title="다양한 채널에서 개별 사용자 행동을 봅니다."
>abstract="**이를 통해** 여러 접점에서 이루어지는 전체 고객 여정 및 상호 작용을 더 잘 이해할 수 있습니다.<br/>**학습한 내용에 따라 사용자 환경 설정을 보다 효과적으로 타겟팅하기 위해 마케팅 활동을 개인화하는 등 여러 가지 작업을 수행할 수 있습니다**."
...
>abstract="**이를 통해** 방문자가 어느 시간대에 활동하고 있는지 더 잘 이해할 수 있습니다.<br/>**학습한 내용에 따라 최소 인원수에 영향을 주는 사이트 유지 관리 조정과 같은 여러 가지 작업을 수행할 수 있습니다**."
...
>title="맵 시각화에서 방문자 위치의 개요를 봅니다."
>abstract="**이를 통해**&#x200B;사이트를 방문하는 방문자의 위치를 더 잘 파악할 수 있습니다. <br/>**학습한 내용에 따라, 가장 흥미와 기회가 있는 곳에서 집중 마케팅 리소스와 같은 여러 가지 작업을 수행할 수 있습니다**."
...
>abstract="**사이트가 구매 행동에 미치는 영향을 더 잘 이해하는 데**&#x200B;도움이 됩니다.<br/>**학습한 내용을 바탕으로** 재방문하여 구매하는 방문자에 초점을 맞추는 등 다양한 작업을 수행하여 신규 방문자에게도 비슷한 행동을 장려할 수 있습니다.<br/>이 템플릿은 고객 충성도 차원을 사용합니다."
...
>title="운영 체제, 브라우저 및 장치와 같이 사용자가 사이트에 액세스하는 데 사용하는 기술과 관련된 정보를 봅니다."
>abstract="**이렇게 하면**&#x200B;사이트에 액세스할 때 가장 자주 사용되는 기술을 더 잘 이해할 수 있습니다.<br/>**학습한 내용에 따라** 사용 중인 기술에 맞게 사이트를 최적화하는 등 여러 가지 작업을 수행할 수 있습니다."
...
| 템플릿 이름 | 이 템플릿 <!-- What do you do with it? What can it help you learn? and What are the potential actions? -->을(를) 사용하는 이유 |
| --- | --- | 
| [!UICONTROL **처음 방문자와 반복 방문자 비교**] | 처음 방문자와 반복 방문자를 비교하여 봅니다. <p>**이렇게 하면 고객의 충성도를 유지하는 사이트의 효율성 또는 새 고객을 획득하는 속도를 더 잘 이해하는 데 도움이 됩니다**.</p><p>**학습한 내용에 따라 나중에 처음 방문하는 사람에게 나중에 구입하도록 인센티브를 제공하는 등 여러 가지 작업을 수행할 수 있습니다**.</p><!-- This template uses the --> |
| **개인 ID** | 다양한 채널에서 개별 사용자 행동을 봅니다.<p>**이를 통해** 여러 접점에서 이루어지는 전체 고객 여정 및 상호 작용을 더 잘 이해할 수 있습니다.</p><p>**학습한 내용에 따라 사용자 환경 설정을 보다 효과적으로 타겟팅하기 위해 마케팅 활동을 개인화하는 등 여러 가지 작업을 수행할 수 있습니다**.</p><!-- This template uses the --> |
...
| **기술 개요** | 운영 체제, 브라우저 및 장치와 같이 사람들이 사이트에 액세스하는 데 사용하는 기술과 관련된 정보를 봅니다. <p>**이렇게 하면**&#x200B;사이트에 액세스할 때 가장 자주 사용되는 기술을 더 잘 이해할 수 있습니다.</p><p>**학습한 내용에 따라** 사용 중인 기술에 맞게 사이트를 최적화하는 등 여러 가지 작업을 수행할 수 있습니다.</p> |
...
>abstract="**이를 통해** 사이트로 유입되는 트래픽 양과 고객이 전환되는지 여부를 더 잘 이해할 수 있습니다.<br/>**학습한 내용에 따라** 광고 캠페인의 비용 효율성 예측과 같은 여러 가지 작업을 수행할 수 있습니다."
...
>abstract="**검색 결과에 페이지가 얼마나 많이 표시되는지 이해하는 데**&#x200B;도움이 됩니다.<br/>**학습한 내용에 따라** SEO 전략을 개선하여 콘텐츠가 검색 결과의 첫 페이지에 표시되도록 하는 등 여러 가지 작업을 수행할 수 있습니다."
...
>title="캠페인에 대한 클릭스루 및 체크아웃 수를 확인합니다."
>abstract="**마케팅 캠페인이 전환을 유도하는 방법을 더 잘 이해하는 데 도움이 됩니다**.<br/>**학습한 내용에 따라** 가장 많은 ROI를 창출하는 마케팅 캠페인을 결정하는 등 여러 가지 작업을 수행할 수 있습니다."
...
>abstract="**이를 통해** 검색에서 사용자가 사이트 트래픽을 초래하는 키워드를 더 잘 이해할 수 있습니다. <br/>**학습한 내용에 따라** 사용 중인 키워드와 사이트 트래픽을 유도하는 키워드 간의 SEO 간격을 식별하고 채우는 등의 여러 작업을 수행할 수 있습니다.<br/>이 템플릿은 검색 키워드 차원을 사용합니다."
...
>abstract="**이를 통해** 검색에서 사용자가 사이트 트래픽을 초래하는 키워드를 더 잘 이해할 수 있습니다.<br/>**학습한 내용에 따라** 사용 중인 키워드와 사이트 트래픽을 유도하는 키워드 간의 SEO 간격을 식별하고 채우는 등의 여러 작업을 수행할 수 있습니다. <br/>이 템플릿은 검색 키워드 - 유료 차원을 사용합니다. "
...
>abstract="**이를 통해** 검색에서 사용자가 사이트 트래픽을 초래하는 키워드를 더 잘 이해할 수 있습니다.<br/>**학습한 내용에 따라** 사용 중인 키워드와 사이트 트래픽을 유도하는 키워드 간의 SEO 간격을 식별하고 채우는 등의 여러 작업을 수행할 수 있습니다.<br/>이 템플릿은 검색 키워드 - 자연어 차원을 사용합니다. "
...
>abstract="**이를 통해** 트래픽이 가장 많이 발생하는 서드파티 사이트를 파악할 수 있습니다. 링크는 외부 사이트에 있어야 하며, 방문자가 링크를 클릭하여 차원 항목을 표시해야 합니다.<br/>**학습한 내용에 따라** 콘텐츠를 만들거나 조정하는 등 다양한 작업을 수행하여 상위 참조 도메인에서 온 방문자의 관심사에 더 가깝게 맞출 수 있습니다. <br/>이 템플릿은 참조 도메인 차원을 사용합니다."
...
| [!UICONTROL **마케팅 채널**] > [!UICONTROL **첫 번째 터치 채널**] | 해당 방문자의 참여 기간(기본적으로 30일) 동안 방문자가 일치하는 첫 번째 마케팅 채널을 봅니다. <p>**이를 통해** 사이트로 초기 트래픽을 유도하는 마케팅 채널을 더 잘 이해할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 가장 효과적인 분야에 마케팅 활동을 집중하는 등 다양한 작업을 수행할 수 있습니다.</p><p>이 템플릿은 첫 번째 터치 채널 차원을 사용합니다.</p> |
...
| [!UICONTROL **캠페인**] > [!UICONTROL **캠페인 전환 단계**] | 캠페인에 대한 클릭스루 및 체크아웃 수를 확인합니다. <p>**마케팅 캠페인이 전환을 유도하는 방법을 더 잘 이해하는 데 도움이 됩니다**.</p><p>**학습한 내용에 따라** 가장 많은 ROI를 창출하는 마케팅 캠페인을 결정하는 등 여러 가지 작업을 수행할 수 있습니다.</p> |
...
| **검색 키워드 - 모두** | 유료 검색이든 자연어 검색이든, 방문자가 사이트에 접속하는 데 사용하는 검색 키워드를 확인할 수 있습니다. <p>**이를 통해** 검색에서 사용자가 사이트 트래픽을 초래하는 키워드를 더 잘 이해할 수 있습니다. </p><p>**학습한 내용에 따라** 사용 중인 키워드와 사이트 트래픽을 유도하는 키워드 간의 SEO 간격을 식별하고 채우는 등의 여러 작업을 수행할 수 있습니다.</p><p>이 템플릿은 검색 키워드 차원을 사용합니다. </p> |
| **검색 키워드 - 유료** | 방문자가 사이트에 도달하는 데 사용하는 검색 키워드를 확인할 수 있으며, 이 키워드는 유료 검색 감지와 일치합니다.<p>**이를 통해** 검색에서 사용자가 사이트 트래픽을 초래하는 키워드를 더 잘 이해할 수 있습니다.</p><p>**학습한 내용에 따라** 사용 중인 키워드와 사이트 트래픽을 유도하는 키워드 간의 SEO 간격을 식별하고 채우는 등의 여러 작업을 수행할 수 있습니다. </p><p>이 템플릿은 검색 키워드 - 유료 차원을 사용합니다. </p> |
| **검색 키워드 - 자연어** | 방문자가 사이트에 도달하는 데 사용하는 검색 키워드를 확인할 수 있으며, 이 키워드는 유료 검색 감지와 일치하지 않습니다.<p>**이를 통해** 검색에서 사용자가 사이트 트래픽을 초래하는 키워드를 더 잘 이해할 수 있습니다.</p><p>**학습한 내용에 따라** 사용 중인 키워드와 사이트 트래픽을 유도하는 키워드 간의 SEO 간격을 식별하고 채우는 등의 여러 작업을 수행할 수 있습니다.</p><p>이 템플릿은 검색 키워드 - 자연어 차원을 사용합니다. </p> |
...
| **원래 참조 도메인** | 사용자가 사이트에 도달하기 위해 클릭한 첫 번째 참조 도메인을 확인할 수 있습니다. (설정되면 해당 방문자 ID의 전체 라이프타임 동안 동일한 값을 포함합니다.)<p>**이를 통해** 원래 사이트로 트래픽을 유도하는 서드파티 사이트를 파악할 수 있습니다.</p><p>**학습한 내용에 따라** 콘텐츠를 만들거나 조정하여 상위 최초 참조 도메인에서 온 방문자의 관심사에 더 잘 부합하도록 하는 등 여러 가지 작업을 수행할 수 있습니다. </p><p>이 템플릿은 최초 참조 도메인 차원을 사용합니다. </p> |
...
>title="모바일 앱의 각 화면과 관련된 이벤트, 세션 및 사용자 수를 봅니다."
>abstract="**가장 인기 있는 사이트의 화면을 이해하는 데**&#x200B;도움이 됩니다.<br/>**학습한 내용에 따라 가장 자주 보는 화면에서 콘텐츠 개선과 같은 다양한 작업을**&#x200B;할 수 있습니다."
...
>title="모바일 앱에서 사람들이 수행하는 작업을 봅니다."
>abstract="**이를 통해** 사람들이 앱을 사용하는 방법과 앱을 통해 얻는 값을 더 잘 이해할 수 있습니다.<br/>**학습한 내용에 따라** 가장 인기 있는 기능을 보완하거나 개선하는 기능을 개발하는 등 여러 가지 작업을 수행할 수 있습니다."
...
| 템플릿 이름 | 이 템플릿 <!-- What do you do with it? What can it help you learn? and What are the potential actions? -->을(를) 사용하는 이유 |
| --- | --- | 
| [!UICONTROL **모바일 앱 화면**] | 모바일 앱의 각 화면과 관련된 이벤트, 세션 및 사용자 수를 봅니다.<p>**가장 인기 있는 사이트의 화면을 이해하는 데**&#x200B;도움이 됩니다.</p><p>**학습한 내용에 따라 가장 자주 보는 화면에서 콘텐츠 개선과 같은 다양한 작업을**&#x200B;할 수 있습니다.</p><p>이 템플릿은 이벤트, 세션, 사람 및 변경 비율 지표를 사용합니다. 또한 페이지 제목 차원을 사용합니다.</p> |
...
| **모바일 앱 사용** | 앱에서 사용자, 시작 및 첫 번째 시작의 수와 평균 세션 길이를 확인합니다.<p>**앱이 얼마나 사용되고 있는지 이해하는 데**&#x200B;도움이 됩니다. </p><p>**학습한 내용을 바탕으로** 사용량에 맞게 확장할 수 있도록 앱 성능 개선 등 다양한 작업을 수행할 수 있습니다.</p><!-- This template uses the --> |
| **모바일 앱 여정** | 모바일 앱의 눈에 띄는 사용 패턴을 확인합니다. <p>**사람들이 앱을 사용하는 방법을 이해하는 데 도움이 됩니다**. </p><p>**학습한 내용을 바탕으로** 가장 일반적인 워크플로를 타기팅하기 위해 사람들이 한 화면에서 다른 화면으로 이동하는 방법을 개선하는 등 다양한 작업을 수행할 수 있습니다. </p><!-- This template uses the --> |
| **모바일 앱 지표** | 가장 일반적인 모바일 앱 지표를 확인합니다. <p>**모바일 앱의 기본 성능을 이해하는 데 도움이 됩니다**.</p><p>**학습한 내용에 따라 앱의 전반적인 상태 및 성능 평가와 같은 다양한 작업을**&#x200B;할 수 있습니다.</p><!-- This template uses the --> |
| **모바일 앱 메시지** | 인앱 메시지와 앱의 푸시 메시지의 실질 데이터를 확인합니다.<p>**이를 통해** 사람들이 인앱 메시지 기능을 어떻게 사용하고 있는지, 푸시 알림이 얼마나 효율적으로 앱으로 트래픽을 유도하고 있는지 파악할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 인앱 메시지 푸시 알림 경험을 개선하는 등 다양한 작업을 수행할 수 있습니다.</p><!-- This template uses the --> |
| **모바일 앱 성능** | 앱의 성능과 사용자 문제가 발생하고 있는 위치를 확인합니다. <p>**앱을 사용하는 사용자의 속도가 느려지거나 성능이 저하되는 경우**&#x200B;더 잘 이해할 수 있습니다. </p><p>**학습한 내용을 바탕으로** 기존 문제를 해결하거나 문제가 발생하기 이전에 앱 성능을 개선하는 등 다양한 작업을 수행할 수 있습니다.</p><!-- This template uses the --> |
| **모바일 앱 유지** | 충성도가 가장 높은 앱 사용자가 누구인지, 앱에서 어떤 작업을 하는지 확인합니다. <p>**이를 통해** 충성도가 가장 높은 사용자가 앱을 어떻게 사용하고 있는지 파악할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 충성도가 가장 높은 사용자가 사용하는 기능에 대한 마케팅 활동을 개선하는 등 다양한 작업을 수행할 수 있습니다.</p><!-- This template uses the --> |
...
>title="사이트의 이벤트, 세션 및 참여자 수를 분 단위로 분류하여 확인합니다. 예: 보고 일정이 1일인 보고서가 있을 경우, 당일 각 시간의 첫 번째 분은 동일한 차원 항목으로 그룹화됩니다."
>abstract="**이를 통해** 세부적인 수준에서 트렌드를 파악할 수 있습니다.<br/>**학습한 내용을 바탕으로** 분 단위까지 피크타임에 맞춰 리소스를 최적화하는 등 다양한 작업을 수행할 수 있습니다.<br/>이 템플릿은 분/시간 차원을 사용합니다."
...
>title="하루 중 시간별로 분류된 사이트의 이벤트, 세션 및 사람을 봅니다. 예: 1월 1일~1월 7일 범위의 보고서가 있을 경우, 각 날의 첫 번째 시간은 동일한 차원 항목으로 그룹화됩니다."
>abstract="**이렇게 하면 사이트를 가장 자주, 그리고 가장 자주 방문하는 시간을 이해하는 데 도움이 됩니다**.<br/>**학습한 내용을 바탕으로** 트래픽이 높은 시간대에 더 많은 컴퓨팅 리소스를 사이트에 할당하는 등 다양한 작업을 수행할 수 있습니다.<br/>이 템플릿은 시간(일 기준) 차원을 사용합니다."
...
>title="사이트의 이벤트, 세션 및 참여자 수를 오전 및 오후 단위로 분류하여 확인합니다. 예: 1월 1일~1월 7일 범위의 보고서가 있을 경우, 각 날의 오전 시간은 동일한 차원 항목으로 그룹화됩니다."
>abstract="***사이트를 가장 자주, 그리고 가장 자주 방문하지 않는 시간을 이해하는 데**&#x200B;도움이 됩니다.<br/>**학습한 내용을 바탕으로** 트래픽이 높은 시간대에 더 많은 컴퓨팅 리소스를 사이트에 할당하는 등 다양한 작업을 수행할 수 있습니다.<br/>이 템플릿은 오전/오후 차원을 사용합니다."
...
>title="요일별로 분류된 사이트의 이벤트, 세션 및 사람을 봅니다. 예: 1월에 걸쳐 처리할 보고서가 있을 경우, 각 요일은 동일한 차원 항목으로 그룹화됩니다."
...
>title="사이트의 이벤트, 세션 및 참여자 수를 월 단위로 분류하여 확인합니다. 예: 1년에 걸쳐 처리할 보고서가 있을 경우, 해당 월의 각 날은 동일한 차원 항목으로 그룹화됩니다."
...
>title="사이트의 이벤트, 세션 및 참여자 수를 일(연 기준) 단위로 분류하여 확인합니다. 예: 여러 해에 걸쳐 처리할 보고서가 있을 경우, 해당 년의 각 날은 동일한 차원 항목으로 그룹화됩니다."
...
>title="사이트의 이벤트, 세션 및 참여자 수를 주중 및 주말 단위로 분류하여 확인합니다. 예: 1월에 걸쳐 처리할 보고서가 있을 경우, 주중 및 주말은 동일한 차원 항목으로 그룹화됩니다."
...
>title="사이트의 이벤트, 세션 및 참여자 수를 주(연 기준) 단위로 분류하여 확인합니다. 예: 여러 해에 걸쳐 처리할 보고서가 있을 경우, 각 주는 동일한 차원 항목으로 그룹화됩니다."
...
>title="사이트의 이벤트, 세션 및 참여자 수를 월(연 기준) 단위로 분류하여 확인합니다. 예: 여러 해에 걸쳐 처리할 보고서가 있을 경우, 각 월은 동일한 차원 항목으로 그룹화됩니다."
...
>title="사이트의 이벤트, 세션 및 참여자 수를 분기(연 기준) 단위로 분류하여 확인합니다. 예: 여러 해에 걸쳐 처리할 보고서가 있을 경우, 각 분기는 동일한 차원 항목으로 그룹화됩니다."
...
| 템플릿 이름 | 이 템플릿 <!-- What do you do with it? What can it help you learn? and What are the potential actions? -->을(를) 사용하는 이유 |
| --- | --- | 
| [!UICONTROL **분/시간**] | 사이트의 이벤트, 세션 및 참여자 수를 분 단위로 분류하여 확인합니다. 예: 보고 일정이 1일인 보고서가 있을 경우, 당일 각 시간의 첫 번째 분은 동일한 차원 항목으로 그룹화됩니다.<p>**이를 통해** 세부적인 수준에서 트렌드를 파악할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 분 단위까지 피크타임에 맞춰 리소스를 최적화하는 등 다양한 작업을 수행할 수 있습니다.</p><p>이 템플릿은 시간(분) 차원을 사용합니다.</p> |
| **시간(일 기준)** | 사이트의 이벤트, 세션 및 참여자 수를 시간(일 기준) 단위로 분류하여 확인합니다. 예: 1월 1일~1월 7일 범위의 보고서가 있을 경우, 각 날의 첫 번째 시간은 동일한 차원 항목으로 그룹화됩니다.<p>**이렇게 하면 사이트를 가장 자주, 그리고 가장 자주 방문하는 시간을 이해하는 데 도움이 됩니다**.</p><p>**학습한 내용을 바탕으로** 트래픽이 높은 시간대에 더 많은 컴퓨팅 리소스를 사이트에 할당하는 등 다양한 작업을 수행할 수 있습니다.</p><p>이 템플릿은 시간 차원을 사용합니다.</p> |
| **오전/오후** | 사이트의 이벤트, 세션 및 참여자 수를 오전 및 오후 단위로 분류하여 확인합니다. 예: 1월 1일~1월 7일 범위의 보고서가 있을 경우, 각 날의 오전 시간은 동일한 차원 항목으로 그룹화됩니다.<p>**이렇게 하면 사이트를 가장 자주, 그리고 가장 자주 방문하는 시간을 이해하는 데 도움이 됩니다**.</p><p>**학습한 내용을 바탕으로** 트래픽이 높은 시간대에 더 많은 컴퓨팅 리소스를 사이트에 할당하는 등 다양한 작업을 수행할 수 있습니다.</p><p>이 템플릿은 오전/오후 차원을 사용합니다.</p> |
| **요일** | 사이트의 이벤트, 세션 및 참여자 수를 요일 단위로 분류하여 확인합니다. 예: 1월에 걸쳐 처리할 보고서가 있을 경우, 각 요일은 동일한 차원 항목으로 그룹화됩니다. <p>**이를 통해** 사이트에 가장 많이 방문하거나 가장 적게 방문하는 요일(주 기준)을 파악할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 트래픽이 높은 요일에 보다 적합한 직원을 콜센터에 배치하는 등 다양한 작업을 수행할 수 있습니다.</p><p>이 템플릿은 요일 차원을 사용합니다.</p> |
| **날짜(월 기준)** | 사이트의 이벤트, 세션 및 참여자 수를 월 단위로 분류하여 확인합니다. 예: 1년에 걸쳐 처리할 보고서가 있을 경우, 해당 월의 각 날은 동일한 차원 항목으로 그룹화됩니다. <p>**이를 통해** 사이트에 가장 많이 방문하거나 가장 적게 방문하는 해당 월의 요일을 파악할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 트래픽이 높은 요일에 보다 적합한 직원을 콜센터에 배치하는 등 다양한 작업을 수행할 수 있습니다.</p><p>이 템플릿은 일 차원을 사용합니다.</p> |
| **일(한 해 기준)** | 사이트의 이벤트, 세션 및 참여자 수를 일(연 기준) 단위로 분류하여 확인합니다. 예: 여러 해에 걸쳐 처리할 보고서가 있을 경우, 해당 년의 각 날은 동일한 차원 항목으로 그룹화됩니다. <p>**이를 통해** 사이트에 가장 많이 방문하거나 가장 적게 방문하는 해당 년의 요일을 파악할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 트래픽이 높은 요일에 보다 적합한 직원을 콜센터에 배치하는 등 다양한 작업을 수행할 수 있습니다.</p><p>이 템플릿은 일 (한 해 기준) 차원을 사용합니다.&lt;/> |
| **평일/주말** | 사이트의 이벤트, 세션 및 참여자 수를 주중 및 주말 단위로 분류하여 확인합니다. 예: 1월에 걸쳐 처리할 보고서가 있을 경우, 주중 및 주말은 동일한 차원 항목으로 그룹화됩니다. <p>**이를 통해** 주중과 주말의 사이트 트래픽 비교 차이를 파악할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 주말이 평일보다 더 바쁘다는 보고서가 있다면 주말에 더 많은 직원을 콜센터에 배치하는 등 여러 가지 조치를 취할 수 있습니다.</p><p>이 템플릿은 평일/주말 차원을 사용합니다.</p> |
| **주** | 사이트의 이벤트, 세션 및 참여자 수를 주(연 기준) 단위로 분류하여 확인합니다. 예: 여러 해에 걸쳐 처리할 보고서가 있을 경우, 각 주는 동일한 차원 항목으로 그룹화됩니다.<p>**이를 통해** 사이트에 가장 많이 방문하거나 가장 적게 방문하는 주(연 기준)를 파악할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 휴일 등 트래픽이 높은 주에 보다 적합한 직원을 콜센터에 배치하는 등 다양한 작업을 수행할 수 있습니다.</p><p>이 템플릿은 주(한 해 기준) 차원을 사용합니다.</p> |
| **월(연 기준)** | 사이트의 이벤트, 세션 및 참여자 수를 월(연 기준) 단위로 분류하여 확인합니다. 예: 여러 해에 걸쳐 처리할 보고서가 있을 경우, 각 월은 동일한 차원 항목으로 그룹화됩니다.<p>**이를 통해** 사이트에 가장 많이 방문하거나 가장 적게 방문하는 월을 파악할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 휴일 등 트래픽이 높은 월에 보다 적합한 직원을 콜센터에 배치하는 등 다양한 작업을 수행할 수 있습니다.</p><p>이 템플릿은 월(연 기준) 차원을 사용합니다.</p> |
| **사분기** | 사이트의 이벤트, 세션 및 참여자 수를 분기(연 기준) 단위로 분류하여 확인합니다. 예: 여러 해에 걸쳐 처리할 보고서가 있을 경우, 각 분기는 동일한 차원 항목으로 그룹화됩니다.<p>**이를 통해** 사이트에 가장 많이 방문하거나 가장 적게 방문하는 분기를 파악할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 기록에서 낮은 트래픽 분기의 매출을 높일 수 있도록 제품 출시의 타이밍을 결정하는 등 다양한 작업을 수행할 수 있습니다.</p><p>이 템플릿은 사분기 차원을 사용합니다.</p> |
...
>abstract="**이를 통해** 트래픽과 참여를 성공적으로 유도하는 채널을 파악할 수 있습니다. <br/>**학습한 내용에 따라** 가장 높은 투자 수익률을 달성하는 채널에 마케팅 노력을 집중하는 등 여러 가지 작업을 수행할 수 있습니다.<br/>이 템플릿은 사용자, 세션 및 이벤트 지표를 사용합니다."
...
| 템플릿 이름 | 이 템플릿 <!-- What do you do with it? What can it help you learn? and What are the potential actions? -->을(를) 사용하는 이유 |
| --- | --- | 
| [!UICONTROL **다중 채널 개요**] | 여러 채널의 트래픽 분포를 확인합니다. <p>**트래픽 및 참여를 더 성공적으로 유도하는 채널을 이해하는 데**&#x200B;도움이 될 수 있습니다. </p><p>**학습한 내용에 따라** 가장 높은 투자 수익률을 달성하는 채널에 마케팅 노력을 집중하는 등 여러 가지 작업을 수행할 수 있습니다.</p><p>이 템플릿은 사용자, 세션 및 이벤트 지표를 사용합니다.</p> |
| **웹+앱** | 웹 트래픽과 모바일 트래픽을 함께 확인합니다.<p>**이를 통해** 사이트로 유입되는 웹 및 모바일 트래픽의 분포를 파악할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 트래픽이 특정 수준에 도달하면 더 많은 리소스를 모바일 앱 경험에 전용하는 등 다양한 작업을 수행할 수 있습니다.</p><p>이 템플릿은 웹 세션, 모바일 앱 세션 및 웹+앱 크로스 채널 세션 지표를 사용합니다.</p> |
...
| **콜 센터 편향** | 웹 트래픽이 콜센터 트래픽에 미치는 영향을 확인합니다.<p>**이를 통해** 웹 사이트의 셀프서비스 콘텐츠가 콜센터 트래픽을 어떻게 성공적으로 억제하고 있는지 파악할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 콜센터 트래픽을 최소화하기 위해 셀프서비스 콘텐츠를 개선하거나 지원 전화를 줄임으로써 절감된 비용을 계산하여 셀프서비스 콘텐츠의 ROI를 측정하는 등 다양한 작업을 수행할 수 있습니다.</p><p>이 템플릿은 웹 세션, 모바일 앱 세션 및 웹+앱 크로스 채널 세션 지표를 사용합니다.</p> |
...
>title="고객이 전화한 이유 및 횟수를 포함한 콜센터 데이터를 봅니다."
>abstract="**이를 통해** 고객의 문제 상황과 콜센터 리소스가 소비되는 위치를 더 잘 파악할 수 있습니다.<br/>**학습한 내용에 따라** 더 높은 콜센터 트래픽을 유도하는 제품 문제 해결과 같은 다양한 작업을 수행하여 궁극적으로 제품 수익성을 향상시킬 수 있습니다."
...
>abstract="**이를 통해** 매장 위치와 온라인에서 가장 많이 팔리는 제품을 더 잘 이해할 수 있습니다.<br/>**학습한 내용을 바탕으로** 성과가 가장 높은 제품과 채널에 더 많은 마케팅 리소스를 사이트에 할당하는 등 다양한 작업을 수행할 수 있습니다."
...
>title="Adobe Journey Optimizer을 사용하여 디자인하고 보내는 이메일이 새 멤버십, 충성도 멤버 및 크로스셀 기회를 생성하는 방법을 봅니다."
>abstract="**이를 통해** Adobe Journey Optimizer을 사용하여 디자인하고 보내는 전자 메일의 효과를 더 잘 이해할 수 있습니다.<br/>**학습한 내용에 따라** 특정 이메일 캠페인에 대한 이메일 전략 조정과 같은 다양한 작업을 수행할 수 있습니다."
...
| 템플릿 이름 | 이 템플릿 <!-- What do you do with it? What can it help you learn? and What are the potential actions? -->을(를) 사용하는 이유 |
| --- | --- | 
| [!UICONTROL **콜 센터 대시보드**] | 고객이 전화한 이유 및 횟수를 포함한 콜센터 데이터를 봅니다. <p>**이를 통해** 고객의 문제 상황과 콜센터 리소스가 소비되는 위치를 더 잘 파악할 수 있습니다.</p><p>**학습한 내용에 따라** 더 높은 콜센터 트래픽을 유도하는 제품 문제 해결과 같은 다양한 작업을 수행하여 궁극적으로 제품 수익성을 향상시킬 수 있습니다.</p> |
| **판매 지점** | 매출, 주문, 판매 수량을 비롯해 POS(판매 시점) 거래 데이터를 조회합니다. 이 템플릿에는 최우수 스토어, 최우수 제품, 최우수 제품 카테고리와 온라인 대 오프라인 판매에 대한 정보를 표시하는 시각화도 포함됩니다. <p>**이를 통해** 어떤 제품이 스토어와 온라인에서 가장 많이 판매되는지 파악할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 성과가 가장 높은 제품과 채널에 더 많은 마케팅 리소스를 사이트에 할당하는 등 다양한 작업을 수행할 수 있습니다.</p><p>이 템플릿은 사용자, 매출 및 주문 지표를 사용합니다.</p> |
| **Journey Optimizer 전자 메일 분석** | Adobe Journey Optimizer을 사용하여 디자인하고 보내는 이메일이 새 멤버십, 충성도 멤버 및 크로스셀 기회를 생성하는 방법을 봅니다. <p>**이를 통해** Adobe Journey Optimizer을 사용하여 디자인하고 보내는 전자 메일의 효과를 더 잘 이해할 수 있습니다.</p><p>**학습한 내용에 따라** 특정 이메일 캠페인에 대한 이메일 전략 조정과 같은 다양한 작업을 수행할 수 있습니다.</p> |
| **설문 조사** | 설문 조사를 수행하기 위해 사용자 참여도를 조회합니다. 시작 횟수와 완료 횟수, 가장 많이 나온 질문 및 답변, 첫 참가자 수 대 반복 참가자 수를 조회합니다.<p>**이를 통해** 설문 조사의 참여도와 참여 성공률을 파악할 수 있습니다.</p><p>**학습한 내용을 바탕으로** 향후 설문 조사를 조정하여 참여도를 높이는 등 다양한 작업을 수행할 수 있습니다.</p><p>이 템플릿은 사용자, 이벤트, 설문 시작, 설문 조사 완료 및 설문 조사 완료율 지표를 사용합니다.</p> |
...
| **Journey Optimizer 랜딩 페이지** | 사용자 행동, 참여 패턴, 전환율 및 다른 주요 지표를 확인합니다.<p>**랜딩 페이지의 효과를 더 잘 이해하는 데**&#x200B;도움이 됩니다. </p><p>**학습한 내용을 바탕으로** 랜딩 페이지 성능을 최적화하는 등 다양한 작업을 수행할 수 있습니다.</p> |
| **Journey Optimizer 개요 보고서** | 환경 내 모든 캠페인과 여정의 트래픽 및 참여 지표에 대한 상세한 요약을 확인합니다.<p>**이를 통해**&#x200B;캠페인과 여정의 높은 수준의 효과를 더 잘 이해할 수 있습니다. </p><p>**학습한 내용을 바탕으로** 타깃 대상자의 참여 수준에 따라 캠페인 및 여정을 조정하는 등 다양한 작업을 수행할 수 있습니다.</p> |
