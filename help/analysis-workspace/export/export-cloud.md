...
1. 다음에서 [!UICONTROL **새 전체 테이블 내보내기**] 대화 상자에서 다음 정보를 지정합니다.
   ...
   | 데이터 표 | 내보내는 자유 형식 테이블을 표시합니다. 왼쪽 레일에서 테이블로 구성 요소를 끌어 데이터 테이블을 수정할 수 있습니다. 캔버스에 구성 요소를 추가하면 테이블이 동적으로 업데이트됩니다.  <p>프로젝트의 전체 테이블에 적용된 모든 세그먼트는 테이블의 각 개별 열 맨 위에 나타납니다.</p> |
