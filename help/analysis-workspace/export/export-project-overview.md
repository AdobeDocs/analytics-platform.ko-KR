...
Analysis Workspace에서 Customer Journey Analytics 데이터를 내보낼 수 있습니다. 대량의 내역 데이터를 저장하거나 서드파티 도구에서 원시 데이터를 분석하는 등 여러 가지 이유로 Customer Journey Analytics 데이터를 내보낼 수 있습니다.

다음 섹션에서는 지원되는 파일 유형, 내보내기에 사용할 수 있는 다양한 방법 및 각 방법의 장점에 대해 설명합니다.

## 지원되는 파일 유형

Customer Journey Analytics 데이터를 PDF 또는 CSV 파일로 내보낼 수 있습니다.
...
| [클라우드 애플리케이션으로 보내기](/help/analysis-workspace/export/export-cloud.md) | 다음 작업을 수행하려면 이 메서드를 사용하십시오. <ul><li>Google Cloud Platform, Microsoft Azure, Amazon S3, Snowflake 또는 Adobe Experience Platform과 같은 공유 위치로 내보냅니다.</li><li>애드혹 또는 일정에 포함될 수 있습니다.</li><li>대량의 내역 Customer Journey Analytics 데이터를 저장합니다.</br>이러한 유형의 데이터는 비즈니스 인텔리전스를 얻고 궁극적으로 더 나은 비즈니스 의사 결정을 이끌어내기 위해 장기적인 추세를 감지하는 데 사용할 수 있습니다.</li><li>수천 또는 수백만 개의 행이 포함된 전체 테이블을 내보냅니다.<!-- What other things? Wiki talks about things that aren't even possible in Data Warehouse. What are they? --> </li> |
