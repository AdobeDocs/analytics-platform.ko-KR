...
| 요소 | 정의 | 최적화 |
| --- | --- | --- |
| 쿼리 개수 | 프로젝트에 표시되는 데이터를 검색하도록 Adobe에 적용되는 총 쿼리(요청) 개수입니다. 쿼리에는 테이블에 대한 등급 요청, 예외 항목 탐지, 스파크라인, 왼쪽 레일에 표시된 구성 요소 등이 포함됩니다. 축소된 패널 및 시각화를 제외합니다. 지침은 100개입니다. | 특정 목적 또는 이해 관계자 그룹에 도움이 되는 여러 프로젝트로 데이터를 분할하여 프로젝트를 최대한 간소화합니다. 태그를 사용하여 프로젝트를 테마로 분류하고, 이해 관계자가 필요한 내용을 더 손쉽게 찾을 수 있도록 [직접 연결](/help/analysis-workspace/curate-share/shareable-links.md)을 사용하여 내부 목차를 만듭니다. |
...
| 사용 가능한 구성 요소 | 프로젝트의 모든 보고서 세트에 걸쳐 프로젝트의 왼쪽 레일에서 검색된 총 구성 요소 수입니다. 지침은 2,000개입니다. | 구성 요소 세트가 더욱 맞춤화되는 선별된 가상 보고서 세트를 만드는 방법은 제품 관리자에게 문의하십시오. |
