...
| 요소 | 정의 | 영향을 주는 요소 | 최적화 |
| --- | --- | --- | --- |
| 연산 속도 | 컴퓨터에서 처리 테스트를 수행하는 속도입니다. 지침은 750ms 미만입니다. | 하드웨어는 물론, 동시 실행 프로그램도 이 요소에 영향을 줍니다. | 컴퓨터의 작업 관리자(PC) 또는 활동 모니터(Mac)을 열어 프로그램을 닫을 수 있는지 확인합니다. 그런 다음, 사용하지 않는 브라우저 탭 또는 기타 프로그램을 닫습니다. <br><br>이러한 작업이 도움이 되지 않는 경우 IT 팀에 하드웨어 세부 사항을 문의합니다. |
...
전체 CJA 성능을 제어하는 추가 요소에 대해서는 [CJA 성능 최적화](/help/admin/cja-performance.md).