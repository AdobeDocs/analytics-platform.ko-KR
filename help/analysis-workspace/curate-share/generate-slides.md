...
이렇게 자동 생성된 데이터 스토리는 Workspace 프로젝트의 결과를 표시하는 데 필요한 시간, 노력 및 전문성을 줄입니다. 분석가는 데이터 탐색에 더 집중할 수 있는 동시에 Customer Journey Analytics이 경영진 설명을 작성하고 형식을 지정하며 비즈니스 영향을 이해 당사자에게 전달할 수 있습니다.

## 슬라이드 프레젠테이션의 데이터 스토리 이해

Analysis Workspace은 생성 AI를 사용하여 슬라이드 프레젠테이션 형식으로 Workspace 데이터를 기반으로 하는 이야기를 만듭니다.

다음 섹션에서는 데이터 스토리가 제공하는 추가 값, 내러티브를 형성하는 데 도움이 되는 프로젝트의 필수 요소 및 각 프레젠테이션 출력에 포함된 주요 요소에 대해 설명합니다.
...
* 주어진 컨텍스트에서 데이터의 양호 또는 불량에 대한 지침을 제공합니다.
