...
>[!NOTE]
> 제품 프로필은 사용자가 볼 수 있는 구성 요소를 관리하는 기본 메커니즘으로, 이러한 지표는 [Adobe Experience Cloud Admin Console](https://docs.adobe.com/content/help/ko-KR/core-services/interface/manage-users-and-products/admin-getting-started.html)을 통해 관리됩니다. 큐레이션은 보조 필터입니다.
...
선별된 프로젝트에서 수신자에게 왼쪽 레일에 있는 **[!UICONTROL 모두 표시]** 구성 요소에 대한 옵션이 표시됩니다. [!UICONTROL 모두 표시]를 선택하면 다음 항목에 따라 다른 구성 요소 세트가 표시됩니다.

* 사용자의 권한 수준(관리자 또는 비관리자)
* 프로젝트 역할(소유자/편집자 아님)
* 적용된 조정 유형(프로젝트 수준에서)

| 조정 유형 | 관리자는 | 관리자가 아닌 프로젝트 소유자(또는 역할 편집)는 | 관리자가 아닌 중복 역할은 |
| --- | --- | --- | --- |
