...
* 표시된 테이블을 복사하려면 테이블 헤더를 마우스 오른쪽 단추로 클릭하고 **표시된 데이터를 클립보드로 복사**.
...
* 테이블 헤더에서 마우스 오른쪽 버튼을 클릭합니다 **[!UICONTROL 표시된 데이터를 CSV로 다운로드]**. 이렇게 하면 테이블에 표시된 데이터가 CSV로 다운로드됩니다.
* 테이블에서 선택한 경우 **[!UICONTROL 선택 항목을 CSV로 다운로드]**. 표시된 전체 테이블이 아닌 이 옵션과 함께 선택한 항목만 다운로드됩니다.
* 시각화의 헤더에서 마우스 오른쪽 버튼을 클릭합니다 **[!UICONTROL 데이터를 CSV로 다운로드]**. 시각화에 대한 데이터 소스 테이블을 CSV로 다운로드합니다. 참고:맵 시각화는 이 옵션을 지원하지 않습니다.
...
| 어떤 기능 **[!UICONTROL 시각화 복사]** 뭐? | **[!UICONTROL 시각화 복사]** 은 내보내기 옵션이 아닙니다. 시각화 또는 패널을 작업 공간의 한 위치에서 다른 위치로 복사할 수 있습니다. 예를 들어 동일한 프로젝트의 패널 간 또는 하나의 프로젝트에서 다른 프로젝트로 이어지는 경우입니다. [비디오 보기](https://www.youtube.com/watch?v=lvmAdKNfWQw) |
