...
| [!UICONTROL 지표] | 지표는 보기 수, 클릭스루 횟수, 다시 로드 횟수, 평균 체류 시간, 판매량, 주문 수, 매출액 등과 같은 개인 활동에 대한 수량 정보입니다. |
...
| [!UICONTROL 필터] | 필터를 사용하여 특성 또는 웹 사이트 상호 작용에 따라 개인 하위 집합을 식별할 수 있습니다. 예를 들어 다음을 빌드할 수 있습니다. [!UICONTROL 방문자] 필터: 속성 기반: 브라우저 유형, 디바이스, 방문 횟수, 국가, 성별 또는 상호 작용 기반: 캠페인, 키워드 검색, 검색 엔진 또는 종료 및 항목 기반: Facebook의 개인, 정의된 랜딩 페이지, 참조 도메인 또는 사용자 지정 변수 기반: 양식 필드, 정의된 카테고리, 고객 ID. |
