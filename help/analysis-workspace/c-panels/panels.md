...
패널 필터링을 시작하려면 왼쪽 레일에서 패널 드롭 영역으로 필터를 끌어다 놓습니다.
...
드롭다운 필터를 통해 제어된 방식으로 데이터와 상호 작용할 수 있습니다. 예를 들어 모바일 디바이스 유형에 대한 드롭다운 필터를 추가하여 태블릿, 휴대폰 또는 데스크탑 별로 패널을 필터링할 수 있습니다.
...
* 차원 항목을 사용하는 드롭다운 필터의 경우 왼쪽 레일에서 원하는 차원 옆에 있는 오른쪽 화살표 아이콘을 클릭합니다. 이 작업을 수행하면 사용 가능한 모든 차원 항목이 표시됩니다. `[Shift + Click]` 또는 `[Ctrl + Click]`을 사용하여 이 목록에서 여러 차원 항목을 선택한 다음 **`[Shift]`**&#x200B;를 누른 상태에서 패널 놓기 영역에 놓습니다.
* 지표, 필터 또는 날짜 범위 등 다른 구성 요소를 사용하는 드롭다운 필터의 경우 `[Shift + Click]` 또는 `[Ctrl + Click]`을 사용하여 여러 구성 요소를 선택할 수 있습니다. **`[Shift]`**&#x200B;를 누른 상태에서 선택 항목을 패널 놓기 영역에 놓습니다. 이 컨텍스트에서는 모든 구성 요소 유형이 필터로 처리됩니다.
* 단일 드롭다운 필터는 단일 구성 요소 유형만 포함할 수 있습니다. 선택 항목에 여러 구성 요소 유형이 포함된 경우 구성 요소 유형별로 별도의 드롭다운 필터가 만들어집니다. 예를 들어 선택 항목에 지표와 차원 항목이 모두 포함되어 있으면 두 개의 별도 드롭다운 필터가 만들어집니다. 드롭다운 필터 하나에는 차원 항목이 포함되고 다른 하나에는 지표가 포함됩니다.

드롭다운 목록에서 옵션 중 하나를 선택하여 패널의 데이터를 변경합니다. **[!UICONTROL 필터 없음]**&#x200B;을 선택하여 패널 데이터를 필터링하지 않도록 선택할 수도 있습니다.
