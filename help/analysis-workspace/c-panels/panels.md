...
패널 필터링을 시작하려면 왼쪽 레일에서 패널 드롭 영역으로 필터를 드래그 앤 드롭합니다. 이 프로세스를 반복하여 패널에 필터를 더 추가합니다. 패널 상단에 필터가 나란히 나타납니다.

![왼쪽 레일에 사용 가능한 지표와 패널 놓기 영역으로 끌어온 모바일 고객 지표가 표시됩니다.](assets/segment-filter.png)

### 임시 필터

필터가 아닌 구성 요소를 드롭 영역으로 직접 끌어와 임시 필터를 만들 수도 있으므로 필터 빌더로 이동하는 데 드는 시간과 노력을 절약할 수 있습니다. 이러한 방식으로 만든 필터는 자동으로 이벤트 수준 필터로 정의됩니다. 이 정의는 필터 옆에 있는 정보 아이콘(i)을 클릭한 다음 연필 모양의 편집 아이콘을 클릭하고 필터 빌더에서 편집하여 수정할 수 있습니다.
...
정적 드롭다운 필터를 사용하면 제어된 방식으로 데이터와 상호 작용할 수 있습니다. 예를 들어, 태블릿, 휴대폰 또는 데스크탑 별로 패널을 필터링할 수 있도록 모바일 디바이스 유형에 대한 드롭다운 필터를 추가할 수 있습니다.

정적 드롭다운 필터를 사용하여 많은 프로젝트를 하나로 통합할 수도 있습니다. 예를 들어 다른 국가 필터가 적용된 동일한 프로젝트의 여러 버전이 있는 경우 모든 버전을 단일 프로젝트로 통합하고 국가 드롭다운 필터를 추가할 수 있습니다.

![시장 채널 “직접” 필터가 강조 표시된 정적 드롭다운 필터입니다. ](assets/dropdown-filter-intro.png)

#### 정적 드롭다운 필터 만들기

* 차원 항목을 사용하는 드롭다운 필터의 경우 왼쪽 레일에서 단일 차원을 선택하여 패널 드롭 영역에 드롭합니다 **잠시 중단`[Shift]`**. 이렇게 하면 해당 차원과 연결된 모든 차원 항목으로 드롭다운 필터가 만들어집니다.

  또는, 드롭다운 필터에 차원과 연결된 특정 차원 항목만 포함하려면 왼쪽 레일에서 원하는 차원 옆에 있는 오른쪽 화살표 아이콘을 클릭합니다. 이 작업을 수행하면 사용 가능한 모든 차원 항목이 표시됩니다. 다음을 사용하여 이 목록에서 여러 차원 항목 선택 `[Shift + Click]` 또는 `[Ctrl + Click]`을 클릭한 다음 패널 드롭 영역에 드롭합니다 **잠시 중단** `[Shift]`.

* 단일 구성 요소 유형을 사용하는 드롭다운 필터의 경우(예: 차원만, 필터만 또는 지표만) 를 사용하여 왼쪽 레일에서 동일한 유형의 여러 항목을 선택합니다 `[Shift + Click]` 또는 `[Ctrl + Click]`을 클릭한 다음 패널 드롭 영역에 드롭합니다 **잠시 중단`[Shift]`**.

  선택한 구성 요소로 단일 드롭다운 필터가 만들어집니다.

* 구성 요소 유형 혼합을 사용하는 드롭다운 필터(예: 2개 지표 및 3개 필터)의 경우 다음을 사용하여 여러 구성 요소를 선택합니다 `[Shift + Click]` 또는 `[Ctrl + Click]`. **`[Shift]`**&#x200B;를 누른 상태에서 선택 항목을 패널 놓기 영역에 놓습니다. 이 컨텍스트에서 모든 구성 요소 유형은 별도의 드롭다운 필터로 처리됩니다. 예를 들어, 선택 항목에 지표와 차원 항목을 모두 포함하는 경우 두 개의 별도 드롭다운 필터가 만들어집니다. 한 드롭다운 필터에는 차원 항목이 포함되고 다른 드롭다운 필터에는 지표가 포함됩니다.

  ![정적 드롭다운 필터를 놓는 데 사용할 수 있는 모바일 고객 세그먼트 필드가 있는 패널 창입니다. ](assets/create-dropdown.png)

드롭다운 필터를 마우스 오른쪽 버튼으로 클릭하면 다음 옵션이 제공됩니다.

* **[!UICONTROL 삭제 드롭다운]**: 패널에서 드롭다운 필터를 제거합니다.
...
* **[!UICONTROL 선택 필요]**: 패널에 필터가 설정되어 있어야 합니다.
...
* 드롭다운 필터에서 두 개 이상의 필터를 선택하여 패널에 여러 필터를 적용합니다. 패널은 선택한 필터를 포함하도록 필터링됩니다.
...
* **`[Shift]`**&#x200B;를 누른 상태에서 단일 차원을 패널 놓기 영역으로 끌어다 놓습니다.
