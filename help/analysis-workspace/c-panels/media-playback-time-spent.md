...
| 시간 형식 | 다음 중 하나에서 소요된 재생 시간을 볼 수 있습니다. `Hours:Minutes:Seconds` (기본값) 또는 `Minutes` (정수로 표시되며 .5로 반올림됨). |
...
| 지표 | 설명 |
|---|---|
| 재생 소요 시간 | 합계 `hours:minutes:seconds` (또는 `minutes`일시 중지, 버퍼 및 시작 시간을 포함하여 선택한 세부 기간 동안 본 콘텐츠의 ). |
