...
## 무작위가 아닌 차원 해석 {#non-randomized}

CJA를 사용하면 분석가가 차원을 &quot;실험&quot;으로 선택할 수 있습니다. 하지만 실험으로 선택한 차원이 방문자가 무작위가 아닌 분석을 어떻게 해석합니까?

예를 들어 방문자에게 표시되는 광고를 고려해 보십시오. 방문자를 &quot;광고 A&quot; 대신 &quot;ad B&quot;로 표시하도록 결정하는 경우 일부 지표(예: 평균 매출액)의 변경 사항을 측정하는 데 관심이 있을 수 있습니다. 광고 A 대신 광고 B를 표시하면 마케팅 의사 결정에 도달하는 것이 매우 중요합니다. 광고 B를 표시하는 대체 전략으로 광고 A를 보여주는 현상을 대체하는 경우, 이러한 원인별 효과는 전체 인구의 평균 수익으로 측정될 수 있습니다.

A/B 테스트는 이러한 개입의 효과를 객관적으로 측정하기 위한 업계 최고의 표준입니다. A/B 테스트가 인과적 추정을 일으키는 중요한 이유는 가능한 변형 중 하나를 받도록 방문자가 임의로 분류하기 때문입니다.

이제 방문자의 미국 상태와 같이 무작위화로 달성되지 않은 차원을 고려합니다. 우리 방문객들이 주로 뉴욕과 캘리포니아에서 왔다고 가정해 보겠습니다. 겨울철 의류 브랜드 판매의 평균 매출은 지역 날씨 차이로 두 주에서 다를 수 있습니다. 그런 상황에서 날씨는 겨울옷판매의 진짜 원인일 수 있고, 방문자의 지리적 상태가 다르다는 것이 아니다.

Customer Journey Analytics의 실험 패널을 사용하면 방문자 상태별로 데이터를 평균 수입 차이로 분석할 수 있습니다. 이런 경우 출력에는 인과해석도 없다. 그러나, 그러한 분석은 여전히 흥미로울 수 있다. 방문자 상태별 평균 수익 차이에 대한 예상(불확실성 측정 포함)을 제공합니다. 이를 &quot;통계적 가설 테스트&quot;라고도 합니다. 이 분석의 결과는 흥미로울 수 있지만, 반드시 실행 가능한 것은 아닐 수도 있고, 경우에 따라 차원의 가능한 값 중 하나에 방문자를 무작위할 수도 없기 때문입니다.

다음 그림은 이러한 상황을 대비합니다.

![무작위 실험](assets/randomize.png)

결과 Y에 미치는 개입 X의 영향을 측정하려는 경우 두 가지 요소의 실제 원인은 연결 요소 C일 수 있습니다. X에서 방문자를 무작위화하여 데이터가 달성되지 않으면 영향을 측정하기가 더 어려워지고 분석에서 C에 대한 명확한 설명을 제공합니다. 무작위 분류는 X의 의존성을 C에 중단하고 다른 변수에 대한 걱정 없이 Y에 대한 X의 효과를 측정할 수 있도록 해줍니다.
