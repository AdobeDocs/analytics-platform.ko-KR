...
CJA를 통해 분석가는 모든 차원을 &quot;실험&quot;으로 선택할 수 있습니다. 하지만 실험으로 선택한 차원이 무작위화된 방문자가 아닌 분석을 어떻게 해석합니까?

예를 들어 방문자에게 표시되는 광고를 고려해 보십시오. 방문자에게 &quot;광고 A&quot; 대신 &quot;광고 B&quot;를 표시하기로 결정한 경우 일부 지표(예: 평균 매출)의 변화를 측정하는 데 관심이 있을 수 있습니다. 광고 A 대신 광고 B를 보여주는 인과적 효과는 마케팅 의사 결정에 도달하는데 있어서 핵심적으로 중요하다. 광고 A의 상태 quo를 광고 B의 대체 전략으로 대체한 경우, 이러한 인과 효과는 전체 모집단에 대한 평균 수익으로 측정될 수 있다.

A/B 테스트는 이러한 개입의 효과를 객관적으로 측정하기 위한 업계 내 Gold 표준입니다. A/B 테스트가 인과 관계 예측을 생성하는 중요한 이유는 가능한 변형 중 하나를 수신하기 위한 방문자의 무작위 지정 때문입니다.

이제 무작위 지정으로 달성되지 않은 차원(예: 방문자의 미국 상태)을 고려해 보십시오. 우리의 방문자들이 주로 뉴욕과 캘리포니아, 두 주에서 온다고 가정해 보자. 한 겨울 의류 브랜드의 평균 매출액은 지역 날씨의 차이로 인해 두 주에서 차이가 날 수 있다. 이런 상황에서 날씨는 겨울철 의류 판매의 진정한 원인일 수 있고, 방문객들의 지리적 상태가 다르다는 사실이 아닐 수 있다.

Customer Journey Analytics의 실험 패널을 사용하면 데이터를 방문자 상태별 평균 수익 차이로 분석할 수 있습니다. 이와 같은 상황에서 산출물은 인과적 해석을 하지 않는다. 그러나 이러한 분석은 여전히 관심의 대상일 수 있습니다. 방문자 주별 평균 수익 차이에 대한 추정치(불확실성 측정치와 함께)를 제공합니다. 이를 &quot;통계적 가설 테스트&quot;라고도 합니다. 이 분석의 출력은 흥미롭지만, 경우에 따라 방문자를 차원의 가능한 값 중 하나로 무작위화할 수 없거나 없으므로 반드시 실행 가능한 것은 아닙니다.

다음 그림은 이러한 상황을 대비합니다.

![무작위 실험](assets/randomize.png)

중재 X가 결과 Y에 미치는 영향을 측정하고자 할 때, 양자의 진짜 원인은 교란 요인 C일 가능성이 있다. X에 대한 방문자를 무작위화함으로써 데이터가 달성되지 않는 경우, 그 영향은 측정하기 더 어렵고, 분석에서 C를 명시적으로 설명할 것이다. 무작위화는 C에 대한 X의 의존성을 무너뜨리기 때문에, 다른 변수에 대한 걱정 없이 Y에 대한 X의 영향을 측정할 수 있게 해준다.
