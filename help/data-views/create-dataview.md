...
| [!UICONTROL 스키마 데이터 유형] | 구성 요소가 문자열, 정수 등인지 여부를 나타냅니다.  플랫폼에서 지원되는 스키마 필드 유형을 사용할 수 있지만, 모든 필드 유형이 CJA에서 지원되지는 않습니다. 문자열 또는 정수 이외의 스키마 필드 유형을 사용하여 CJA에 데이터 세트를 추가할 수 있지만 CJA에서는 해당 데이터를 표시할 수 없습니다. 또한 현재 조회 데이터 세트에는 문자열만 사용할 수 있습니다. |
