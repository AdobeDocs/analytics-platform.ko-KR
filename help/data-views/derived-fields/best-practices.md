...
* 속성 및 지속성: [다음 또는 이전](./derived-fields.md#next-or-previous) 또는 다른 순차적 논리를 사용하여 파생 필드에서 차원을 시뮬레이트하지 마십시오. 차원에 대한 데이터 보기 [속성](/help/data-views/component-settings/attribution.md) 및 [지속성](/help/data-views/component-settings/persistence.md) 설정을 사용하십시오.
...
* 출력이 레이블인 경우:
   * 구성 요소 유형을 **[!UICONTROL Dimension]**(으)로 설정하고 그에 따라 [속성](/help/data-views/component-settings/attribution.md) 및 [지속성](/help/data-views/component-settings/persistence.md) 설정을 구성하십시오.
...
* 속성 또는 지속성이 있는 중복성: 일부 사용 사례(예: 세션의 마지막 터치 채널 속성)는 데이터 보기 속성 설정에서 더 잘 다룹니다.

### 추천 항목

* 표준 속성과 유사한 패턴의 경우 [다음 또는 이전](/help/data-views/component-settings/attribution.md)으로 시뮬레이션하는 대신 데이터 보기에서 차원 [속성](/help/data-views/component-settings/persistence.md) 및 [지속성](./derived-fields.md#next-or-previous) 설정을 사용하십시오.
* 속성만으로는 달성할 수 없는 고급 다중 단계 경로 또는 funnel 레이블에 대해 [다음 또는 이전](./derived-fields.md#next-or-previous)을(를) 예약합니다(예: 채널 시퀀스 연결).
...
* 의미 체계 불일치: 차원의 레이블에서 실제로 구성된 것과 다른 [속성](/help/data-views/component-settings/attribution.md) 또는 [만료](/help/data-views/component-settings/persistence.md) 동작(예: 첫 번째 터치 또는 원본 소스)을 제안합니다.
* 이러한 불일치는 분석가가 보고서를 잘못 해석하거나 이름별로 유사하지만 다른 속성 모델을 사용하는 구성 요소를 비교할 위험을 증가시킵니다.
