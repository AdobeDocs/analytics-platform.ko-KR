...
* 페이지 URL에서 [소문자](./derived-fields.md#lowercase), [트리밍](./derived-fields.md#trim) 또는 [Case When](./derived-fields.md#case-when) 확인과 같은 간단한 작업은 낮은 카디널리티 필드의 동일한 논리보다 종종 비용이 더 듭니다.
...
* [Regex 바꾸기](./derived-fields.md#regex-replace) 또는 regex 기반 조건에서는 광범위한 패턴을 사용합니다. [포함](./derived-fields.md#case-when) 또는 **[!UICONTROL 다음으로 시작]**&#x200B;을 사용하는 간단한 **[!UICONTROL 경우]**&#x200B;이(가) 더 나은 대안입니다.
...
>[!NOTE]
>
>가능한 경우 복잡한 파생 필드 내에서 카디널리티가 높은 필드에 대한 의존도를 줄입니다(예: 정규화된 키 또는 그룹화된 분류 사용). 쿼리 비용과 [연산자 또는 함수 제한에 도달할 가능성](./derived-fields.md#limitations)을 제한하십시오.

CustoCustomer 여정 분석 [문서](./derived-fields.md#limitations) 함수 유형별 제한을 포함하여 파생 필드당 최대 함수 및 연산자입니다.**
