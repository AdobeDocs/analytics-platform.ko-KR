...
description: Query Service, Power BI, Tableau 또는 기타 BI 및 SQL 도구를 사용하여 Customer Journey Analytics BI 확장을 사용하여 데이터 보기에 액세스하는 방법에 대해 알아봅니다.
...
# Customer Journey Analytics BI 확장
...
* BI 도구 자체 내에서 Customer Journey Analytics 데이터 보기의 동등한 표시를 다시 만들 필요가 없습니다. <br/>다음을 참조하십시오 [데이터 보기](data-views.md) 를 참조하십시오.
...
* 관련 제품 프로필, 사용자 그룹 및/또는 개별 사용자에 대한 기능을 구성합니다. 액세스 요구 사항은 다음과 같습니다.
   * Adobe Experience Platform 쿼리 서비스
   * Customer Journey Analytics의 작업 공간 프로젝트
   * 사용하려는 CJA 데이터 보기
   * 데이터 보기 도구에서 BI 확장 기능에 액세스

* 만료되지 않는 자격 증명의 만료일을 사용하여 BI 도구를 [!DNL Customer Journey Analytics BI extension]. 다음 [자격 증명 안내서](https://experienceleague.adobe.com/en/docs/experience-platform/query/ui/credentials) 만료 자격 증명 또는 만료되지 않는 자격 증명 설정에 대한 자세한 내용을 제공합니다.
...
1. Adobe Experience Platform에서 PostgresSQL 자격 증명을 조회하고 복사합니다.
...
1. 로그인하여 쿼리 실행을 시작하려면 터미널에 명령 문자열을 붙여넣습니다.

+++

자세한 내용은 [Query Editor UI 안내서](https://experienceleague.adobe.com/en/docs/experience-platform/query/ui/user-guide)를 참조하십시오.


### BI 도구

현재, [!DNL Customer Journey Analytics BI extension] 은 Power BI 및 Tableau에 대해서만 지원 및 테스트됩니다. PSQL 인터페이스를 사용하는 다른 BI 도구도 작동할 수 있지만 아직 공식적으로 지원되지 않습니다.

+++ Power BI

1. Adobe Experience Platform에서 PostgresSQL 자격 증명의 세부 정보를 조회합니다.
...
1. Power BI에서 다음 작업을 수행합니다.
   ...
   1. **[!UICONTROL ** PostgressSQL 데이터베이스&#x200B;**]** 대화 상자에서 다음 작업을 수행합니다.
      ...
      1. 을 묻는 메시지가 표시되면 **[!UICONTROL 데이터 연결]** 모드, 선택 **[!UICONTROL DirectQuery]**.

      1. **[!UICONTROL 사용자 이름]** 및 **[!UICONTROL 암호]**&#x200B;를 묻는 메시지가 표시됩니다. Experience Platform 쿼리 [!UICONTROL 자격 증명]의 동등한 매개변수 사용


   1. 로그인에 성공하면 Customer Journey Analytics 데이터 보기 테이블이 Power BI의 **[!UICONTROL **&#x200B;네비게이터&#x200B;**]**.
...
1. Tableau에서 다음 작업을 수행합니다.
   ...
   1. [!UICONTROL PostgresSQL] 대화 상자에서 다음 작업을 수행합니다.
      ...
      1. **[!UICONTROL **&#x200B;데이터베이스&#x200B;**]** 텍스트 필드에 Experience Platform 쿼리 [!UICONTROL 자격 증명]의 **[!UICONTROL **&#x200B;데이터베이스&#x200B;**]** 매개변수를 붙여넣습니다.
   ...
   1. Customer Journey Analytics 데이터 보기는 **[!UICONTROL **&#x200B;표&#x200B;**]** 목록을 표시합니다.
...
기본적으로 데이터 보기에는 친숙한 이름에서 생성된 테이블 보안 이름이 있습니다. (예: 이름이 인 데이터 보기) [!UICONTROL 내 웹 데이터 보기] 보기 이름 있음 `my_web_data_view`.
...
Experience Platform에서 사용하는 데이터 세트에 생성된 개인정보 보호 레이블 및 정책은 Customer Journey Analytics 데이터 보기 워크플로에 표시될 수 있습니다. 따라서 를 사용하여 쿼리하는 데이터 [!DNL Customer Journey Analytics BI extension] 정의된 개인정보 보호 레이블 및 정책을 준수하지 않을 때 적절한 경고 또는 오류를 표시합니다.
...
* `SELECT COUNT(DISTINCT dimension)` 또는 `SELECT APPROX_COUNT_DISTINCT(dimension)`는 차원의 대략적인 고유 값을 계산합니다. 자세한 내용은에서 참조하십시오. [고유 값 계산](#counting-distinct-values).
...
의 지표 표현식에 추가 계산을 적용할 수 있습니다. `SELECT` 계산된 지표에 수학을 정의하는 대신. 다음 표에는 지원되는 표현식 유형이 나와 있습니다.
...
* 최대값만 제공되는 경우 (`timestamp < X` 또는 `timestamp <= X`), 범위는 X -30일 ~ X입니다.
* 아무 것도 제공하지 않는 경우 범위는 지금부터 마이너스 30일까지입니다.

타임스탬프 범위는 RankedRequest에서 날짜 범위 전역 필터로 변환됩니다.
타임스탬프 필드는 날짜/시간 함수에서 이벤트 타임스탬프를 구문 분석하거나 자르는 데 사용할 수도 있습니다.

#### 날짜 범위

`daterange` 특수 열은 `timestamp`와 유사하게 작동하나, 필터링은 하루 종일로 제한됩니다. `daterange`는 옵션이며 범위 기본값은 `timestamp`와 동일합니다.
다음 `daterange` 필드는 날짜/시간 함수에서 이벤트 날짜를 구문 분석하거나 자르는 데 사용할 수도 있습니다.

다음 `daterangeName` 특수 열은 다음과 같은 명명된 날짜 범위를 사용하여 쿼리를 필터링하는 데 사용할 수 있습니다. `Last Quarter`.
...
과 함께 `filterId`, 다음을 사용할 수 있습니다 `filterName` 필터 이름을 ID 대신 사용합니다.

### Where 절

다음 `WHERE` 조항은 다음 세 단계로 처리됩니다.

1. 다음에서 날짜 범위를 찾습니다. `timestamp`, `daterange`, 또는 `daterangeName` 특수 필드.

1. 외부에서 정의된 항목 찾기 `filterId`s 또는 `filterName`필터링에 포함할 s입니다.
...
| 함수 | 예 | 세부 사항 |
|---|---|---|
| [캐스트](https://spark.apache.org/docs/latest/api/sql/index.html#cast) | ``CAST(`timestamp` AS STRING)`` 또는 <br/> `` `timestamp`::string `` | 유형 캐스팅은 현재 지원되지 않지만 오류는 발생하지 않습니다. `CAST` 함수는 무시됩니다. |
...
| [대상 타임스탬프](https://spark.apache.org/docs/latest/api/sql/index.html#to_timestamp) | `` WHERE `timestamp` >= TO_TIMESTAMP('01/01/2022', 'MM/dd/yyyy') AND `timestamp` < TO_TIMESTAMP('01/02/2022', 'MM/dd/yyyy') `` | 시간 문자열을 `WHERE` 절 내에서 사용할 타임스탬프로 구문 분석하고, 옵션으로 해당 시간 문자열에 대한 형식을 제공합니다. |
...
| [종료 날짜](https://spark.apache.org/docs/latest/api/sql/index.html#to_date) | `` WHERE `timestamp` >= TO_DATE('01/01/2022', 'MM/dd/yyyy') AND `timestamp` < TO_DATE('01/02/2022', 'MM/dd/yyyy') `` | 날짜 문자열을 `WHERE` 절 내에서 사용할 타임스탬프로 구문 분석하고, 옵션으로 해당 날짜 문자열에 대한 형식을 제공합니다. |
