...
속성 설정을 적용하기 전에 다음 사항에 유의하십시오.

* 데이터 뷰 사용자 인터페이스에서 기본 속성을 지정합니다. **참고**:나중에 작업 공간 프로젝트에서 이러한 설정을 무시할 수 있습니다. 그러나 이 기능은 현재 사용할 수 없습니다.

* 고객 경로 분석의 속성 설정은 비파괴 및 소급 적용됩니다. 즉, Customer Journey Analytics에서는 데이터 세트에 되돌이킬 수 없는 손상을 줄 수 없습니다. 실수로 어떤 것을 삭제하더라도 언제든지 다시 [!UICONTROL Experience Platform] 돌아가 데이터 세트를 다시 가져올 수 있습니다. 그러나 데이터 세트를 다시 가져올 경우 추가 비용이 발생할 수 있습니다.

* 기존 eVar(전환 변수)처럼 &quot;행동&quot; 차원이 있으려면 기본적으로 &quot;마지막 터치 방문&quot; 속성을 사용하여 구성해야 합니다.

* 기존 prop(트래픽 변수)처럼 &quot;작업&quot; 차원을 사용하려면 기본적으로 &quot;동일한 터치&quot; 속성을 사용하여 구성해야 합니다.

* 기본 지표처럼 &quot;작동&quot; 지표를 만들려면 아무 것도 변경하지 마십시오.

* 지표에 대한 속성 설정은 차원에 대한 속성 설정을 무시합니다.

## 구성 요소 및 속성 설정 지정

데이터 보기 설정 [및 구성 요소를](/help/data-views/create-dataview.md) 설정 및 저장한 후 속성 설정을 지정할 수 있습니다(원하는 경우). 차원 및 지표에 대한 속성/만료/조회 설정을 지정할 수 있습니다. 예를 들어 차원에 대한 속성을 유지하려면 사용자 지정 만료 시간을 설정해야 할 수 있습니다. 예를 들어 &#39;추적 코드&#39; 차원(캠페인 변수)을 &#39;마지막 터치&#39; 속성으로 한 주 동안 유지하려면 사용자 지정 만료 기간을 1주로 추가합니다.

>[!IMPORTANT]
>할당/만료를 설정하지 않도록 선택할 수 있습니다. 이 경우 차원은 prop(&quot;동일한 터치&quot; 속성 모델)처럼 동작합니다. 속성 설정이 설정되지 않은 지표는 이 지표가 적용되는 차원에 대한 설정을 상속합니다.

![](assets/edit-component.png)

1. 차원 및 지표에 대한 구성 요소 및 속성 설정을 지정합니다. 개별 설정에 대한 자세한 내용은 아래를 참조하십시오.

1. 을 **[!UICONTROL Save]** 클릭하여 데이터 뷰를 저장합니다.


### 구성 요소 설정

지표나 차원의 이름을 보다 친숙한 이름으로 변경할 수 있습니다. 기본 이름은 변경되지 않고 표시 이름만 변경됩니다.

### 속성 모델

모델은 그룹의 이벤트에 대한 전환 분포를 설명합니다. 예를 들어 첫 번째 터치 또는 마지막 터치가 있습니다. 변수가 이벤트 전에 여러 값을 받을 경우 고객 경로 분석에서 성공 이벤트에 대한 크레딧을 할당하는 방법을 결정합니다.
...
| 설정 | 정의 |
|---|---|
| 세션 | 이전에 &#39;방문&#39; 레벨이라고 합니다. 페이지 보기 또는 세션을 벗어나는 전환 이벤트는 차원이나 지표와 연결되지 않습니다. |
| 개인(보고 창) | 이전에 &#39;방문자&#39; 수준이라고도 합니다. 이 사람과 연결되지 않은 전환 이벤트는 차원이나 지표와 연결되지 않습니다. |
| 사용자 지정 시간 | 사용자 지정 분, 시간, 일, 월 또는 분기를 지정합니다. 지정된 기간을 벗어나는 전환 이벤트는 차원이나 지표와 연결되지 않습니다. |
...
* **세션:** 전환이 발생한 세션의 시작 부분으로 돌아갑니다. 방문 조회 창은 세션 범위를 벗어나지 않으므로 좁습니다. 세션 조회 창은 데이터 보기에서 수정된 방문 정의를 따릅니다.
* **개인(보고 창):** 현재 날짜 범위의 월 1일까지 모든 세션을 검색합니다. 많은 세션에 걸쳐 볼 수 있으므로 뒤쪽의 창은 넓습니다. 예를 들어 보고서 날짜 범위가 9월 15일 - 9월 30일이면 조회 날짜 범위에 9월 1일 - 9월 30일이 포함됩니다.
