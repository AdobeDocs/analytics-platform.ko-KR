...
Adobe Analytics과 Customer Journey Analytics은 근본적으로 다르므로 1:1 열 매핑이 불가능합니다. 이러한 차이는 모든 Adobe Analytics 및 Customer Journey Analytics 구현이 서로 다르기 때문에 더욱 악화됩니다.

이 참조는 데이터 엔지니어가 Adobe Analytics 중심의 데이터 피드 워크플로우를 열별로 Customer Journey Analytics 데이터 피드를 기반으로 하는 워크플로우로 조정할 수 있도록 설계되었습니다.
...
브라우저를 나타내는 숫자 ID. `browser.tsv` 조회 테이블을 참조합니다.
...
Adobe Advertising 통합 변수입니다. 이동통신사를 지정합니다. `carrier.tsv` 동적 조회에 대한 키 값입니다.
...
`c_color` 열의 값을 기반으로 하는 색상 심도 ID입니다. `color_depth.tsv` 조회 테이블을 참조합니다.

+++

+++**`connection_type`**

연결 유형 차원을 나타내는 숫자 ID입니다. `connection_type.tsv` 조회 테이블을 참조합니다.
...
방문자의 최초 레퍼러 유형을 나타내는 숫자 ID. `referrer_type.tsv` 조회 테이블을 참조합니다.
...
`j_jscript`를 기반으로 하는 JavaScript 버전의 조회 ID. `javascript_version` 조회 테이블을 참조합니다.
...
방문자의 언어를 나타내는 숫자 ID. `languages.tsv` 조회 테이블을 참조합니다.
...
+++**`mobile_id`**
사용자가 모바일 디바이스를 사용하는 경우 디바이스의 숫자 ID입니다. `mobile_attributes.tsv` 동적 조회에 대한 키 값입니다.
...
방문자의 운영 체제를 나타내는 숫자 ID. `user_agent` 열을 기반으로 합니다. `operating_system.tsv` 표준 조회 및 `operating_system_type.tsv` 동적 조회에 대한 키 값입니다.
...
이미지 요청(표준 히트, 다운로드 링크, 사용자 정의 링크, 종료 링크)에서 전송된 히트 유형입니다. 페이지 이벤트 조회를 참조하십시오.
...
모니터의 해상도를 나타내는 숫자 ID. 모니터 해상도 차원에 사용됩니다. `resolution.tsv` 조회 테이블을 사용합니다.

+++

+++**`search_engine`**

방문자에게 사이트를 참조하도록 하는 검색 엔진을 나타내는 숫자 ID. 검색 엔진 차원에 사용됩니다. `search_engines.tsv` 조회 테이블을 참조합니다.
...
마지막 터치 채널 차원을 식별하는 숫자 ID입니다. 이 ID에 대한 조회는 마케팅 채널 관리자에서 찾을 수 있습니다.
...
첫 번째 터치 채널 차원을 식별하는 숫자 ID입니다. 이 ID에 대한 조회는 마케팅 채널 관리자에서 찾을 수 있습니다.
...
방문의 첫 번째 레퍼러 유형을 나타내는 숫자 ID. `referrer_type.tsv` 조회 테이블을 참조합니다.

+++

+++**`visit_search_engine`**

방문의 첫 번째 검색 엔진에 대한 숫자 ID. `search_engines.tsv` 조회 테이블을 참조합니다.
