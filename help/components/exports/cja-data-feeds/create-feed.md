...
* 데이터 피드를 전송해야 하는 빈도(늦게 도착하는 히트를 포함하도록 선택한 경우 전환 확인 기간 포함)
...
1. 오른쪽 상단의 9제곱 아이콘을 선택한 다음 [!UICONTROL **Customer Journey Analytics**]&#x200B;을 선택합니다.
...
1. [!UICONTROL **세부 정보**] 섹션에서 다음 필드를 작성합니다.
   ...
   | 필드 | 함수 |
   |---------|----------|
   | [!UICONTROL **이름**] | 데이터 피드의 이름. 이름은 선택한 보고서 세트 내에서 고유해야 하며 최대 255자까지 사용할 수 있습니다. <!--[Learn more](/help/export/analytics-data-feed/df-faq.md#must-feed-names-be-unique)--> |
...
1. 데이터 피드 구성에 열을 추가합니다. 왼쪽의 **[!UICONTROL 사용 가능]** 섹션에서 포함할 열을 선택한 다음 **[!UICONTROL 포함]**&#x200B;을 선택합니다. Adobe Analytics의 모든 데이터 열을 사용할 수 있습니다. **[!UICONTROL Shift]**&#x200B;을 누르거나 **[!UICONTROL Command]**(macOS) 또는 **[!UICONTROL Ctrl]**(Windows)을 눌러 여러 열을 선택할 수 있습니다. 데이터 피드에 모든 열을 포함하려면 **[!UICONTROL 모두 추가]**&#x200B;를 클릭하십시오.
   ...
   모든 데이터 피드에는 다음 구성 요소가 포함되어야 합니다.

   | 구성 요소 이름 | 참고 | 데이터 피드 | 기타 보고 |
   |---|---|---|---|
   | 타임스탬프 | 이벤트 기간의 타임스탬프. 밀리초 세부 기간. UTC로 표시됩니다. | 필수 | 사용할 수 없음 |
   | 행 ID | 고유 행 식별자 | 필수 | 사용할 수 없음 |
   | 세션 ID | 각 세션에 대한 고유 식별자 | 필수 | 사용할 수 없음 |
   | 개인 ID | 데이터 보기 및 연결에 대한 개인 식별자 | 필수 | 선택 사항 표준 |
   | 계정 ID (B2B) | 계정 컨테이너를 사용할 때의 계정 ID | 필수(B2B만 해당) | 선택 사항 표준(B2B만 해당) |
   ...
   | 구성 요소 이름 | 참고 | 데이터 피드 |
   |---|---|---|
   | 계정 | [B2B edition] 연결에 지정된 계정 ID 기반 | 사용할 수 없음. 계정 ID 고유 개수 사용. |
   | 구매 그룹 | 연결에서 구매 그룹 ID를 기반으로 하는 [B2B edition] 구매 그룹 | 사용할 수 없음. 구매 그룹 ID의 고유 개수를 사용합니다. |
   | 이벤트 | 연결의 모든 이벤트 데이터 세트의 행 수 | 사용할 수 없음. 행 ID의 고유 개수 사용. |
   | 글로벌 계정 | 연결의 글로벌 계정 ID를 기반으로 한 [B2B edition] | 사용할 수 없음. 글로벌 계정 ID의 고유 개수를 사용합니다. |
   | 기회 | 연결의 영업 기회 ID에 따른 [B2B edition] 영업 기회 | 사용할 수 없음. 영업 기회 ID 고유 개수 사용. |
   ...
   | 개인 ID | 차원 | 데이터 보기 및 연결에 대한 개인 식별자 | **필수** |
   | 개인 ID 네임스페이스 | 차원 | 개인 ID가 구성하는 ID 유형(예: 이메일 또는 쿠키 ID) | 사용 가능 |
   | 글로벌 계정 ID | [B2B edition] Dimension | 글로벌 계정 컨테이너를 사용할 때의 글로벌 계정 ID | 사용 가능 |
   | 계정 ID | [B2B edition] Dimension | 계정 컨테이너를 사용할 때의 계정 ID | **필수**(B2B만 해당) |
   | 기회 ID | [B2B edition] Dimension | 영업 기회 컨테이너를 사용할 때의 영업 기회 ID | 사용 가능 |
   | 구매 그룹 ID | [B2B edition] Dimension | 구매 그룹 컨테이너를 사용할 때 구매 그룹 ID | 사용 가능 |
...
1. [!UICONTROL **게재**] 섹션에서 다음 정보를 지정하십시오.
   ...
   | [!UICONTROL **빈도**] | 데이터 피드를 전송해야 하는 빈도를 선택합니다. 빈도 창에 속하는 타임스탬프가 있는 이벤트는 데이터 피드 배달에 포함됩니다. [!UICONTROL **전환 확인 날짜 범위**] 및 [!UICONTROL **처리 지연**] 필드는 선택한 게재 빈도의 데이터에 포함된 이벤트에도 영향을 줄 수 있습니다.<p>1시간 분량의 데이터 또는 1일 분량의 데이터를 포함하려면 선택합니다.</p><ul><li>**일별**: 피드에 데이터 보기의 시간대에 자정부터 자정까지 하루 분량의 데이터가 포함됩니다. 피드 채우기 또는 라이브 피드에 이 옵션을 사용합니다.</li><li>**시간별**: 피드에는 1시간 동안의 데이터가 포함됩니다. 라이브 피드에 이 옵션을 사용합니다.</li></ul> |
   ...
   | [!UICONTROL **처리 지연**] | 데이터 피드 파일을 처리하기 전에 지정된 시간을 대기할지 여부를 선택합니다. 처리 지연 중에 들어오는 모든 늦게 도착하는 히트는 데이터 피드에 포함됩니다.<p>지연은 오프라인 디바이스를 온라인 상태로 전환하여 데이터를 전송할 수 있는 기회를 모바일 구현에 제공하는 데 유용합니다. 또한 이전에 처리된 파일을 관리할 때 조직의 서버측 프로세스를 수용하는 데 사용할 수도 있습니다. 대부분의 경우에는 지연이 필요하지 않습니다. 사용자 지정 시간(9,999분 또는 약 1주)을 선택한 경우 피드를 최대 8시간(480분) 또는 더 오래 지연시킬 수 있습니다.<p>지연이 설정되지 않은 경우 빈도 창(마지막 날 또는 시간)에 속하는 이벤트만 피드에 포함됩니다.</p> <p>방문 횟수를 포함하려면 이 마감 이후에 시작해야 합니다. 마감 전에 시작되고 처리 지연 내에 끝나는 방문 횟수는 포함되지 않습니다.</p> <p>세션, 지속성 및 세그먼트에 필요합니다.</p><p>차원에 사용되지 않습니다. 차원은 차원의 할당 및 만료에 따라 차원별로 제어됩니다. Dimension 전환 확인은 처리 지연을 초과할 수 없습니다.</p> |
