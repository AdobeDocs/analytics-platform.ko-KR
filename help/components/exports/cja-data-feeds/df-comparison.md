...
* 파생 필드
  ...
  데이터 피드에 파생 필드 구성 요소를 포함합니다.

* 결합

  여러 장치의 이벤트를 한 사람에게 연결하여 장치 간 ID 확인을 활성화합니다.

* 구조화된 데이터 보기

  전달된 파일에서 데이터 피드를 작성할 때 구조화된 데이터를 사용합니다. Adobe Analytics 데이터 피드는 문자열을 사용합니다.

* Analysis Workspace과 일치하는 차원 및 지표가 있는 구성 요소 레일

  데이터 보기에서 사용할 수 있는 차원 및 지표를 사용합니다. Adobe Analytics에서는 사전 정의된 필드 및 열 목록이 사용됩니다.

* 데이터 보기에 적용된 모든 세그먼트는 데이터 피드에서 자동으로 상속됩니다

* 세그먼트는 데이터 보기에 이미 적용된 모든 세그먼트 외에도 데이터 피드에 직접 적용할 수 있습니다

* 피드가 데이터 보기 시간대 <!-- how did it work in AA? -->과(와) 일치합니다.

* 쪽모이 세공

  복잡한 중첩 데이터와 구조화된 데이터를 기본적으로 지원하는 최신 Parquet 파일을 출력합니다. 제품 목록은 구조화된 배열/중첩된 개체로 표시됩니다.

* 하이브 스타일 경로

* 데이터 보기의 구성 요소에 적용된 변경 내용은 데이터 피드에 적용됩니다
...
| **게재 대상** | Amazon S3, Azure RBAC, Azure SAS, Google Cloud Platform | Amazon S3, Azure RBAC, Azure SAS, Google Cloud Platform **SFTP**&#x200B;도 지원합니다. |
