...
* **구조화된 데이터 모델**: post_product_list와 같은 플랫 문자열이 아닌 구조화된 데이터를 사용하여 피드가 만들어지고 전달됩니다. XDM 스키마 및 데이터 보기의 기존 구조를 반영합니다.

* **Parquet 출력**: 파일은 기본적으로 복잡한 중첩 데이터와 구조화된 데이터를 지원하는 Parquet 형식으로 전달됩니다. 즉, 업계 표준 사례를 사용하여 데이터베이스에서 데이터에 보다 쉽게 액세스할 수 있습니다.
...
* **구성 요소 업데이트가 소급하여 적용됩니다**: 데이터 보기의 구성 요소에 대한 변경 내용은 과거 다시 채우기에 반영됩니다.

* **조회**: 분류는 Adobe 분석 데이터 피드에 포함되지 않습니다. Customer Journey Analytics에서 모든 조회는 데이터에 직접 포함됩니다.

* **Analysis Workspace 사용자에게 익숙한 인터페이스**: 구성 요소는 데이터 보기에서 직접 제공되며 Analysis Workspace에서도 사용할 수 있습니다. 변수 이름의 정적 목록이 아니라 Analysis Workspace과 동일한 구성 요소 레일을 사용하여 차원 및 지표를 선택할 수 있습니다.

* **지속성 모델 추가 사용 가능**: Customer Journey Analytics 데이터 피드에서 사용할 수 있는 5가지 지속성 모델이 있습니다.
...
| **Analysis Workspace과의 유사성**<br/>&#x200B;데이터 피드 빌더가 Analysis Workspace과 동일한 구성 요소와 용어를 사용하는지 여부. | 데이터 피드의 왼쪽 레일은 Workspace 왼쪽 레일과 유사하며 데이터 피드에서 사용할 수 있는 구성 요소는 Workspace에서도 사용할 수 있습니다. | Analysis Workspace에 표시되는 항목과 일치하지 않는 변수 이름의 정적 목록입니다. |
| **지속성 모델 가용성**<br/>&#x200B;데이터 피드의 차원에 사용할 수 있는 지속성 모델입니다. | 데이터 피드에는 원본, 가장 최근, 모두, 처음 알려짐, 마지막 알려짐 5개의 지속성 모델을 사용할 수 있습니다 | 데이터 피드에는 첫 번째 터치와 마지막 터치라는 두 가지 지속성 모델을 사용할 수 있습니다 |
