...
>title="파일 이름 및 경로"
>abstract="이 위치로 전송되는 자동 내보내기에 사용할 동적 사용자 정의 파일 이름을 지정합니다. 파일 이름 앞에 동적 사용자 정의 파일 경로를 추가할 수도 있습니다. <br/>파일 이름과 경로에 변수를 사용하여 동적으로 만듭니다. <br/>예를 들어 `${yyyy}/${MM}/${dd}/my-report-${instance_id}-${idx}`를 지정하면 2026년 1월 15일에 자동으로 이 대상에 전송되는 내보내기 파일 경로와 이름은 `[prefix_folder_name]/2026/01/15/my-report-[UUID]-1.csv`가 됩니다. <br/>사용 가능한 변수 목록을 보려면 아래 링크를 클릭하십시오."

<!-- markdownlint-enable MD034 -->

Customer Journey Analytics 보고서를 클라우드 대상([Analysis Workspace](/help/analysis-workspace/export/export-cloud.md) 또는 [Report Builder](/help/report-builder/report-builder-export.md))으로 내보내려면 먼저 데이터를 전송할 위치를 추가하고 구성해야 합니다. 이 프로세스는 다음으로 구성됩니다.

1. [클라우드 내보내기 계정 구성](/help/components/exports/cloud-export-accounts.md)에 설명된 대로 계정 추가 및 구성(Amazon S3, Google Cloud Platform 등)

1. 이 문서에 설명된 대로 해당 계정(계정 내 폴더 등) 내 위치 추가 및 구성
...
1. [!UICONTROL **위치**] 탭을 선택한 다음 [!UICONTROL **위치 추가**]&#x200B;를 선택합니다.
   ...
   ![선택한 위치 추가를 보여 주는 GCP 계정 및 줄임표 드롭다운 메뉴](assets/add-location-existing-account.png)
...
1. AEP 데이터 랜딩 구역에서 데이터에 액세스하는 가장 쉬운 방법은 Microsoft Azure Storage Explorer를 사용하는 것입니다. 저장소 탐색기는 [AEP 데이터 랜딩 영역 계정 구성](/help/components/exports/cloud-export-accounts.md#aep-data-landing-zone)의 지침에 사용되는 것과 동일한 도구입니다.
...
1. [!UICONTROL **위치 추가**] 대화 상자의 [!UICONTROL **위치 속성**] 섹션에서 다음 정보를 지정하여 Amazon S3 역할 ARN 위치를 구성하십시오.
   ...
   | 필드 | 함수 |
   |---------|----------|
   | [!UICONTROL **버킷**] | Customer Journey Analytics 데이터를 전송할 Amazon S3 계정 내의 버킷입니다. <p>이 버킷에 파일을 업로드하려면 Adobe에서 제공한 사용자 ARN에 `S3:PutObject` 권한이 있는지 확인합니다. </p><p>버킷 이름은 특정 이름 지정 규칙을 충족해야 합니다. 예를 들면 3~63자 사이여야 하며 소문자, 숫자, 점(.), 하이픈(-)만 사용할 수 있고 문자나 숫자로 시작하고 끝나야 합니다. [전체 명명 규칙 목록은 AWS 설명서에서 사용할 수 있습니다](https://docs.aws.amazon.com/AmazonS3/latest/userguide/bucketnamingrules.html). </p> |
