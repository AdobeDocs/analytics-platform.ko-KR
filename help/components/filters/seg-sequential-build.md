...
순차적 필터를 만드는 기본 사항은 [필터 빌더](filter-builder.md)를 사용하여 일반 필터를 만드는 것과 다를 바 없습니다. [정의 빌더](filter-builder.md#definition-builder)를 사용하여 필터 정의를 구성할 수 있습니다. 해당 구성에서는 구성 요소, 컨테이너, 연산자 및 논리를 사용합니다. 기본 정의나 [정의 빌더](filter-builder.md#definition-builder)에서 사용하는 컨테이너에서 **[!UICONTROL Then]** 연산자를 선택하면 일반 필터가 자동으로 순차적 필터가 됩니다.
...
| **[!UICONTROL 내부]** | [!UICONTROL Within] 연산자는 두 체크포인트 사이의 시간에 대한 최대 한도 지정에 사용됩니다. 예를 들어, 페이지 A를 방문한 다음 하루 이내에 페이지 B를 다시 방문하기 위해 [!UICONTROL Within] 연산자를 컨테이너에 설정한 경우, 그 하루는 페이지 A를 나갈 때 시작됩니다. 필터에 포함되기 위해 페이지 B를 열기 전에 최대 하루 시간을 갖습니다. 사람이 필터에 포함되려면 페이지 B를 보려면 페이지 A에서 나간 후 최대 1440분(하루) 이내에 페이지 B를 열어야 페이지 B를 볼 수 있습니다. |
...
순차적 필터는 전체적으로 주어진 패턴과 일치하는 데이터를 식별합니다.  다음은 하나의 제품 범주 페이지를 방문한 사람을 찾는 기본 시퀀스 필터의 예입니다(여성) | 신발), 체크아웃 페이지(체크아웃) | 감사합니다.) 필터가 ![사용자 그룹](/help/assets/icons/UserGroup.svg) **[!UICONTROL 모든 사용자 포함]**(으)로 설정되어 있습니다.
