...
| 결과... | 시퀀스 |
|--- | --- |
| 일치 | A 다음에 B<br>A 다음 (다른 방문에서) BA 다음 D 다음 B |
| 일치하지 않음 | B 다음에 A |

## 시퀀스 이전에만 및 시퀀스 이후에만 {#only_before_after}

옵션 **[!UICONTROL 시퀀스 이전에만]** 및 **[!UICONTROL 시퀀스 이후에만]** 지정된 시퀀스의 전 또는 후에 필터를 데이터 하위 집합으로 필터링합니다.
...
이 예제는 두 가지 기능이 있습니다. **[!UICONTROL 방문]** 최상위 수준 내 컨테이너 **[!UICONTROL 방문자]** 컨테이너 및 를 사용하여 필터 시퀀스 지정 [!UICONTROL 그런 다음] 연산자.
...
[!UICONTROL 방문자] 컨테이너 내에 여러 개의 [!UICONTROL 히트] 컨테이너를 추가하면 같은 유형의 컨테이너 사이에 적절한 연산자를 적용하고, 페이지 및 방문 횟수와 같은 규칙과 차원을 이용하여 페이지 보기를 정의하며, [!UICONTROL 히트] 컨테이너 내에 시퀀스 차원을 제공할 수 있습니다. 히트 수준에서 논리를 적용하면 의 동일한 히트 수준에서 일치 항목을 조합하여 제한하고 결합할 수 있습니다. [!UICONTROL 방문자] 다양한 필터 유형을 빌드하는 컨테이너입니다.
...
두 위치에 체크포인트를 배치함으로써 [!UICONTROL 방문] 및 [!UICONTROL 히트] 특정 방문 및 특정 히트 내에서 요구 사항을 충족하도록 필터를 제한할 수 있습니다.
...
* **페이지 제외**. 필터 규칙을 사용하여 특정 페이지(예: *`Home Page`*)를 보고서에서 제외하고, 페이지가 &quot;홈 페이지&quot;와 같은 히트 규칙을 만든 다음 제외시킵니다. 이 규칙은 자동으로 홈 페이지를 제외한 모든 값을 포함합니다.
* **참조 도메인 제외**. Google.com의 참조 도메인만 포함하고 나머지는 모두 제외하는 규칙을 사용합니다.
* **비구매자 파악**. 주문이 0보다 큰 경우를 파악하고 해당 [!UICONTROL 방문자]를 제외합니다.

[!UICONTROL 제외] 연산자를 사용하여 방문자가 특정 방문 또는 히트를 수행하지 않는 시퀀스를 식별할 수 있습니다. [!UICONTROL 체크포인트 제외] 논리 그룹 내에 포함될 수도 있습니다(아래 참조).
...
단순, 혼합 수준 또는 중첩된 순차적 필터에 대해 만드는 것처럼 필터를 만든 다음 [!UICONTROL 제외] 컨테이너 요소에 대한 연산자입니다. 아래 예제는 다음 세 가지가 있는 집계 필터입니다 [!UICONTROL 히트] 컨테이너를 캔버스, [!UICONTROL 그런 다음] 컨테이너 논리를 결합하는 데 할당된 연산자를 만든 다음, 중간 페이지 보기 컨테이너를 제외하여 시퀀스에서 페이지 A에서 페이지 C로 이동한 방문자만 포함합니다.
...
2개를 끌어 간단한 시퀀스 필터를 만듭니다. [!UICONTROL 히트] 컨테이너를 캔버스에 연결하고 [!UICONTROL 그런 다음] 연산자. 그런 다음 [!UICONTROL EXCLUDE] 연산자를 시퀀스의 두 번째 [!UICONTROL 히트] 컨테이너에 할당합니다.
...
>[!NOTE]
>
>A [!UICONTROL 논리 그룹] 는 순차적 필터에서만 정의할 수 있으며 이는 다음을 의미합니다. [!UICONTROL 그런 다음] 연산자가 표현식 내에 사용됩니다.

| 컨테이너 계층 | 그림 | 정의 |
|---|---|---|
| 표준 컨테이너 계층 | ![](assets/nesting_container.png) | 다음 범위 내 [!UICONTROL 방문자] 컨테이너, [!UICONTROL 방문] 및 [!UICONTROL 히트] 컨테이너는 히트, 방문 횟수 및 방문자를 기준으로 필터를 추출하도록 시퀀스에 중첩됩니다. |
...
**논리 그룹이 순차적 필터 작성을 간소화합니까?**
예, 가능합니다. 이 방문자 필터를 식별하려고 한다고 가정해 보겠습니다. **페이지 A를 본 다음 B, C 및 D의 각 페이지를 본 방문자**
...
[!UICONTROL 논리 그룹]을 사용하면 해당 그룹 내에 시퀀스 외부에 있는 조건을 충족할 수 있습니다. 필터를 빌드할 수 있습니다. [!UICONTROL 방문] 또는 [!UICONTROL 히트] 컨테이너는 일반 계층에 관계없이 수행됩니다.
...
[!UICONTROL 논리 그룹]을 사용하면 해당 그룹 내에 시퀀스 외부에 있는 조건을 충족할 수 있습니다. 이 순서가 없는 첫 번째 일치 필터에서는 [!UICONTROL 논리 그룹] 규칙은 먼저 페이지 B 또는 페이지 C의 페이지 보기가 된 다음 필요한 페이지 A 보기가 되도록 식별됩니다.
...
다음을 사용하여 필터 빌드 [!UICONTROL 논리 그룹] 여러 페이지 보기를 합계하여 다른 페이지가 확실히 누락된 상태에서 반드시 히트해야 하는 페이지를 정의하는 경우. ****
...
## 다음 시간 이내 및 이후 시간 필터 구축 {#time_within_after}
...
| AFTER/WITHIN | After 및 Within 연산자를 모두 사용할 때는 두 연산자가 모두 순차적이 아니라 나란히 시작하고 끝난다는 사실을 이해하는 것이 중요합니다.   예를 들어 컨테이너를 로 설정한 필터를 만드는 경우:<br>`After = 1 Week(s) and Within = 2 Week(s)`<br>그런 다음 필터에서 방문자를 식별하는 조건은 1주와 2주 사이에만 충족됩니다. 두 조건 모두 첫 번째 페이지 히트에서만 적용됩니다. |
...
**세그먼트 만들기**: 이 필터는 [!UICONTROL 방문자] 2개가 있는 컨테이너 [!UICONTROL 히트] 컨테이너. 그런 다음 [!UICONTROL THEN] 연산자를 설정하고 [!UICONTROL AFTER] 연산자 드롭다운을 열어서 주 수를 설정할 수 있습니다.
...
**필터 만들기**: 이 필터는 [!UICONTROL 방문자] 컨테이너, 2로 드래그 [!UICONTROL 히트] 컨테이너. [!UICONTROL THEN] 연산자를 설정하고 [!UICONTROL AFTER] 연산자 드롭다운을 열고 간격: 히트, 페이지 보기, 방문, 분, 시간, 일, 주, 월, 분기 또는 년을 설정합니다.
...
사용 [!UICONTROL 다음 범위 내] 및 [!UICONTROL 다음 이후] 필터 양끝에 최대 및 최소 끝점을 제공합니다.

**예**: 방문자는 페이지 A를 방문하고 2주 후, 단 1개월 내에 페이지 B를 방문했습니다.

![](assets/time_between_using_both_operators.png)

**세그먼트 만들기**: 2개의 순서를 지정하여 필터 만들기 [!UICONTROL 히트] 다음 내의 컨테이너: [!UICONTROL 방문자] 컨테이너. 그런 다음 [!UICONTROL After] 및 [!UICONTROL Within] 연산자를 설정합니다.

![](assets/within_after_together.png)

**일치**

2019년 6월 1일에 페이지 A를 히트하고 2019년 6월 15일 00:01 이후에 돌아오는 모든 방문자 *다음 이전* 2019년 7월 1일이 필터에 포함됩니다. 시간 간격 제외의 섹션과 비교합니다.

다음 [!UICONTROL 다음 이후] 및 [!UICONTROL 다음 범위 내] 연산자를 함께 사용하여 순차적 필터를 정의할 수 있습니다.
