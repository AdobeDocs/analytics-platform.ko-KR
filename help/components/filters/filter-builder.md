...
1. **[!UICONTROL 데이터 보기 선택기]**: 이 필터를 저장할 보고서 세트를 선택할 수 있습니다. 모든 데이터 보기에서 필터를 계속 활용할 수 있습니다.
1. **[!UICONTROL 필터 미리 보기]**: 주요 지표를 미리 보기하여 유효한 필터가 있는지, 그리고 필터가 얼마나 광범위한지 확인할 수 있습니다. 이 필터를 적용하는지 확인할 수 있는 데이터 집합의 분류를 나타냅니다. 3개의 동심원 및 목록을 표시하여 일치 수 및 비율을 표시합니다 [!UICONTROL 이벤트], [!UICONTROL 개인], 및 [!UICONTROL Session] 데이터 세트에 대해 실행되는 필터입니다. 이 차트는 필터 정의를 만들거나 변경한 직후에 업데이트됩니다.
