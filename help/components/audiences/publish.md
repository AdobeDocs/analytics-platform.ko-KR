...
대상자가 생성된 후 Adobe는 각각의 새로운 CJA 대상자에 대해 Experience Platform 스트리밍 세그먼트를 만듭니다. AEP 스트리밍 세그먼트는 조직이 스트리밍 세그먼테이션에 대해 설정된 경우에만 생성됩니다.
...
| 3 | 스트리밍 세그먼트의 자동 만들기를 포함하여 실시간 고객 프로필에 게시하고 세그먼트를 데이터를 받을 준비가 되도록 합니다. | 약 60분 |
| 4 | 대상자에 대한 새로 고침 빈도 | <ul><li>일회성 새로 고침(지연 시간 5분 미만)</li><li>4시간마다, 매일, 매주, 매월 새로 고침(지연 시간은 새로 고침 빈도와 밀접한 관련이 있음) |
| 5 | AEP에서 대상 만들기: 새 세그먼트 활성화 | 1-2시간 |
