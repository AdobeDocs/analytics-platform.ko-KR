...
1. (선택 사항) 다른 작업 영역 프로젝트에서와 마찬가지로 이러한 시간 비교에 기반을 두고 시각화를 만들 수 있습니다. 예를 들어 다음은 막대 그래프입니다.
   ...
   ![Workspace 프로젝트 막대 그래프.](assets/compare-time-barchart.png)
...
1. 테이블에서 열을 마우스 오른쪽 단추로 클릭하고 **[!UICONTROL 기간 열 추가]** ![기간 열 추가 강조 표시된 테이블 열 목록 ](assets/add-time-period-column.png)
...
1. 원하는 만큼 여러 번 열을 추가하고 다른 날짜 범위를 조합할 수 있습니다.
   ...
   ![1년 전 이번 달, 이전 달, 이전 달 및 1년 전 이전 달의 1주일 동안의 발생 횟수를 보여 주는 자유 형식 테이블입니다.](assets/add-time-period-column4.png)
...
![선택한 같은 행에서 각 열의 날짜가 모두 시작되도록 정렬하는 테이블 설정 팝업이 있는 자유 형식 테이블.](assets/date-comparison-setting.png)
