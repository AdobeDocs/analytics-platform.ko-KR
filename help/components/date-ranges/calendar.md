...
달력을 사용하여 날짜 및 날짜 범위를 지정하거나 사전 설정을 선택할 수 있습니다. 날짜 범위는 Workspace 프로젝트에서 사용할 수 있는 구성 요소 유형입니다. 시간 경과에 따른 트렌드 데이터를 보거나 사건이 가장 많이 발생하는 시기를 확인할 수 있습니다. 날짜 범위는 자주색으로 구분됩니다. 사용자 지정 날짜 범위를 사용하면 Workspace 프로젝트에 표시되는 날짜를 사용자 지정할 수 있습니다.
...
![2022년 10월과 2022년 11월을 보여 주는 달력(11월 1일부터 30일까지)을 선택했습니다.](assets/aw_calendar2.png){width="60%"}
...
상대 날짜를 선택한 경우, 순환 날짜는 오늘 날짜가 아닌 패널 캘린더의 시작 날짜를 기준으로 합니다.
   * 이 옵션을 선택하지 않으면 순환 날짜는 오늘 날짜를 기준으로 합니다.
   ...
   패널 달력을 기준으로 날짜 범위 구성 요소를 선택한 ![달력](assets/relative-date-selected.png){width="60%"}
...
필터를 빌드하거나 순환 날짜가 있는 필터(예: 최근 7일 또는 최근 2주)를 사용하는 경우 필터 미리 보기를 클릭하면 패널 시작 날짜 대신 *오늘*&#x200B;부터 순환 날짜가 시작됩니다. 따라서 테이블에서 필터를 실제로 사용할 때는 필터 미리보기가 일치하지 않습니다. 필터 자체가 아니라 미리보기에 영향을 줍니다.
