---
title: 참조 - 고급 함수
description: 함수 드롭다운 목록에서 고급 표시를 선택하여 이 함수들에 액세스하십시오.
translation-type: tm+mt
source-git-commit: b521079bb9b3828ec3487b635366f5442f6fc4bd

---


# 참조 - 고급 함수

Access these functions by checking **[!UICONTROL Show Advanced]** in the **[!UICONTROL Functions]** drop-down list.

## 테이블 함수 대 행 함수

테이블 함수는 출력이 표의 모든 행에 대해 동일한 함수입니다. 행 함수는 출력이 표의 모든 행에 대해 다른 함수입니다.

## Include-Zeros 매개 변수는 무엇을 의미합니까?

계산에 0을 포함할지 여부를 지정합니다. 때때로 0은 &quot;아무것도&quot; 을 의미하지만, 때때로 그것은 중요합니다.

예를 들어 매출액 지표가 있는 다음 페이지 보기 지표를 보고서에 추가하는 경우 매출에 대해 모두 0인 행이 갑자기 더 많아집니다. MEAN, MIN, QUARTILE 등에 영향을 주지 않을 수 있습니다. 매출 열에 있는 계산. 이 경우 include-zeros 매개 변수를 확인합니다.

반면에, 관심이 있는 두 개의 지표가 있는 경우 일부 행이 0이기 때문에 평균이 높거나 최소값이 있다고 간주하는 것이 적절하지 않을 수 있으므로, 0을 포함하는 매개 변수를 확인하지 않습니다.

## 및

인수의 값을 반환합니다. NOT을 사용하여 값이 하나의 특정 값과 같지 않은지 확인합니다.

> [!NOTE] 0(영)은 False를 의미하며, 다른 값은 True입니다.

```
AND(logical_test1,[logical_test2],...)
```

| 인수 | 설명 |
|---|---|
| *logical_test1* | 필수. TRUE 또는 FALSE로 평가할 수 있는 임의 값 또는 표현식. |
| *logical_test2* | 선택 사항입니다. TRUE 또는 FALSE로 평가할 추가 조건 |

## 근사 고유 개수(차원)

선택한 차원에 대한 차원 항목의 근사 고유 개수를 반환합니다. 이 함수는 개별 카운트를 근사화하는 HLL(HyperLogLog) 메서드를 사용합니다.  값이 실제 값의 5% 이내 95% 이내임을 보장하도록 구성됩니다.

```
Approximate Count Distinct (dimension)
```

| 인수 |  |
|---|---|
| *차원* | 대략적인 고유 항목 수를 계산할 차원입니다. |

## 사용 사례 예

근사 고유 개수(고객 ID eVar)는 이 기능의 일반적인 사용 사례입니다.

새로운 &#39;예상 고객&#39; 계산된 지표에 대한 정의:

![](assets/approx-count-distinct.png)

이것은 보고에서 &quot;대략적인 고객&quot; 지표를 사용할 수 있는 방법입니다.

![](assets/approx-customers.png)

## 고유 수가 초과되었습니다

Count() 및 RowCount()와 마찬가지로 Approximate Count Distinct()는 [&quot;고유 수가 초과되었습니다&quot;](https://marketing.adobe.com/resources/help/en_US/reference/metrics_uniques_high_numbers.html)제한을 받습니다. 차원에 대한 특정 월 내에 &quot;고유 수가 초과되었습니다&quot; 제한에 도달하면 값이 1차원 항목으로 계산됩니다.

## 카운트 함수 비교

Approximate Count Distinct()는 Count() 및 RowCount() 함수에 대한 개선입니다. 만들어진 지표를 차원 보고서에서 사용하여 별도의 차원에 대한 항목의 대략적인 개수를 렌더링할 수 있기 때문입니다. 예를 들어 모바일 장치 유형 보고서에 사용된 고객 ID 수입니다.

이 함수는 HLL 메서드를 사용하지만 Count() 및 RowCount()는 정확한 카운트이므로 Count() 및 RowCount()보다 약간 더 정확합니다.

## 아크코사인(행)

지표의 아크코사인 또는 역 코사인을 반환합니다. 아크코사인은 코사인이 숫자인 각도입니다. 반환된 각도는 0에서 pi 범위의 라디안 단위로 지정됩니다. 라디안 단위 결과를 도 단위로 변환하려면 180/PI( )를 곱합니다.

```
ACOS(metric)
```

| 인수 |  |
|---|---|
| *metric* | 원하는 각도의 코사인(-1 ~ 1 범위). |

## 아크사인(행)

숫자의 아크사인 또는 역 사인을 반환합니다. 아크사인은 사인이 숫자인 각도입니다. 반환된 각도는 -pi/2에서 pi/2 범위의 라디안 단위로 지정됩니다. 아크사인을 도 단위로 표현하려면 결과에 180/PI( )를 곱합니다.

```
ASIN(metric) 
```

| 인수 |  |
|---|---|
| *metric* | 원하는 각도의 코사인(-1 ~ 1 범위). |

## 아크탄젠트(행)

숫자의 아크탄젠트 또는 역 탄젠트를 반환합니다. 아크탄젠트는 탄젠트가 숫자인 각도입니다. 반환된 각도는 -pi/2에서 pi/2 범위의 라디안 단위로 지정됩니다. 아크탄젠트를 도 단위로 표현하려면 결과에 180/PI( )를 곱합니다.

```
ATAN(metric)
```

| 인수 |  |
|---|---|
| *metric* | 원하는 각도의 코사인(-1 ~ 1 범위). |

## 지수 회귀: 예측된 Y(행)

를 기반으로 최적선을 계산하기 위해 &quot;최소 제곱법&quot;을 사용하여 알려진 x-값(metric_X)이 주어지면 예측된 y-값(metric_Y)을 계산합니다.

```
ESTIMATE.EXP(metric_X, metric_Y)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | 종속 데이터로 지정할 지표. |
| *metric_Y* | 독립 데이터로 지정할 지표. |

## Cdf-T

자유도가 n인 student-t 분포에서 x보다 적은 z-점수가 있는 값들의 비율을 반환합니다.

```
cdf_t( -∞, n ) = 0 
cdf_t(  ∞, n ) = 1 
cdf_t( 3, 5 ) ? 0.99865 
cdf_t( -2, 7 ) ? 0.0227501 
cdf_t( x, ∞ ) ? cdf_z( x )
```

## Cdf-Z

정규 분포에서 x보다 적은 z-점수가 있는 값들의 비율을 반환합니다.

```
cdf_z( -∞ ) = 0 
cdf_z( ∞ ) = 1 
cdf_z( 0 ) = 0.5 
cdf_z( 2 ) ? 0.97725 
cdf_z( -3 ) ? 0.0013499 
 
```

## 천정(행)

주어진 값보다 작지 않은 가장 작은 정수를 반환합니다. 예를 들어, 수입에 대해 소수 통화를 보고하지 않으려 하고, 제품에 $569.34가 있을 경우, 공식 CEILING(*수입*)을 사용하여 수입을 가장 근접한 달러 또는 $570으로 올림하십시오.

```
CEILING(metric)
```

| 인수 | 설명 |
|---|---|
| *metric* | 반올림할 지표. |

## 코사인(행)

주어진 각도의 코사인을 반환합니다. 각도가 도 단위인 경우 각도에 PI( )/180을 곱합니다.

```
COS(metric)
```

| 인수 | 설명 |
|---|---|
| *metric* | 코사인이 필요한 라디안 단위 각도. |

## 세제곱근

숫자의 양수 세제곱근을 반환합니다. 숫자의 세제곱근은 해당 숫자의 1/3승(거듭제곱) 값입니다.

```
CBRT(metric)
```

| 인수 | 설명 |
|---|---|
| *metric* | 세제곱근이 필요한 지표. |

## 누적

문자열 기반 필드에 대해 해시 값을 사용하여 마지막 N개 행에 대한 x의 합을 반환합니다.

N &lt;= 0이면 이전 행을 모두 사용합니다. 차원으로 순서가 지정되므로 날짜 또는 경로 길이와 같은 자연스런 순서가 있는 차원에서만 유용합니다.

```
| Date | Rev  | cumul(0,Rev) | cumul(2,Rev) | 
|------+------+--------------+--------------| 
| May  | $500 | $500         | $500         | 
| June | $200 | $700         | $700         | 
| July | $400 | $1100        | $600         | 
 
```

## 누적 평균

마지막 N개 행의 평균을 반환합니다.

N &lt;= 0이면 이전 행을 모두 사용합니다. 차원으로 순서가 지정되므로 날짜 또는 경로 길이와 같은 자연스런 순서가 있는 차원에서만 유용합니다.

> [!NOTE] 수입/방문자와 같은 비율 지표에는 예상대로 이렇게 적용되지 않습니다. 마지막 N에 대한 수입을 합하고 마지막 N에 대한 방문자의 수를 합한 다음 이 수들을 나누는 대신 비율의 평균을 구합니다. 대신, 다음 공식을 사용하십시오.

```
cumul(revenue)/cumul(visitor)
```

## Equal

숫자나 문자열 값에 대해 정확히 일치하는 항목을 반환합니다.

## 지수 회귀_ 상관 계수(테이블)

다음 회귀방정식에 대한 두 지표 열(*metric_A*&#x200B;와 *metric_B*) 간의 상관 계수 *r*&#x200B;을 반환합니다.

```
CORREL.EXP(metric_X, metric_Y)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | *metric_Y와 관련지을 지표*. |
| *metric_Y* | *metric_X와 관련지을 지표*. |

## 지수 회귀: 절편(테이블)

다음 식에 대한 두 지표 열(*metric_X*&#x200B;와 *metric_Y*) 간의 절편 *b*&#x200B;를 반환합니다.

```
INTERCEPT.EXP(metric_X, metric_Y)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | 종속 데이터로 지정할 지표. |
| *metric_Y* | 독립 데이터로 지정할 지표. |

## 지수 회귀: 기울기(테이블)

다음 식에 대한 두 지표 열(*metric_X*&#x200B;와 *metric_Y*) 간의 기울기 *a*&#x200B;를 반환합니다.

```
SLOPE.EXP(metric_X, metric_Y)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | 종속 데이터로 지정할 지표. |
| *metric_Y* | 독립 데이터로 지정할 지표. |

## 내림(행)

주어진 값보다 크지 않은 가장 큰 정수를 반환합니다. 예를 들어, 수입에 대해 소수 통화를 보고하지 않으려 하고, 제품에 $569.34가 있을 경우, 공식 FLOOR(*수입*)을 사용하여 수입을 가장 근접한 달러 또는 $569로 내림하십시오.

```
FLOOR(metric)
```

| 인수 | 설명 |
|---|---|
| *metric* | 반올림할 지표. |

## 보다 큼

숫자 값이 입력한 값보다 큰 항목을 반환합니다.

## 크거나 같음

숫자 값이 입력한 값보다 크거나 같은 항목을 반환합니다.

## 쌍곡코사인(행)

숫자의 쌍곡코사인을 반환합니다.

```
COSH(metric)
```

| 인수 | 설명 |
|---|---|
| *metric* | 쌍곡코사인을 찾을 라디안 단위 각도. |

## 쌍곡사인(행)

숫자의 쌍곡사인을 반환합니다.

```
SINH(metric)
```

| 인수 | 설명 |
|---|---|
| *metric* | 쌍곡사인을 찾을 라디안 단위 각도. |

## 쌍곡탄젠트(행)

숫자의 쌍곡탄젠트를 반환합니다.

```
TANH(metric)
```

| 인수 | 설명 |
|---|---|
| *metric* | 쌍곡탄젠트를 찾을 라디안 단위 각도. |

## IF(행)

IF 함수는 지정하는 조건이 TRUE로 평가되면 한 값을 반환하고, 해당 조건이 FALSE로 평가되면 다른 값을 반환합니다.

```
IF(logical_test, [value_if_true], [value_if_false])
```

| 인수 | 설명 |
|---|---|
| *logical_test* | 필수. TRUE 또는 FALSE로 평가할 수 있는 임의 값 또는 표현식. |
| *[value_if_true]* | *logical_test* 인수가 TRUE로 평가되는 경우 반환할 값입니다.이 인수는 포함되지 않은 경우 기본적으로 0으로 설정됩니다. |
| *[value_if_false]* | *logical_test* 인수가 FALSE로 평가되는 경우 반환할 값입니다.이 인수는 포함되지 않은 경우 기본적으로 0으로 설정됩니다. |

## 보다 작음

숫자 값이 입력한 값보다 작은 항목을 반환합니다.

## 작거나 같음

숫자 값이 입력한 값보다 작거나 같은 항목을 반환합니다.

## 선형 회귀_ 상관 계수

Y = a X + b. 상관 계수를 반환합니다.

## 선형 회귀_ 절편

Y = a X + b. b를 반환합니다.

## 선형 회귀_ 예측된 Y

Y = a X + b. Y를 반환합니다.

## 선형 회귀_ 기울기

Y = a X + b. a를 반환합니다.

## 로그 밑 10(행)

숫자의 밑이 10인 로그를 반환합니다.

```
LOG10(metric)
```

| 인수 | 설명 |
|---|---|
| *metric* | 밑이 10인 로그가 필요한 양의 실수. |

## 로그 회귀: 상관 계수(테이블)

회귀방정식 [!DNL Y = a ln(X) + b]에 대한 두 지표 열(*metric_X*&#x200B;와 *metric_Y*) 간의 상관 계수 *r*&#x200B;을 반환합니다. 이것은 CORREL 방정식을 사용하여 계산됩니다.

```
CORREL.LOG(metric_X,metric_Y)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | *metric_Y와 관련지을 지표*. |
| *metric_Y* | *metric_X와 관련지을 지표*. |

## 로그 회귀: 절편(테이블)

회귀방정식 *에 대한 두 지표 열(* metric_X *와* metric_Y *) 간의 최소 제곱 회귀로서* b[!DNL Y = a ln(X) + b]를 반환합니다. 이것은 INTERCEPT 방정식을 사용하여 계산됩니다.

```
INTERCEPT.LOG(metric_X, metric_Y)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | 종속 데이터로 지정할 지표. |
| *metric_Y* | 독립 데이터로 지정할 지표. |

## 로그 회귀: 예측된 Y(행)

[!DNL Y = a ln(X) + b]를 기반으로 최적선을 계산하기 위해 &quot;최소 제곱법&quot;을 사용하여 알려진 [!DNL x] 값(metric_X)이 주어지면 예측된 [!DNL y] 값(metric_Y)을 계산합니다. 이것은 ESTIMATE 방정식을 사용하여 계산됩니다.

회귀 분석에서 이 함수는 회귀 방정식 [!DNL Y = a ln(X) + b]에 최적선을 계산하기 위해 로그를 사용하여 알려진 [!DNL x] 값(*metric_X*)이 주어질 때 예측된 [!DNL y] 값(*metric_Y*)을 계산합니다. [!DNL a] 값은 각 x 값에 해당하고 [!DNL b]는 상수 값입니다.

```
ESTIMATE.LOG(metric_X, metric_Y)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | 종속 데이터로 지정할 지표. |
| *metric_Y* | 독립 데이터로 지정할 지표. |

## 로그 회귀: 기울기(테이블)

회귀방정식 [!DNL Y = a ln(X) + b]에 대한 두 지표 열(*metric_X*&#x200B;와 *metric_Y*) 간의 기울기 *a*&#x200B;를 반환합니다. 이것은 SLOPE 방정식을 사용하여 계산됩니다.

```
SLOPE.LOG(metric_A, metric_B)
```

| 인수 | 설명 |
|---|---|
| *metric_A* | 종속 데이터로 지정할 지표. |
| *metric_B* | 독립 데이터로 지정할 지표. |

## 자연 로그

Returns the natural logarithm of a number. Natural logarithms are based on the constant *e* (2.71828182845904). LN is the inverse of the EXP function.

```
LN(metric)
```

| 인수 | 설명 |
|---|---|
| *metric* | 자연 로그가 필요한 양의 실수. |

## NOT

숫자가 0이면 1을 반환하고 다른 숫자이면 0을 반환합니다.

```
NOT(logical)
```

| 인수 | 설명 |
|---|---|
| *논리* | 필수. TRUE 또는 FALSE로 평가할 수 있는 값 또는 표현식. |

NOT을 사용하기 위해서는 표현식(&lt;, >, =, &lt;> 등)이 0이나 1 값을 반환할지 여부를 0 또는 1 값을 반환합니다.

## 같지 않음

입력한 값의 정확한 일치를 포함하지 않는 모든 항목을 반환합니다.

## Or(행)

인수가 TRUE이면 TRUE를 반환하고, 인수가 FALSE이면 FALSE를 반환합니다.

> [!NOTE] 0(영)은 False를 의미하며, 다른 값은 True입니다.

```
OR(logical_test1,[logical_test2],...)
```

| 인수 | 설명 |
|---|---|
| *logical_test1* | 필수. TRUE 또는 FALSE로 평가할 수 있는 임의 값 또는 표현식. |
| *logical_test2* | 선택 사항입니다. TRUE 또는 FALSE로 평가할 추가 조건 |

## Pi

상수 PI를 정확히 15자릿수 3.14159265358979로 반환합니다.

```
PI()
```

[!DNL PI] 함수에는 인수가 없습니다.

## 거듭제곱 회귀: 상관 계수(테이블)

[!DNL Y = b*X]에 대한 두 지표 열(*metric_X*&#x200B;와 *metric_Y*) 간의 상관 계수 *r*&#x200B;을 반환합니다.

```
CORREL.POWER(metric_X, metric_Y)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | *metric_Y와 관련지을 지표*. |
| *metric_Y* | *metric_X와 관련지을 지표*. |

## 거듭제곱 회귀: 절편(테이블)

[!DNL Y = b*X]에 대한 두 지표 열(*metric_X*&#x200B;와 *metric_Y*) 간의 절편 *b*&#x200B;를 반환합니다.

```
 INTERCEPT.POWER(metric_X, metric_Y)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | 종속 데이터로 지정할 지표. |
| *metric_Y* | 독립 데이터로 지정할 지표. |

## 거듭제곱 회귀: 예측된 Y(행)

[!DNL Y = b*X]에 대해 최적선을 계산하기 위해 &quot;최소 제곱법&quot;을 사용하여 알려진 [!DNL x] 값([!DNL metric_X])이 주어지면 예측된 [!DNL y] 값([!DNL metric_Y])을 계산합니다.

```
 ESTIMATE.POWER(metric_X, metric_Y)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | 종속 데이터로 지정할 지표. |
| *metric_Y* | 독립 데이터로 지정할 지표. |

## 거듭제곱 회귀: 기울기(테이블)

[!DNL Y = b*X]에 대한 두 지표 열(*metric_X*&#x200B;와 *metric_Y*) 간의 기울기 *a*&#x200B;를 반환합니다.

```
SLOPE.POWER(metric_X, metric_Y)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | 종속 데이터로 지정할 지표. |
| *metric_Y* | 독립 데이터로 지정할 지표. |

## 이차 회귀: 상관 계수(테이블)

[!DNL Y=(a*X+b)]****에 대한 두 지표 열(*metric_X*&#x200B;와 *metric_Y*) 간의 상관 계수 *r*&#x200B;을 반환합니다.

```
CORREL.QUADRATIC(metric_X, metric_Y)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | *metric_Y와 관련지을 지표*. |
| *metric_Y* | *metric_X와 관련지을 지표*. |

## 이차 회귀: 절편(테이블)

[!DNL Y=(a*X+b)]****에 대한 두 지표 열(*metric_X*&#x200B;와 *metric_Y*) 간의 절편 *b*&#x200B;를 반환합니다.

```
INTERCEPT.POWER(metric_X, metric_Y)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | 종속 데이터로 지정할 지표. |
| *metric_Y* | 독립 데이터로 지정할 지표. |

## 이차 회귀: 예측된 Y(행)

[!DNL Y=(a*X+b)]****를 사용하여 최적선을 계산하기 위해 최소 제곱법을 사용하여 알려진 [!DNL x] 값(metric_X)이 주어질 때 예측된 [!DNL y] 값(metric_Y)을 계산합니다.

```
ESTIMATE.QUADRATIC(metric_A, metric_B)
```

| 인수 | 설명 |
|---|---|
| *metric_A* | 종속 데이터로 지정할 지표. |
| *metric_B* | 종속 데이터로 지정할 지표. |

## 이차 회귀: 기울기(테이블)

[!DNL Y=(a*X+b)]****에 대한 두 지표 열(*metric_X*&#x200B;와 metric_Y) 간의 기울기 *a*&#x200B;를 반환합니다.

```
SLOPE.QUADRATIC(metric_X, metric_Y)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | 종속 데이터로 지정할 지표. |
| *metric_Y* | 독립 데이터로 지정할 지표. |

## 역수 회귀: 상관 계수(테이블)

*에 대한 두 지표 열(* metric_X)*와* metric_Y *) 간의 상관 계수* r[!DNL Y = a/X+b]을 반환합니다.

```
CORREL.RECIPROCAL(metric_X, metric_Y)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | *metric_Y와 관련지을 지표*. |
| *metric_Y* | *metric_X와 관련지을 지표*. |

## 역수 회귀: 절편(테이블)

[!DNL Y = a/X+b]에 대한 두 지표 열(*metric_X*&#x200B;와 *metric_Y*) 간의 절편 *b*&#x200B;를 반환합니다.

```
INTERCEPT.RECIPROCAL(metric_A, metric_B)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | 종속 데이터로 지정할 지표. |
| *metric_Y* | 독립 데이터로 지정할 지표. |

## 역수 회귀: 예측된 Y(행)

[!DNL Y = a/X+b]를 사용하여 최적선을 계산하기 위해 최소 제곱법을 사용하여 알려진 [!DNL x] 값(metric_X)이 주어질 때 예측된 [!DNL y] 값(metric_Y)을 계산합니다.

```
ESTIMATE.RECIPROCAL(metric_X, metric_Y)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | 종속 데이터로 지정할 지표. |
| *metric_Y* | 독립 데이터로 지정할 지표. |

## 역수 회귀: 기울기(테이블)

[!DNL Y = a/X+b]에 대한 두 지표 열(*metric_X*&#x200B;와 *metric_Y*) 간의 기울기 *a*&#x200B;를 반환합니다.

```
SLOPE.RECIPROCAL(metric_X, metric_Y)
```

| 인수 | 설명 |
|---|---|
| *metric_X* | 종속 데이터로 지정할 지표. |
| *metric_Y* | 독립 데이터로 지정할 지표. |

## 사인(행)

주어진 각도의 사인을 반환합니다. 각도가 도 단위인 경우 각도에 PI( )/180을 곱합니다.

```
SIN(metric)
```

| 인수 | 설명 |
|---|---|
| *metric* | 사인이 필요한 라디안 단위 각도. |

## T 스코어

T 스코어에 대한 별칭, 즉, 표준 편차로 나눈 평균과의 편차

## T-테스트

t 점수가 col이고 자유도가 n인 m측 검증 t 테스트를 수행합니다.

서명은 `t_test( x, n, m )`입니다. 이 식은 아래에서 `m*cdf_t(-abs(x),n)`을 호출합니다. (이것은 `m*cdf_z(-abs(x))`를 실행하는 z 테스트 함수와 유사합니다.)

여기서 `m`은 꼬리(tail) 수이고 `n`은 자유 정도입니다. 둘 모두 숫자여야 합니다(전체 보고서에서 상수, 즉, 행 단위로 변경되지 않음).

`X`는 t-test 통계로 종종 지표를 기반으로 하는 공식(예: zscore)으로 표시되며 모든 행에서 평가됩니다.

반환 값은 자유도 및 꼬리 수가 주어진 테스트 통계 x을 볼 가능성입니다.

**예:**

1. 범위 밖 아웃라이어를 찾을 때 사용:

   ```
   t_test( zscore(bouncerate), row-count-1, 2)
   ```

1. 매우 높거나 낮은 바운스 비율을 무시하기 위해 `if`와 결합하여 사용하며 그 외 다른 모든 경우에 방문 수를 계산합니다.

   ```
   if ( t_test( z-score(bouncerate), row-count, 2) < 0.01, 0, visits )
   ```

## 탄젠트

주어진 각도의 탄젠트를 반환합니다. 각도가 도 단위인 경우 각도에 PI( )/180을 곱합니다.

```
TAN (metric)
```

| 인수 | 설명 |
|---|---|
| *metric* | 탄젠트가 필요한 라디안 단위 각도. |

## Z 스코어(행)

정규 분포를 기준으로 Z 점수 또는 정규 점수를 반환합니다. Z 스코어는 평균에서 관찰하는 표준 편차의 수입니다. Z 스코어가 0(영)이면 점수는 평균과 동일합니다. Z 스코어는 양수 또는 음수일 수 있으며, 이것은 평균의 위 또는 아래 여부와 표준 편차의 수를 나타냅니다.

Z 스코어에 대한 방정식은 다음과 같습니다.

![](assets/z_score.png)

여기서 [!DNL x]는 원시 스코어이고, [!DNL μ]는 인구의 평균이고 [!DNL σ]는 인구의 표준 편차입니다.

> [!NOTE][!DNL μ] (mu)와[!DNL σ] (sigma) 는 지표에서 자동으로 계산됩니다.

Z 점수(지표)

<table id="table_AEA3622A58F54EA495468A9402651E1B"> 
 <thead> 
  <tr> 
   <th colname="col1" class="entry"> 인수 </th> 
   <th colname="col2" class="entry"> 설명 </th> 
  </tr> 
 </thead>
 <tbody> 
  <tr> 
   <td colname="col1"> <i>metric</i> </td> 
   <td colname="col2"> <p> 0이 아닌 첫 번째 인수의 값을 반환합니다. </p> </td> 
  </tr> 
 </tbody> 
</table>

## Z-테스트

Z 스코어가 A인 n측 검증 Z 테스트를 수행합니다.

현재 행이 열에서 우연히 볼 수 있는 확률을 반환합니다.

> [!NOTE] 값은 정상적으로 배분된다고 가정합니다.

