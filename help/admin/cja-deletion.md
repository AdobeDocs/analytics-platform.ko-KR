...
| [!UICONTROL Adobe Experience Platform]에서 데이터 세트를 삭제하시겠습니까 | AEP에서 데이터 세트를 삭제하면 해당 데이터 세트에서 해당 데이터 세트를 포함하는 모든 연결로의 데이터 흐름이 중지됩니다. 해당 데이터 세트의 데이터는 연결된 Customer Journey Analytics 연결에서 자동으로 삭제되지 않습니다. |
