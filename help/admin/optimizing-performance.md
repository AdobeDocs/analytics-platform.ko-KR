...
| **차원 설정의 복잡성** | 매우 복잡한 차원은 프로젝트 성능에 중요한 영향을 줄 수 있습니다. | 사용자 정의 필드는 복잡성을 추가합니다. | 사용자 지정 필드의 수를 줄이거나 별도의 차원을 만듭니다. |
| **고유 값이 많은 Dimension** | 높은 카디널리티 차원으로도 알려진 이 차원은 다음에 대해 보고될 수 있는 것에 영향을 줄 수 있습니다. | 다음을 참조하십시오 [높은 카디널리티 차원](/help/components/dimensions/high-cardinality.md) | 다음을 참조하십시오 [높은 카디널리티 차원](/help/components/dimensions/high-cardinality.md) |
