...
* 요청을 취소할 때 지정된 기간 동안 후속 요청을 제한하도록 선택할 수도 있습니다.
  ...
  후속 요청을 제한하는 경우 해당 액션은 EMBARGO라는 액션 이름으로 [감사 로그](/help/privacy/audit-log.md)에 기록됩니다.

* 요청의 [!UICONTROL **사용자**] 열이 [!UICONTROL **인식되지 않음**]&#x200B;으로 표시되는 경우 요청을 취소할 수 없습니다. 이런 현상이 발생하는 경우, 해당 사용자가 관리 권한이 없는 로그인 회사에 속해 있다는 의미입니다.

보고 활동 관리자에 대한 자세한 내용, 주요 이점 및 권한 요구 사항을 알아보려면 [보고 활동 관리자 개요](/help/reporting-activity-manager/reporting-activity-overview.md)를 참조하십시오.

## 특정 요청 취소

보고 용량이 많이 소모되는 개별 요청을 취소할 수 있습니다. 요청을 취소할 때 특정 기간 동안 요청을 더욱 제한하도록 선택할 수 있습니다.

1. Customer Journey Analytics에서 **[!UICONTROL 도구]** > **[!UICONTROL 보고 활동 관리자]**&#x200B;로 이동합니다.

1. 보고 요청을 취소할 연결을 선택합니다. <!--double-check this step-->

   이 페이지에서 제공하는 데이터에 대한 자세한 내용은 [보고 활동 관리자에서 보고 활동 보기](/help/reporting-activity-manager/reporting-activity.md)를 참조하십시오.
...
1. (선택 사항) 지정된 기간 동안 향후 요청을 제한하는 방법:
   ...
   1. [!UICONTROL **후속 요청 제한**] 옵션 활성화.
      ...
      ![1개의 요청을 취소하면 “후속 요청 제한”이 선택되어 있고 취소 메시지가 표시됩니다.](assets/restrict-subsequent-requests.png)

   1. 다음 선택 사항 중 하나를 선택합니다.

      | 옵션 | 함수 |
      |---------|----------|
      | [!UICONTROL **사용자 및 프로젝트**] | 선택한 요청과 관련된 사용자는 관련 프로젝트에 대한 보고 요청을 실행하지 못하도록 일시적으로 제한됩니다. |
...
한 명 이상의 사용자와 관련된 모든 요청을 취소할 수 있습니다. 사용자와 관련된 요청을 취소할 때 지정된 기간 동안 해당 사용자의 요청을 추가로 제한하도록 선택할 수 있습니다.
...
1. (선택 사항) 지정된 기간 동안 향후 요청을 제한하는 방법:
   ...
   1. [!UICONTROL **후속 요청 제한**] 옵션 활성화
      ...
      ![선택된 사용자별 후속 요청 제한을 표시하는 1개의 요청을 취소합니다.](assets/restrict-subsequent-requests-user.png)

   1. 다음 선택 사항 중 하나를 선택합니다.

      | 옵션 | 함수 |
      |---------|----------|
      | [!UICONTROL **사용자 및 프로젝트**] | 선택한 사용자는 연관된 프로젝트에 대해 보고 요청을 하지 못하도록 일시적으로 제한됩니다. <p>가장 덜 제한적인 옵션입니다.</p> |
      | [!UICONTROL **사용자**] | 선택한 사용자는 보고 요청을 하지 못하도록 일시적으로 제한됩니다. |
...
한 개 이상의 프로젝트와 관련된 모든 요청을 취소할 수 있습니다. 프로젝트와 관련된 요청을 취소할 때 지정된 기간 동안 해당 프로젝트와 관련된 요청을 추가로 제한하도록 선택할 수 있습니다.
...
1. (선택 사항) 지정된 기간 동안 향후 요청을 제한하는 방법:
   ...
   1. [!UICONTROL **후속 요청 제한**] 옵션 활성화.
      ...
      ![프로젝트별 후속 요청 제한을 표시하는 1개 요청 취소](assets/restrict-subsequent-requests-project.png)

   1. 다음 선택 사항 중 하나를 선택합니다.

      | 옵션 | 함수 |
      |---------|----------|
      | [!UICONTROL **사용자 및 프로젝트**] | 선택한 프로젝트는 연관된 사용자가 수행하는 모든 보고 요청에서 일시적으로 제한됩니다.<p>가장 덜 제한적인 옵션입니다.</p> |
      | [!UICONTROL **사용자**] | 선택한 프로젝트와 연계된 사용자는 보고 요청을 전혀 할 수 없도록 제한됩니다. |
...
1. (선택 사항) 지정된 기간 동안 향후 요청을 제한하는 방법:
   ...
   1. [!UICONTROL **후속 요청 제한**] 옵션 활성화
      ...
      ![선택된 애플리케이션별 후속 요청 제한을 표시하는 1개의 요청을 취소합니다.](assets/restrict-subsequent-requests-application.png)

   1. 다음 선택 사항 중 하나를 선택합니다.

      | 옵션 | 함수 |
      |---------|----------|
      | [!UICONTROL **사용자 및 프로젝트**] | 선택한 애플리케이션은 연관된 사용자와 프로젝트가 수행하는 모든 보고 요청이 일시적으로 제한됩니다.<p>가장 덜 제한적인 옵션입니다.</p> |
      | [!UICONTROL **사용자**] | 선택한 애플리케이션과 연관된 사용자는 어떤 보고 요청도 수행할 수 없도록 제한됩니다. |
