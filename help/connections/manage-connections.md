...
| [!UICONTROL 생략된 기록] 위젯 | 선택된 기간 내에 **선택한 데이터 세트 및 날짜 범위**&#x200B;에 대해 얼마나 많은 행이 생략되었는지 나타냅니다. 레코드를 건너뛰는 이유는 다음과 같습니다. 타임스탬프가 없거나 누락된 사람 ID 등이 있습니다. 10분마다 업데이트됩니다.<p>**참고**: **[!UICONTROL 생략된 레코드]**&#x200B;에 대한 데이터에는 현재 프로필이나 조회 데이터가 아닌 이벤트 데이터만 포함됩니다.<p>잘못된 개인 ID(정의되지 않음, 모든 0)는 특정 사용자 또는 개인에게 귀속될 수 없습니다. 시스템에 수집할 수 없으며 오류가 발생하기 쉬운 수집 및 보고 프로세스가 됩니다. 잘못된 개인 ID를 수정하려면 다음 3가지 옵션을 사용할 수 있습니다.<ul><li>결합을 사용하여 정의되지 않았거나 0이 아닌 사용자 ID를 유효한 사용자 ID로 채웁니다.</li><li>사용자 ID를 비워 두면 수집 중에 생략됩니다(유효하지 않거나 0인 사용자 ID보다 선호됨).</li><li>데이터를 수집하기 전에 시스템에서 잘못된 사용자 ID를 수정하십시오.</li></ul> |
...
| [!UICONTROL 배치 추가됨] | 이 데이터 세트에 추가된 데이터 일괄 처리의 수를 나타냅니다. |
| [!UICONTROL 생략된 레코드] | 선택한 기간에 수집되는 동안 건너뛴 행 수를 나타냅니다.<p>**참고**: **[!UICONTROL 생략된 레코드]**&#x200B;에 대한 데이터에는 현재 프로필이나 조회 데이터가 아닌 이벤트 데이터만 포함됩니다.<p>잘못된 개인 ID(정의되지 않음, 모든 0)는 특정 사용자 또는 개인에게 귀속될 수 없습니다. 시스템에 수집할 수 없으며 오류가 발생하기 쉬운 수집 및 보고 프로세스가 됩니다. 잘못된 개인 ID를 수정하려면 다음 3가지 옵션을 사용할 수 있습니다.<ul><li>결합을 사용하여 정의되지 않았거나 0이 아닌 사용자 ID를 유효한 사용자 ID로 채웁니다.</li><li>사용자 ID를 비워 두면 수집 중에 생략됩니다(유효하지 않거나 0인 사용자 ID보다 선호됨).</li><li>데이터를 수집하기 전에 시스템에서 잘못된 사용자 ID를 수정하십시오.</li></ul> |
| [!UICONTROL 마지막으로 추가됨] | 마지막 일괄 처리가 추가된 시기를 나타냅니다. |
