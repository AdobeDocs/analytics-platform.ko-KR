...
새로운 연결 워크플로는 최근 CJA(Customer Journey Analytics)에서 시작되었습니다. 새로운 연결 만들기 및 편집 워크플로우 경험은 모든 데이터 세트 및 연결 구성 설정을 보조 워크플로우를 사용하여 화면 중앙에 가져옵니다.  데이터 세트 유형, 크기, 스키마, 데이터 세트 ID, 배치 상태, 채우기 상태, 개인 ID 등과 같은 중요한 정보가 포함된 세부 데이터 세트 선택, 구성 및 검토 경험을 제공하여 잘못된 연결 구성의 위험을 줄였습니다. 다음은 새로운 기능에 대한 개요입니다.
...
1. 연결 설정을 구성합니다.
   ...
   | **[!UICONTROL 채우기 데이터]** | 이벤트 타임스탬프를 기반으로 데이터 집합에 있는 데이터 채우기 요청을 할 수 있습니다. 예를 들어, 최근 7일 동안의 데이터를 채우기 위해 요청하거나, 올바른 개인 ID를 구성하고, 올바른 구성을 위해 연결을 테스트할 수 있습니다. 모든 것이 좋아 보이면 나머지 데이터를 모두 쉽게 채울 수 있습니다.<p>또한 데이터 집합별로 새 데이터를 가져올 수 있습니다. 예를 들어 조회 데이터에 대해서만 새 데이터 가져오기를 활성화할 수 있습니다. |
...
1. 하나 이상의 데이터 세트를 선택하고 **[!UICONTROL 다음]**&#x200B;을 클릭합니다.

   하나 이상의 이벤트 데이터 세트가 연결의 일부가 되어야 합니다.
1. 이제 데이터 세트를 하나씩 구성합니다.
   ...
   | **[!UICONTROL 데이터 세트 크기]** | 데이터 세트의 크기입니다. |
   ...
   | **[!UICONTROL 제거]** | 데이터 세트를 삭제하거나 제거하고 전체 연결을 삭제하지 않고 개인 ID를 변경할 수 있습니다. 이렇게 하면 데이터 수집과 관련된 비용과 전체 연결 및 관련 데이터 보기를 다시 만드는 번거로운 프로세스가 줄어듭니다. |
...
이 미리보기에는 연결 구성을 나열하는 열이 여러 개 있습니다. 표시되는 열 유형은 개별 데이터 세트에 따라 다릅니다.
...
1. [Adobe Experience Platform 쿼리 서비스](https://experienceleague.adobe.com/docs/experience-platform/query/home.html?lang=ko)로 이동하여 새 쿼리를 만듭니다.
