...
사용자는 이러한 대규모 조회 데이터 세트의 알고리즘 가지치기를 요청할 수 있습니다. 이 가지치기에서는 연결의 이벤트 데이터 세트 부분에서 키를 사용할 수 있는 조회 데이터 세트의 데이터만 로드합니다. 알고리즘에 사용되는 참조 횟수는 90일 전까지 돌아가며 매주 한 번씩 새로 고쳐집니다.
