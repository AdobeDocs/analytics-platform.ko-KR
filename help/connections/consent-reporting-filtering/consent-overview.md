...
동의 보고 및 필터링은 두 가지 별개의 기능입니다. **동의 보고**&#x200B;를 통해 Analysis Workspace을 사용하여 Experience Platform 프로필 데이터 세트에 구성된 다양한 동의 정책과 일치하는 방문자를 보고할 수 있습니다. **동의 필터링**&#x200B;은(는) 수집 시 동의하지 않는 방문자를 제외하도록 Customer Journey Analytics에 지시합니다.

동의 보고 또는 필터링을 개별적으로 활성화하거나 둘 다 함께 사용할 수 있습니다. 동의 보고만 활성화하면 많은 비즈니스 사용 사례에 충분합니다.

다음 정보는 각 구성을 사용하는 이유를 설명합니다.

* **가장 일반적인 항목:**

  **보고에 동의 데이터 사용(필터링 안 함)**: 이 구성은 Analysis Workspace에서 동의 정책 멤버십을 분석하려는 경우 유용하며, 수집에서 방문자 데이터를 제외할 필요가 없습니다.

* **덜 일반적:**

  **동의 데이터를 보고 및 필터링할 때 모두 사용**: 이 구성은 Analysis Workspace에서 동의 정책 멤버십 데이터를 분석하려는 경우와 조직에서 수집 시 동의하지 않는 방문자 데이터를 제외하도록 요구하는 경우에 유용합니다.

* **일반적이지 않음:**

  **동의 데이터 필터링(보고 없음)**: 이 구성은 흔하지 않지만, 조직에서 수집 시 동의하지 않는 방문자 데이터를 제외하도록 요구하지만 필터링된 데이터에 속하지 않는 다른 동의 선택 사항에 대해 보고할 필요는 없는 경우 유용합니다. 예를 들어, 조직의 준수 요구 사항은 보고에서 사용하는 동의 정책 조회 데이터 세트를 만들고 유지 관리할 필요 없이 동의를 기반으로 데이터를 필터링하도록 요구할 수 있습니다.
