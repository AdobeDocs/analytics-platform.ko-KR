...
description: Customer Journey Analytics에서 데이터 세트를 결합하여 연결을 만드는 방법을 알아봅니다.
...
| example_id | 타임스탬프 | string_color | string_animal | metric_a |
| --- | --- | --- | --- | ---: |
| user_310 | 1월 1일 오전 7:02 | 빨간색 | 여우 | |
| user_310 | 1월 1일 오전 7:04 | | | 2 |
| user_310 | 1월 1일 오전 7:08 | 파란색 | | 3 |
| user_847 | 1월 2일 12:31 오후 | | 거북 | 4 |
| user_847 | 1월 2일 12:44 오후 | | | 2 |

| different_id | 타임스탬프 | string_color | string_shape | metric_b |
| --- | --- | --- | --- | ---: |
| user_847 | 1월 2일 12:26 오후 | 노란색 | 원 | 8.5 |
...
| ID | 타임스탬프 | string_color | string_animal | string_shape | metric_a | metric_b |
| --- | --- | --- | --- | --- | ---: | ---: |
| user_310 | 1월 1일 오전 7:02 | 빨간색 | 여우 | | | |
| user_310 | 1월 1일 오전 7:04 | | | | 2 | |
| user_310 | 1월 1일 오전 7:08 | 파란색 | | | 3 | |
...
| ID | 타임스탬프 | _experience.<br/>무엇이든.<br/>string_color | _experience.<br/>다른 기능.<br/>string_color | string_animal | string_shape | metric_a | metric_b |
|---|---|---|---|---|---|---:|---:|
| user_310 | 1월 1일 오전 7:02 | 빨간색 | | 여우 | | | |
| user_310 | 1월 1일 오전 7:04 | | | | | 2 | |
| user_310 | 1월 1일 오전 7:08 | 파란색 | | | | 3 | |
...
이 개념은 속성에도 적용됩니다. 행이 어느 데이터 세트에 포함되어 있든 상관없습니다. 속성은 모든 이벤트가 단일 데이터 세트에 포함된 것처럼 정확하게 작동합니다. 위의 테이블을 예로 사용합니다.

연결에 첫 번째 테이블만 포함되고 두 번째 테이블은 포함되지 않은 경우 `string_color` 차원을 사용하여 보고서를 가져오고 마지막 터치 속성을 사용하여 `metric_a` 지표를 가져오면 다음이 표시됩니다.
...
하지만 두 테이블을 모두 연결에 포함한 경우 `user_847`이 두 데이터 세트에 있으므로 속성 변경됩니다. 두 번째 데이터 세트 속성의 한 행 `metric_a`가 이전에 지정되지 않은 &#39;노란색&#39;이면 다음이 표시됩니다.
