...
CCA에서 수집 시 개별 이벤트를 알려진 장치와 채널에 결합하려고 합니다. Bob이 두 개의 채널을 사용하는 다음 예를 생각해봅시다.
...
| `4` | `3579` | - | - | `3579` | Bob이 인증되지 않은 모바일 장치에서 귀하의 사이트에 액세스함 | `3` (246, Bob 및 3579) |
...
새 장치에서 인증되지 않은 이벤트와 인증된 이벤트는 모두 별도의 사람(일시적으로)으로 계산됩니다. 인식된 장치의 인증되지 않은 이벤트는 라이브 결합됩니다.

속성은 식별 사용자 지정 변수가 장치에 연결되자 마자 작동합니다. 위의 예에서 이벤트 1과 4를 제외한 모든 이벤트는 라이브로 결합됩니다(모두 `Bob` 식별자를 사용). 속성은 재생 결합이 끝난 후 이벤트 1 및 4에서 작동합니다.

## 2단계: 재생 결합

정기적으로(선택한 전환 확인 기간에 따라 주 1회 또는 매일 한 번) CCA는 현재 인식되는 장치를 기준으로 내역 데이터를 다시 계산합니다. 장치가 인증되지 않은 상태에서 처음에 데이터를 보낸 다음 로그인하는 경우 CCA는 인증되지 않은 이벤트를 올바른 사용자에게 연결합니다. 다음 테이블은 위와 동일한 데이터를 나타내지만, 데이터 재생에 따라 다른 숫자를 표시합니다.
...
| `4` | `3579` | - | - | `Bob` | Bob이 인증되지 않은 모바일 장치에서 귀하의 사이트에 액세스함 | `1` (Bob) |
...
* CCA에서는 알려진 장치를 즉시 결합하지만 새 장치나 인식되지 않은 장치는 즉시 결합하지 않습니다.
* 데이터는 정기적으로 재생되며 식별하도록 인식된 장치에 따라 연결의 내역 데이터를 변경합니다.
