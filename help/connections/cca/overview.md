...
>[!IMPORTANT]
>
>글로벌 이벤트 데이터 세트 스키마에 대한 모든 변경 사항은 결합된 새 데이터 세트 스키마에도 적용되어야 합니다. 그렇지 않으면 결합된 데이터 세트가 손상될 수 있습니다.
>
...
* 필드 기반 결합은 대/소문자를 구분하므로 Analytics 소스 커넥터를 통해 생성된 Analytics 데이터 세트의 경우, 임시 ID 필드에 적용되는 VISTA 규칙 또는 처리 규칙을 검토하여 이러한 규칙 중 어느 하나도 동일한 ID의 새로운 양식을 도입하지 않도록 하는 것이 좋습니다. 예를 들어 VISTA 또는 처리 규칙이 이벤트의 일부에서만 임시 ID 필드에 소문자를 도입하지 않는지 확인해 보아야 합니다.
