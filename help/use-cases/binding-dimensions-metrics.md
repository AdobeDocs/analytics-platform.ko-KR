...
## 예제 1: 결합 차원을 사용하여 추가 제품 속성을 구매에 지정합니다

객체 배열 내의 차원 항목을 다른 차원에 바인딩할 수 있습니다. 바인딩된 차원 항목이 표시되면 CJA는 바인딩된 차원을 회수하여 이벤트에 포함합니다. 다음 고객 여정을 고려하십시오.

1. 방문자가 세탁기에서 제품 페이지를 봅니다.
...
1. 그런 다음 방문자는 건조기에서 제품 페이지를 봅니다.
...
1. 궁극적으로 그들은 구매를 합니다. 각 제품의 색상이 구매 이벤트에 포함되지 않았습니다.
...
결합 차원 없이 색상별로 수익을 보려면 차원입니다 `product.color` 크레딧을 건조기의 색상으로 지속하거나 잘못 표시합니다.
...
이 지속성 모델을 설정하면 Adobe은 제품 색상이 설정될 때마다 제품 이름을 기록합니다. 이 방문자의 후속 이벤트에서 동일한 제품 이름을 인식하면 제품 색상도 함께 제공됩니다. 제품 색상을 제품 이름에 바인딩할 때 동일한 데이터는 다음과 비슷합니다.
...
## 예제 2: 결합 지표를 사용하여 검색어를 제품 구매에 연결합니다
...
Adobe은 바인딩 차원을 사용하여 바인딩된 차원 항목을 메모합니다. 후속 이벤트에서 동일한 바인딩 값이 표시되면 원하는 지표를 그 값에 지정할 수 있도록 차원 항목을 가져옵니다. 이 예에서는 search_term에 대한 결합 차원을 제품 이름으로 설정할 수 있습니다. 데이터 보기 관리자에서 이 차원을 설정하면 바인딩 차원이 개체 배열에 있으므로 바인딩 지표도 설정해야 합니다. 바인딩 지표는 바인딩 차원에 대한 트리거 역할을 하므로 결합 지표가 있는 이벤트에만 바인딩합니다. 이 예제 구현에서 검색 결과 페이지에는 항상 검색어 차원과 검색 지표가 포함되어 있습니다. 검색 지표가 있을 때마다 제품 이름에 검색어를 바인딩할 수 있습니다.
