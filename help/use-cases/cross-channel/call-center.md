...
1. 원하는 [속성 모델](/help/analysis-workspace/visualizations/freeform-table/column-row-settings/column-settings.md)을 설정합니다. 예: 반감기가 15분인 시간 감소 모델 및 세션의 전환 확인 기간. 이 기여도 모델은 콜센터로 전화하기 전까지 이어지는 페이지에 크레딧을 제공합니다.
...
4. 원하는 통화 사유 차원 항목을 플로우 시각화의 가운데 위치로 드래그합니다.
5. 플로우 시각화는 이전 및 다음 통화 사유를 자동으로 표시합니다. 이전 호출 사유를 웹 사이트 페이지 차원으로 대체합니다.
