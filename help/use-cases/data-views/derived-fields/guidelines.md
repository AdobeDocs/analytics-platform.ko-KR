...
Customer Journey Analytics [파생 필드](/help/data-views/derived-fields/derived-fields.md)를 사용하면 소스 데이터 세트를 수정하지 않고 쿼리 시간에 데이터를 변환, 분류 및 보강할 수 있습니다. 이러한 유연성으로 인해 규율 없이 적용할 경우 복잡성, 성능 문제 및 유지 관리 오버헤드가 발생할 수 있습니다.
...
* [높은 카디널리티 파생 필드](#high-cardinality-derived-fields)
* [규칙 체인이 있는 경우 지나치게 복잡한 경우](#over-complex-case-when-rule-chains)
* [잘못된 사용](#wrong-usage)
* [지표 및 차원의 잘못된 분류](#misclassifications-of-metrics-and-dimensions)
* [마케팅 채널 및 캠페인 논리 위험](#marketing-channel-and-campaign-logic-pitfalls)
* [조회에 사용되는 정규화되지 않은 문자열 키](#non-normalized-string-keys-used-in-lookups)
* [정규 표현식 오용 또는 도달 범위](#regex-misuse-or-overreach)
* [파생 필드의 계산된 지표 스타일 논리](#calculated-metric-style-logic-in-derived-fields)
* [다음 또는 이전 또는 순차적 함수의 과다 사용](#over-usage-of-next-or-previous-or-sequential-functions)
* [세션 및 사용자 수준 컨텍스트 무시](#ignoring-session-and-person-level-context)
* [문서화된 기능 제한에 도달하거나 근접한 경우](#hitting-or-nearing-documented-function-limits)
* [데이터 보기별 최적화 규칙](#data-view-specific-optimization-rules)
...
* 페이지 URL에서 [소문자](/help/data-views/derived-fields/derived-fields.md#lowercase), [트리밍](/help/data-views/derived-fields/derived-fields.md#trim) 또는 [Case When](/help/data-views/derived-fields/derived-fields.md#case-when) 확인과 같은 간단한 작업은 낮은 카디널리티 필드의 동일한 논리보다 종종 비용이 더 듭니다.
...
이 섹션에서는 [Case When](/help/data-views/derived-fields/derived-fields.md#case-when) 규칙의 복잡한 체인에 대해 설명합니다.

Customer Journey Analytics은 파생 필드당 명시적 [함수 및 연산자 제한](/help/data-views/derived-fields/derived-fields.md#limitations)을(를) 적용합니다(예: 유형당 최대 연산자 수, 최대 함수 수). 함수 내에 있는 복잡한 함수와 체인은 유지하기가 더 어렵고 오류가 발생하기 쉽습니다.

**패턴**

* 복잡한 [If](/help/data-views/derived-fields/derived-fields.md#case-when) 및 **[!UICONTROL Else If]** 체인을 사용하는 매우 큰 **[!UICONTROL Case When]** 함수:
...
* 잠재적인 성능 및 제한 위험: 특히 분류와 유사한 패턴을 사용하여 [연산자 또는 함수 제한](/help/data-views/derived-fields/derived-fields.md#limitations)에 도달하거나 접근할 수 있습니다.

**추천**

* 여러 파생 필드로 분할합니다. 예를 들어, 모든 것을 하나의 큰 규칙으로 결합하는 대신 채널 버킷팅에서 *캠페인 표준화*(일치하지 않는 캠페인 식별자를 표준 값에 매핑)를 분리하십시오.
* 조회 데이터 세트를 사용합니다. 많은 **[!UICONTROL If 값 _값_ 기준 _기준_ Then _값_을(를) 값]**(으)로 설정하는 것이 긴 [Case When](/help/getting-started/cja-upgrade/cja-upgrade-dataset-lookup.md) 체인을 사용하는 대신 [Lookup](/help/data-views/derived-fields/derived-fields.md#lookup) 함수와 결합된 [조회 데이터 세트](/help/data-views/derived-fields/derived-fields.md#case-when)(으)로 더 잘 구현됩니다.
...
* 파생 필드는 구성 요소 설정에서 이미 사용 가능한 동작을 복제합니다.
   ...
   * 데이터 보기 [속성](/help/data-views/derived-fields/derived-fields.md#next-or-previous) 및 [만료](/help/data-views/component-settings/attribution.md) 설정이 충분할 경우 [다음 또는 이전](/help/data-views/component-settings/persistence.md) 또는 수동 시퀀스 논리로 코딩된 지속성 또는 속성 논리입니다.
...
* 속성 및 지속성: [다음 또는 이전](/help/data-views/component-settings/persistence.md) 또는 기타 순차적 논리를 사용하여 파생 필드에서 차원을 시뮬레이션하는 대신 차원에 대한 데이터 보기 **[!UICONTROL 지속성]** 설정(**[!UICONTROL 할당 모델]** 및 [만료](/help/data-views/derived-fields/derived-fields.md#next-or-previous))을 사용합니다.
...
>[!NOTE]
>
>업스트림 간소화를 고려해 보십시오. [데이터 준비](https://experienceleague.adobe.com/ko/docs/experience-platform/data-prep/home), [조회 데이터 세트](/help/getting-started/cja-upgrade/cja-upgrade-dataset-lookup.md) 또는 [분류](/help/data-views/derived-fields/derived-fields.md#classify)와 같은 파생 필드 함수를 사용하여 유사한 마케팅 채널 규칙을 통합하고 [Case When](/help/data-views/derived-fields/derived-fields.md#case-when) 논리의 연산자 수를 줄이십시오. 또한 채널 분류 로직에서 참조되는 높은 카디널리티 필드의 수를 제한하십시오(예: 많은 개별 쿼리 매개 변수 키). 이러한 필드는 카디널리티와 쿼리 비용을 모두 증가시킵니다.
...
* 조회 데이터 집합을 제공하는 이벤트 또는 프로필 필드에 대한 [조회](/help/data-views/derived-fields/derived-fields.md#lookup) 함수입니다.
* 키를 표준화하는 이전 [Lowercase](/help/data-views/derived-fields/derived-fields.md#lowercase), [Trim](/help/data-views/derived-fields/derived-fields.md#trim) 또는 [Regex Replace](/help/data-views/derived-fields/derived-fields.md#regex-replace)가 없습니다.
...
* 대문자나 소문자를 유지하는 이유가 문서화되어 있지 않은 한 [Lookup](/help/data-views/derived-fields/derived-fields.md#lowercase) 함수 앞에 [Lowercase](/help/data-views/derived-fields/derived-fields.md#trim) 및 [Trim](/help/data-views/derived-fields/derived-fields.md#lookup) 함수를 추가하십시오.
...
* [Regex 바꾸기](/help/data-views/derived-fields/derived-fields.md#regex-replace) 또는 regex 기반 조건에서는 광범위한 패턴을 사용합니다. [포함](/help/data-views/derived-fields/derived-fields.md#case-when) 또는 **[!UICONTROL 다음으로 시작]**&#x200B;을 사용하는 간단한 **[!UICONTROL 경우]**&#x200B;이(가) 더 나은 대안입니다.
...
* [URL 구문 분석](/help/data-views/derived-fields/derived-fields.md#url-parse) 함수를 사용하는 대신 URL을 구문 분석하는 대량 정규 표현식 사용입니다.
...
* [Regex 바꾸기](/help/data-views/derived-fields/derived-fields.md#url-parse)보다 표준 URL 요소(도메인, 경로, 쿼리 매개 변수)에 대해 [URL 구문 분석](/help/data-views/derived-fields/derived-fields.md#regex-replace)을 선호합니다.
* 간단한 패턴 검사의 경우 [Regex Replace](/help/data-views/derived-fields/derived-fields.md#case-when)를 사용하는 정규 표현식 대신 **[!UICONTROL Case When]** with **[!UICONTROL Contains]**, **[!UICONTROL Starts with]** 또는 [Ends with](/help/data-views/derived-fields/derived-fields.md#regex-replace) 논리를 사용하십시오.
...
이 단원에서는 [다음 또는 이전](/help/data-views/derived-fields/derived-fields.md#next-or-previous) 또는 순차적 함수의 과다 사용에 대해 설명합니다.

**패턴**

* 파생 필드는 [다음 또는 이전](/help/data-views/derived-fields/derived-fields.md#next-or-previous) 함수를 여러 번 사용합니다(필드당 문서화된 제한에 가까움).
* [다음 또는 이전](/help/data-views/derived-fields/derived-fields.md#next-or-previous)은(는) 데이터 보기 지속성을 사용하는 대신 지속성과 유사한 논리(예: 캠페인 전달)를 구현하는 데 사용됩니다.

**위험 진단: 데이터 품질, 높은 유지 관리**

* 복잡성 및 취약성: 무거운 순차적 논리는 추론하기 어렵고 세션화 규칙 또는 순서가 변경되는 경우 중단될 수 있습니다.
* 차원 지속성이 있는 중복성: 차원의 데이터 보기 [지속성](/help/data-views/component-settings/persistence.md) 설정(할당 모델)에서 일부 사용 사례(예: 세션의 마지막 터치 채널)를 더 잘 다룹니다.

**추천**

* 표준 지속성과 유사한 패턴(예: 세션 또는 사용자 간에 값을 전달)의 경우, [다음 또는 이전](/help/data-views/component-settings/persistence.md)으로 이러한 패턴을 시뮬레이션하는 대신 데이터 보기에서 차원의 **[!UICONTROL 지속성]** 설정(**[!UICONTROL 할당 모델]** 및 [만료](/help/data-views/derived-fields/derived-fields.md#next-or-previous))을 사용하십시오.
* 차원 지속성만으로는 달성할 수 없는 고급 다중 단계 경로 또는 funnel 레이블에 대해 [다음 또는 이전](/help/data-views/derived-fields/derived-fields.md#next-or-previous)을(를) 예약합니다(예: 채널 시퀀스 연결).
...
>[!NOTE]
>
>가능한 경우 복잡한 파생 필드 내에서 카디널리티가 높은 필드에 대한 의존도를 줄입니다(예: 정규화된 키 또는 그룹화된 분류 사용). 쿼리 비용과 [연산자 또는 함수 제한에 도달할 가능성](/help/data-views/derived-fields/derived-fields.md#limitations)을 제한하십시오.

CustoCustomer 여정 분석 [문서](/help/data-views/derived-fields/derived-fields.md#limitations) 함수 유형별 제한을 포함하여 파생 필드당 최대 함수 및 연산자입니다.**

* 파생 필드는 많은 [조회](/help/data-views/derived-fields/derived-fields.md#lookup), [계산](/help/data-views/derived-fields/derived-fields.md#math) 작업, [분할](/help/data-views/derived-fields/derived-fields.md#split) 또는 기타 함수를 사용합니다.
* 연산자 수가 [문서화된 제한](/help/data-views/derived-fields/derived-fields.md#limitations)에 가깝습니다(예: 허용된 수의 70% 이상 - 80%).
