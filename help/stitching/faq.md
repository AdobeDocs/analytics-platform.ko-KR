...
**질문**: 사람들이 한 채널에서 다른 채널로 이동하는 방법을 확인하기 위해 결합을 어떻게 사용할 수 있습니까?
...
**질문**: 재생 프로세스는 정확히 언제 실행됩니까?

+++답변

* **주별** 재생은 각 **토요일**&#x200B;밤(고객 시간대)에 시작되며, 데이터는 월요일 아침까지 Customer Journey Analytics 보고에서 업데이트됩니다.
* **일별** 재생은 오전 **3시(고객 시간대)에 실행되며 데이터는 아침까지 Customer Journey Analytics 보고에서 업데이트됩니다.**

>[!IMPORTANT]
>
>주간 재생 요일 및 일간 재생 시간은 사용자 지정 요청을 통해 변경할 수 없습니다.
>
...
**질문**: 결합은 한 사람이 많은 영구 ID를 가진 상황을 어떻게 처리합니까?
...
**질문**: 원하는 정보를 사용하여 Adobe 계정 팀에 연락하면 다시 입력된 데이터 세트를 사용할 수 있는 데 시간이 얼마나 걸립니까?
...
**질문**: 크로스 디바이스 분석(기존 Analytics의 기능)과 크로스 채널 분석의 차이점은 무엇입니까?
...
| 결합된 데이터 세트 | ID | 타임스탬프 | 영구 ID | 영구 네임스페이스 | 개인 ID | 개인 네임스페이스 | 결과 ID | 결합된 네임스페이스 |
|---|---|---|---|---|---|---|---|---|
...
**질문**: 하나 이상의 이벤트에 있는 영구 ID 필드가 비어 있으면 어떻게 됩니까?

+++답변

결합되는 데이터 세트의 이벤트에서 영구 ID 필드가 비어 있는 경우, 해당 이벤트에 대한 결과 ID는 다음 두 가지 방법 중 하나로 결정됩니다.

* 개인 ID 필드가 비어 있지 않으면 Customer Journey Analytics에서는 개인 ID 값을 결과 ID로 사용합니다.
* 개인 ID 필드가 비어 있으면 Customer Journey Analytics도 결과 ID를 비워 둡니다. 이 경우 이벤트에서 영구 ID, 개인 ID 및 결과 ID는 모두 비어 있습니다. 이러한 유형의 이벤트는 결과 ID가 개인 ID로 선택된 위치에서 결합되는 데이터 세트를 사용하여 모든 Customer Journey Analytics 연결에서 삭제됩니다.
...
**질문**: 하나 이상의 이벤트에 있는 개인 ID 필드에 `Undefined`과(와) 같은 자리 표시자 값이 있는 경우 어떻게 됩니까?
...
| 이벤트 | 타임스탬프 | 영구 ID(쿠키 ID) | 임시 ID | 결과 ID(재생 후) |
|---|---|---|---|---|
...
**질문**: Customer Journey Analytics에 결합된 데이터 세트의 지표를 Customer Journey Analytics에 결합되지 않은 데이터 세트의 유사한 지표 및 Adobe Analytics과 어떻게 비교합니까?
...
**질문**: Customer Journey Analytics에서 ID 맵 필드를 사용할 수 있습니까?
...
**질문**: 필드 기반 결합에서 그래프 기반 결합으로 전환하려면 데이터를 다시 수집해야 합니까?
...
**질문**: ID 서비스에 대해서만 데이터 세트를 활성화하는 방법
...
기존 데이터 세트를 확인하고 ID 서비스에 대해서만 데이터 세트를 활성화하려면 `unifiedIdentity` 태그만 사용하는 `/datasets` 끝점에 대한 `PATCH` 요청을 사용합니다. 예:
...
**질문**: 결합된 네임스페이스 값이 CJA 연결 내의 다른 데이터 세트 내에서 사용할 수 있는 ID 네임스페이스 값과 항상 일치하지 않는 이유는 무엇입니까?
