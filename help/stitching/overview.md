...
>[!IMPORTANT]
>
>- 사용에 대한 지원 없음 `identityMap` 영구 ID로. 데이터 세트에 특정 식별자를 정의해야 합니다(예: `ECID`)를 영구 ID로 사용하십시오.
>
...
>
>- 데이터 사용 레이블은 결합된 데이터 세트 스키마에 자동으로 전파되지 않습니다. 소스 데이터 세트 스키마에 데이터 사용 레이블이 적용된 경우 이러한 데이터 사용 레이블을 결합된 데이터 세트 스키마에 수동으로 적용해야 합니다. 다음을 참조하십시오 [Experience Platform에서 데이터 사용 레이블 관리](https://experienceleague.adobe.com/en/docs/experience-platform/data-governance/labels/overview) 추가 정보.
...
해당 데이터 세트에 대한 영구 ID(쿠키)와 임시 ID(개인 ID)뿐만 아니라 이벤트 데이터 세트를 지정합니다. 필드 기반 결합은 결합된 새 데이터 세트에 새 결합된 ID 열을 만들고 특정 영구 ID에 대한 임시 ID가 있는 행을 기반으로 이 결합된 ID 열을 업데이트합니다. <br/>Customer Journey Analytics을 독립형 솔루션으로 사용할 때(Experience Platform ID 서비스 및 관련 ID 그래프에 대한 액세스 권한이 없음) 필드 기반 결합을 사용할 수 있습니다. 또는 사용 가능한 ID 그래프를 사용하지 않으려는 경우입니다.
...
- **재생 결합**: 학습한 고유 식별자(임시 ID)를 기반으로 데이터를 &quot;재생&quot;합니다. 이 단계에서 이전에 알 수 없었던 장치(영구 ID)의 히트가 임시 ID에 결합됩니다. Adobe에서는 두 개의 재생 간격을 제공합니다.
   ...
   - **매주**: 선택한 전환 확인 기간을 사용하여 일주일에 한 번 데이터를 재생합니다(참조) [옵션](#options)). 이 옵션은 인증되지 않은 세션을 인증하는 데 보다 관대한 시간을 주는 이점이 있습니다. 그러나 1주일 미만의 연결되지 않은 데이터는 다음 주간 재생까지 재처리되지 않습니다.

- **개인 정보 보호**: 개인 정보 보호 관련 요청이 수신되면 요청된 ID를 제거하는 것 외에도 인증되지 않은 이벤트에서 해당 ID의 결합을 실행 취소해야 합니다.
...
| | | **장치 3개** | | **4명**:<br/>246, `Bob`, `3579`, `81911` | **2명**:<br/>밥 `3579` |  | **3명**:<br/>`246`, `3579`, `81911` |
...
- **라이브 결합**: 각 히트(이벤트)를 들어오는 대로 연결하려고 하며, 영구 ID를 사용하여 id 그래프를 쿼리하여 선택한 네임스페이스에 대한 임시 ID를 조회합니다. 조회에서 임시 ID를 사용할 수 있는 경우 이 임시 ID가 즉시 결합됩니다.

- **재생 결합**: id 그래프에서 업데이트된 id를 기반으로 데이터를 &quot;재생&quot;합니다. 이 단계에서 ID 그래프가 네임스페이스에 대한 ID를 해결함으로써 이전에 알 수 없었던 디바이스(영구 ID)의 히트가 결합됩니다. Adobe에서는 두 개의 재생 간격을 제공합니다.
   - **매일**: 데이터는 매일 24시간 전환 확인 기간으로 재생됩니다. 이 옵션은 재생이 훨씬 빈번한 이점이 있지만 인증되지 않은 방문자는 사이트를 방문하는 당일 인증해야 합니다.
   - **매주**: 전환 확인 기간을 사용하여 일주일에 한 번 데이터를 재생합니다(참조) [옵션](#options)). 이 옵션은 인증되지 않은 세션을 인증하는 데 보다 관대한 시간을 주는 이점이 있습니다. 그러나 1주일 미만의 연결되지 않은 데이터는 다음 주간 재생까지 재처리되지 않습니다.

- **개인 정보 보호**: 개인 정보 보호 관련 요청이 수신되면 소스 데이터 세트에서 요청된 ID를 제거하는 것 외에도 인증되지 않은 이벤트에서 해당 ID의 모든 결합을 실행 취소해야 합니다. 또한 특정 ID에 대한 향후 그래프 기반 결합을 방지하려면 ID 그래프에서 ID를 제거해야 합니다.

조회 창을 벗어난 데이터는 재생되지 않습니다. 방문자는 인증되지 않은 방문과 인증된 방문을 함께 식별하기 위해 주어진 전환 확인 기간 내에서 인증해야 합니다. 디바이스가 인식되면 해당 시점부터 라이브로 결합됩니다.

영구 ID에 대해서는 다음 두 가지 ID 그래프를 고려하십시오 `246` 및 `3579`, 이러한 id 그래프가 시간에 따라 업데이트되는 방법 및 이러한 업데이트가 그래프 기반 결합의 단계에 미치는 영향을 설명합니다.

![ID 그래프 246](assets/identity-graph-246.svg)
![ID 그래프 3579](assets/identity-graph-3579.svg)

다음을 사용하여 특정 프로필에 대한 시간 경과에 따른 ID 그래프를 볼 수 있습니다. [ID 그래프 뷰어](https://experienceleague.adobe.com/en/docs/experience-platform/identity/features/identity-graph-viewer). 참조: [ID 서비스 연결 논리](https://experienceleague.adobe.com/en/docs/experience-platform/identity/features/identity-linking-logic) id를 연결할 때 사용되는 논리를 더 잘 이해할 수 있습니다.
...
조회가 두 개 이상의 결합된 ID로 확인되는 경우(이벤트 7의 경우 등) ID 그래프에서 반환되는 사전 편집된 첫 번째 ID가 선택됩니다(`a.b@yahoo.co.uk` 예제에서).
...
2023-05-13 16:30에 발생하고 24시간 전환 확인 기간 구성이 있는 재생 결합이 있는 경우 샘플의 일부 이벤트가 다시 결합됩니다( 로 표시됨). ![재생](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Replay_18_N.svg)).

| | 시간 | 영구 ID<br/>`ECID` | 네임스페이스<br/>`Email` ![그래프](https://spectrum.adobe.com/static/icons/workflow_18/Smock_DataMapping_18_N.svg) | 결합된 ID<br/>(라이브 결합 후) | 결합된 ID<br/>(24시간 재생 후) |
|---|---|---|---|---|---|
...
| ![재생](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Replay_18_N.svg) 7 | 2023-05-13 16:30 | `246` | `246` ![링크](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Branch1_18_N.svg) `a.b@yahoo.co.uk`<br/>`246` ![링크](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Branch1_18_N.svg) `bob.ab@gmail.com` | `bob.a@gmail.com` | `a.b@yahoo.co.uk` |
...
| | 시간 | 영구 ID<br/>`ECID` | 네임스페이스<br/>`Email` ![그래프](https://spectrum.adobe.com/static/icons/workflow_18/Smock_DataMapping_18_N.svg) | 결합된 ID<br/>(라이브 결합 후) | 결합된 ID<br/>(재생 7일 후) |
|---|---|---|---|---|---|
...
- 다음 [ID 서비스 보호 기능](https://experienceleague.adobe.com/en/docs/experience-platform/identity/guardrails) 적용합니다. 예를 들어 다음을 참조하십시오 [정적 제한](https://experienceleague.adobe.com/en/docs/experience-platform/identity/guardrails#static-limits):
   - 그래프의 최대 ID 수: 50.
...
1. 다음과 관련된 질문이 있는 경우 Adobe 고객 지원 센터에 문의하십시오.
   ...
   - 전환 확인 기간 및 재생 빈도에 대한 사용자 기본 설정입니다. 에 대한 Customer Journey Analytics 패키지 보기 [옵션](#options) 사용 가능.
   - 샌드박스 이름
...
3. 처음 켜면 Adobe에서 결합된 데이터의 채우기를 제공합니다. 에 대한 Customer Journey Analytics 패키지 보기 [옵션](#options) 사용 가능.

4. 크로스 채널 분석에서 결합된 새 데이터 세트를 사용하려면 결합된 새 데이터 세트를 [연결](../connections/overview.md) Customer Journey Analytics. 그런 다음 크로스 채널 분석에 필요한 다른 데이터 세트를 추가하고 각 데이터 세트에 대한 올바른 개인 ID를 선택합니다.
