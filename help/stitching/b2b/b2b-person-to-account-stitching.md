...
>[!IMPORTANT]
>
>연결이 저장되면 B2B 개인 대 계정 연결 구성을 변경할 수 없습니다. 저장한 후 설정을 보려면 **B2B 결합 구성 열기**&#x200B;를 선택합니다. 모든 필드는 읽기 전용 상태로 표시됩니다. 또한 Experience Platform에서 [개인-계정 간 매핑](#prerequisites)에 사용된 데이터 세트가 삭제되면 결합 구성이 삭제되고 연결이 잘못된 상태로 전환되어 사용자 인터페이스에 경고 메시지가 표시됩니다.
