...
>title="주석"
>abstract="주석은 구성 요소 관리자 또는 작업 영역 프로젝트 내부에서 만들 수 있습니다."
...
데모 비디오를 보려면 ![VideoCheckedOut](/help/assets/icons/VideoCheckedOut.svg) [모바일 스코어카드 만들기](https://experienceleague.adobe.com/ko/docs/customer-journey-analytics-learn/tutorials/dashboards/create-a-mobile-scorecard){target="_blank"}를 확인하십시오.
...
Analytics 스코어카드는 아래와 같이 타일 레이아웃으로 경영진 사용자를 위한 주요 데이터 시각화를 표시합니다.

![모바일 스코어카드 데모를 보여 주는 Analytics 스코어카드 예시](assets/intro_scorecard.png)

이 스코어카드의 큐레이터로서 스코어카드 빌더를 사용하여 경영진 소비자를 위해 스코어카드에 표시할 타일을 구성할 수 있습니다. 또한 타일을 탭한 후 세부 보기 또는 분류를 조정하는 방법을 구성할 수 있습니다. 스코어카드 빌더 인터페이스가 아래에 표시되어 있습니다.
...
스코어카드의 모든 타일에 세그먼트를 적용하려면 타일을 스코어카드 위에 놓습니다. 또는 날짜 범위 아래의 세그먼트 메뉴에서 세그먼트를 선택하여 세그먼트를 적용할 수도 있습니다. Customer Journey Analytics Workspace에서 수행한 것과 동일한 방식으로 [스코어카드에 세그먼트를 구성하고 적용](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/analysis-workspace/using-panels/using-drop-down-filters.html?lang=ko-KR)합니다.
...
비교 날짜를 만들려면 드롭다운 메뉴에서 일반적인 시간 비교를 위한 편리한 사전 설정을 선택할 수 있습니다. 오른쪽 레일에서 날짜 구성 요소를 드래그 앤 드롭할 수도 있습니다.
...
이 설정은 기본적으로 *켜져 있습니다*. 비교 날짜를 보지 않으려면 *끄기*&#x200B;로 토글하십시오.

![어제를 강조 표시하고 비교 날짜를 포함한 새로운 모바일 스코어카드](assets/no-comparison-dates.png)

## 시각화 적용 {#viz}

Analytics 대시보드는 차원 항목 및 지표에 대한 뛰어난 인사이트를 제공하는 네 가지 시각화를 제공합니다. 타일 [!UICONTROL 속성]의 [!UICONTROL 차트 유형]을 변경하여 다른 시각화로 변경할 수 있습니다. 원하는 타일을 선택한 다음 차트 유형을 변경하기만 하면 됩니다.
...
라인 시각화는 일정 기간 동안 값이 어떻게 변하는지를 보여 주기 위해 라인을 사용하여 지표를 나타냅니다. 선 그래프는 시간 경과에 따른 차원을 표시하지만 모든 시각화에서 작동합니다. 이 예에서는 제품 카테고리 차원을 시각화하고 있습니다.
...
데모 비디오를 보려면 ![VideoCheckedOut](/help/assets/icons/VideoCheckedOut.svg) [모바일 스코어카드 프로젝트의 데이터 스토리](https://experienceleague.adobe.com/ko/docs/customer-journey-analytics-learn/tutorials/dashboards/create-a-mobile-scorecard){target="_blank"}를 참조하십시오.
