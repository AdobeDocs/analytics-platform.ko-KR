...
다음 정보는 Adobe Analytics 큐레이터에게 경영진 사용자의 대시보드를 구성 및 제공하는 방법에 대해 설명합니다. 시작하는 데 도움이 되는 “Adobe Analytics 대시보드 스코어카드 빌더” 비디오를 시청하십시오.
...
데이터 스토리는 핵심 테마 또는 지표를 기반으로 구축된 지원 데이터 포인트, 비즈니스 컨텍스트 및 관련 지표의 컬렉션입니다.

예를 들어 웹 트래픽에 중점을 두는 경우 가장 중요한 지표는 방문일 수 있지만 새 방문자, 고유 방문자 수에 관심이 있을 수 있으며 웹 페이지나 트래픽이 발생하는 디바이스 유형별로 분류된 데이터를 보고 싶을 수 있습니다. 모바일 스코어카드 프로젝트의 데이터 스토리를 사용하면 여러 세부 슬라이드를 통해 지표 뒤의 전체 스토리를 전달하면서 가장 중요한 지표를 맨 앞과 가운데에 배치할 수 있습니다.
...
새 슬라이드를 추가하려면 슬라이드를 마우스로 가리키거나 클릭한 다음 사용 가능한 옵션 중에서 선택합니다.
   ...
   * 중복 아이콘을 탭하여 기존 슬라이드를 복제합니다.
1. 빈 슬라이드를 만드는 경우 왼쪽 레일에서 구성 요소를 드래그 앤 드롭하거나 레이아웃을 선택하여 타일의 데이터로 슬라이드를 자동으로 채웁니다.
   ![데이터 스토리 만들기](assets/data-story2.png)
슬라이드를 삭제하려면 휴지통 아이콘을 탭합니다.

### 데이터 스토리 사용자 지정 {#customize-data-story}

데이터 스토리를 사용하면 모든 것을 사용자 지정할 수 있으므로 공유하려는 정보를 공유하고 필요하지 않은 모든 것을 제외할 수 있습니다. 타일과 개별 슬라이드를 사용자 정의하여 필터를 추가하고, 분류를 표시하고, 레이아웃을 변경하고, 시각화를 변경할 수 있습니다.

**타일 사용자 지정하기**

1. 타일을 탭합니다. 선택한 타일은 파란색으로 윤곽이 표시되고 오른쪽 패널에 [타일] 속성이 표시됩니다.
1. 제목, 차트 유형 및 기타 타일 옵션을 변경합니다.
1. 구성 요소를 타일로 드래그합니다.
   ![데이터 스토리 만들기](assets/data-story3.png)
시각화와 같은 구성 요소를 타일에 끌어다 놓으면 구성 요소가 모든 데이터 스토리 슬라이드에 적용됩니다.
1. 제목에만 변경 내용을 적용하려면 Shift 키를 누른 상태에서 변경 내용을 적용합니다.
...
데이터 스토리의 개별 슬라이드에 대한 시각화를 변경할 수 있습니다. 예를 들어 특정 슬라이드에 대해 가로 막대를 도넛 그래프로 변경할 수 있습니다. 레이아웃을 변경할 수도 있습니다. 다음을 참조하십시오 [세부 슬라이드 사용자 정의](#customize-detail-slide).

### 데이터 스토리 미리보기 {#preview-data-story}

데이터 스토리를 만든 후 **미리 보기** 마치 앱 사용자인 것처럼 데이터 스토리를 보고 상호 작용하는 단추입니다. 데이터 스토리 미리 보기에 대한 자세한 내용은 [스코어카드 미리보기](#preview)

### 타일과 슬라이드 간 탐색 {#navigate-tiles-slides}

탐색 모음에는 각 슬라이드의 내용을 나타내는 아이콘이 표시됩니다. 탐색 모음을 사용하면 슬라이드가 많을 경우 특정 슬라이드로 쉽게 이동할 수 있습니다.

타일과 슬라이드 사이를 이동하려면 탐색 모음을 탭합니다.
![데이터 스토리 만들기](assets/data-story5.png)
![데이터 스토리 만들기](assets/data-story-nav.png){width="45%"}

키보드의 화살표를 사용하거나 구성 요소를 선택하고 스크롤하기 위해 화면의 왼쪽이나 오른쪽으로 잡아 앞뒤로 이동할 수도 있습니다.
