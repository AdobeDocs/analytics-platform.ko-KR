...
1. **[!UICONTROL 속성]** (오른쪽 레일)에서 데이터를 사용할 **[!UICONTROL 프로젝트 보고서 세트]**&#x200B;를 지정합니다. 보고서 세트는 CJA의 데이터 보기입니다.
...
필터를 개별 타일에 적용하려면 왼쪽 패널에서 필터(세그먼트는 CJA의 필터)를 끌어서 타일 위에 직접 놓습니다.
...
파이 차트와 유사하게 이 시각화는 데이터를 전체의 일부로 표시합니다. 합계의 백분율을 비교할 때 도넛 그래프를 사용하십시오. 예를 들어 총 고유 방문자 수에 기여한 광고 플랫폼을 확인하려고 한다고 가정해 보겠습니다.
...
스코어카드 빌더에서 타일을 클릭하면 오른쪽 레일에 해당 타일과 연관된 속성 및 특성과 세부 슬라이드가 표시됩니다. 이 레일에서 새 항목을 제공할 수 있습니다 **제목** 또는 필터를 적용하여 타일을 구성합니다. 세그먼트는 CJA의 필터입니다.

![속성 타일](assets/properties-tile-new.png)

## 세부 사항 슬라이드 보기 {#view-detail-slides}

타일을 클릭하면 앱에서 세부 사항 슬라이드가 경영진 사용자에게 표시되는 방법이 동적 팝업 창에 표시됩니다. 차원을 추가하여 특정 요구에 대한 데이터를 분류할 수 있습니다. 차원이 적용되지 않은 경우 분류 차원이 **시간** 또는 **일**&#x200B;기본 날짜 범위에 따라 다릅니다.

분류는 다음과 같은 차원 항목별로 지표를 분류하여 분석을 세분화합니다.
...
## 세부 사항 슬라이드 사용자 지정 {#customize-detail-slide}

사용자 지정 세부 사항 슬라이드를 사용하면 대상과 공유하는 정보에 대해 보다 타겟팅할 수 있습니다. 각 세부 슬라이드에 대한 레이아웃을 수정하고 텍스트를 추가하여 최종 사용자가 데이터에서 볼 수 있는 내용을 더 잘 설명할 수 있습니다. 드롭다운 메뉴를 사용하여 차트 유형을 변경할 수도 있습니다.

![사용자 정의 세부 정보 슬라이드](assets/custom-detail-slide.png)

### 슬라이드 레이아웃 변경

가장 중요한 정보에 초점을 맞추도록 슬라이드 레이아웃을 변경합니다. 예를 들어 레이아웃을 변경하여 차트나 표만 표시할 수 있습니다. 슬라이드 레이아웃을 변경하려면 미리 디자인된 형식 중 하나를 선택합니다.

![슬라이드 레이아웃](assets/layout.png)

왼쪽 레일에서 시각화 구성 요소를 캔버스로 끌어다 놓아 슬라이드 레이아웃을 변경할 수도 있습니다. 각 세부 사항 슬라이드는 한 번에 두 개의 시각화만 수용할 수 있습니다.

![슬라이드 레이아웃 변경](assets/slide-layout-change.png)

### 슬라이드에 설명 텍스트 추가

텍스트를 추가하여 차트에 포함된 내용이나 데이터에 대한 뉘앙스에 대한 의미 있는 정보를 제공할 수 있습니다.

세부 사항 슬라이드에 텍스트를 추가하려면 `T` 기호를 클릭하거나 텍스트 시각화 구성 요소를 왼쪽 레일에서 드래그하여 놓습니다. 새 텍스트 시각화를 추가하거나 텍스트가 있는 슬라이드 레이아웃을 선택할 때 텍스트 편집기가 자동으로 열립니다. 텍스트 편집기는 텍스트 서식을 위한 모든 표준 옵션을 제공합니다. 단락, 머리글 및 하위 헤딩과 같은 텍스트 스타일을 적용하고 굵게 및 기울임꼴로 된 글꼴을 적용할 수 있습니다. 텍스트를 정당화하고, 글머리 기호 및 번호 목록을 추가하고, 링크를 추가할 수 있습니다. 편집을 마치면 텍스트 편집기의 오른쪽 위 모서리에서 최소화 단추를 선택하여 닫습니다. 이미 추가한 텍스트를 편집하려면 연필 아이콘을 선택하여 텍스트 편집기를 다시 엽니다.

![슬라이드 레이아웃 변경](assets/add-descriptive-text.png)

## 구성 요소 제거 {#remove}

마찬가지로 전체 스코어카드에 적용된 구성 요소를 제거하려면 타일 바깥쪽에 있는 스코어카드의 아무 곳이나 클릭한 다음 을 클릭하여 제거합니다 **x** 구성 요소를 마우스로 가리키면 다음과 같이 표시됩니다. **최초 방문**:
