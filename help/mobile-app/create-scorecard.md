...
![차원 목록에서 스코어카드 창으로 향하는 화살표가 있는 새 모바일 스코어카드 창](assets/layer_dimensions.png)

## 세그먼트 적용 {#segments}

세그먼트를 개별 타일에 적용하려면 왼쪽 패널의 세그먼트를 타일 위에 바로 드롭합니다.

스코어카드의 모든 타일에 세그먼트를 적용하려면 스코어카드의 맨 위에 타일을 놓습니다. 또는 날짜 범위 아래의 세그먼트 메뉴에서 세그먼트를 선택하여 적용할 수도 있습니다. Customer Journey Analytics Workspace에서와 동일한 방식으로 [스코어카드에 세그먼트를 구성하고 적용](https://experienceleague.adobe.com/docs/analytics-learn/tutorials/analysis-workspace/using-panels/using-drop-down-filters.html?lang=ko-KR)합니다.

![빌드 세그먼트를 강조 표시하는 세그먼트 드롭다운 선택기](assets/segment_ui.png)

## 날짜 범위 추가 {#dates}

날짜 범위 드롭다운을 선택하여 스코어카드에서 선택할 수 있는 날짜 범위 조합을 추가 및 제거합니다.

![새 모바일 스코어카드가 어제와 지난 주 같은 날을 강조 표시](assets/new_score_card.png)
...
![비교 날짜가 지난 주 같은 요일로 설정된 날짜 범위를 강조 표시하는 새 모바일 스코어카드](assets/new_score_card4.png)
...
![어제와 전날이 강조 표시된 새 모바일 스코어카드 및 비교 날짜 포함](assets/include-comparison-dates.png)

이 설정은 기본적으로 *켜져 있습니다*. 비교 날짜를 보지 않으려면 *끄기*&#x200B;로 전환하십시오.

![어제 강조 표시된 새 모바일 스코어카드 및 비교 날짜 포함](assets/no-comparison-dates.png)
...
![13.3K 방문을 강조 표시하는 요약 번호 시각화가 있는 새 모바일 스코어카드](assets/summary-number.png)
...
![선 시각화를 표시하는 새 모바일 스코어카드](assets/line.png)
...
1. 오른쪽 레일의 **[!UICONTROL 드릴인]** 아래에서 제거하려는 **[!UICONTROL 지정되지 않음]** 항목이 있는 차원 항목 옆의 오른쪽 화살표를 선택합니다.
   ...
   ![차원 이름 옆의 오른쪽 화살표를 가리키는 화살표가 있는 속성](assets/unspecified.png)
...
마찬가지로 전체 스코어카드에 적용된 구성 요소를 제거하려면 타일 바깥쪽에 있는 스코어카드의 아무 곳이나 클릭한 다음 **첫 번째 방문**&#x200B;에 대해 아래에 표시된 대로 구성 요소를 마우스로 가리키면 표시되는 **x**&#x200B;을(를) 클릭하여 제거합니다.
...
예를 들어 웹 트래픽에 중점을 두는 경우 가장 중요한 지표는 방문일 수 있지만 새로운 사용자, 고유 사용자에 관심이 있을 수 있으며, 웹 페이지나 트래픽이 발생하는 디바이스 유형별로 분류된 데이터를 보고 싶을 수 있습니다. 모바일 스코어카드 프로젝트에서 데이터 스토리를 사용하면 가장 중요한 지표를 전면 중앙에 배치하는 동시에 여러 세부 슬라이드로 지표에 담겨 있는 전체 스토리를 전달할 수 있습니다.
