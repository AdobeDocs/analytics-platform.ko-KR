...
description: Customer Journey Analytics 에서 액세스 제어를 구현 하는 방법에 대해 알아보십시오.
...
세 가지 액세스 수준 또는 세 가지 역할(제품 관리자 역할, 제품 프로필 관리자 역할 및 사용자 수준 액세스)이 Customer Journey Analytics 제어합니다. 여기서는 이 역할에 대해 자세하게 설명합니다.

또한 이 문서에서는 작업 영역 큐레이션 및 행 수준 및 값 수준 액세스 제어와 같이 액세스를 제한하는 보다 세분화된 방법에 대해 설명합니다.
...
1. Admin Console[로 &#x200B;](https://adminconsole.adobe.com/enterprise/)이동합니다.

1. Customer Journey Analytics > 관리자 탭 관리자&#x200B;[!UICONTROL **추가>**]&#x200B;선택합니다&#x200B;[!UICONTROL **.**] [!UICONTROL **&#x200B;**]

   추가한 사용자에게는 제품 관리자 기본 권한이[&#x200B; 부여](#product-admin-default-permissions)됩니다. 필요한 경우 추가 권한을[&#x200B; 부여](#product-admin-additional-permissions)할 수도 있습니다.
...
* 다른 사용자가 만든 프로젝트, 세그먼트, 계산된 지표, 대상, 주석 또는 세그먼트를 업데이트하고 삭제합니다
* 모든 사용자에게 작업 영역 프로젝트 공유
* 보고 활동 관리자에서 [보고 활동 관리](/help/reporting-activity-manager/reporting-activity-overview.md)
* [Analysis Workspace에서 전체 테이블](/help/analysis-workspace/export/export-cloud.md) 내보내기
...
* [데이터 보기](/help/data-views/data-views.md)를 만들고 업데이트하고 삭제합니다.
* [연결](/help/connections/overview.md) 만들기, 업데이트 및 삭제
  ...
  | 카테고리 | 사용 권한 | 설명 |
  |---|---|---|
  | [!UICONTROL 샌드박스] | [!UICONTROL 하나 이상] | 연결을 위한 관련 샌드박스에 대한 액세스 권한. |
  | [!UICONTROL 데이터 모델링] | [!UICONTROL 스키마 보기] | 스키마 및 관련 리소스에 대한 읽기 전용 액세스 권한Read-only access to schemas and related resources. |
...
* Journey Optimizer가 Journey Optimizer 연결이 있는 Customer Journey Analytics 와 통합된 경우 Connections에 액세스하려면 Journeys 솔루션 권한도 추가해야 합니다.
  ...
  | 카테고리 | 사용 권한 | 설명 |
  |---|---|---|
  | [!UICONTROL 여정] | [!UICONTROL Journeys 과정 이벤트, 데이터 소스 및 작업 보기] | 여정 이벤트, 여정 사용자 지정 작업 및 여정 데이터 소스에 대한 읽기 전용 액세스 권한입니다. |
  | [!UICONTROL 여정] | [!UICONTROL 움직임 프로세스, 이벤트, 데이터 소스 및 작업 관리] | 이벤트, 소스 또는 작업을 읽고, 만들고, 편집하고, 삭제합니다. |
  | [!UICONTROL 여정] | [!UICONTROL 여정 보기] | 여정에 대한 읽기 전용 액세스 권한입니다. |
  | [!UICONTROL 여정] | [!UICONTROL 여정 관리] | 여정을 읽고, 만들고, 편집하고, 삭제합니다. |

* 대상으로 데이터 세트 [내보내기](https://experienceleague.adobe.com/ko/docs/experience-platform/destinations/ui/activate/export-datasets)
...
* [BI 확장 사용](../data-views/bi-extension.md)
   ...
   * 은(는) 사용자에게 적절한 Customer Journey Analytics 권한이 있는지 확인해야 합니다.
      * 관련 데이터 보기에 액세스할 수 있는 권한. 사용자 수준 액세스[!UICONTROL 의 &#x200B;]데이터 보기를[&#x200B; 참조하십시오](#user-level-access).
      * Customer Journey Analytics BI 확장에 액세스할 수 있는 권한. 사용자 수준 액세스의 [!UICONTROL 데이터 보기 도구]를 참조하십시오[.](#user-level-access)

### 제품 프로필 관리자 역할

제품 프로필은 권한 그룹입니다. 제품 관리자는 제품 프로필을 만들고 제품 프로필 관리자를 할당하여 하나 이상의 제품 프로필을 관리 할 수 있습니다. 제품 프로필 관리자는 다음을 수행할 수 있습니다.

* 할당된 제품 프로필을 관리합니다. 예: 사용자 또는 사용자 그룹 추가 또는 제거, 제품 프로필에 대한 권한 수정.

* Customer Journey Analytics에서 할당된 제품 프로필의 일부인 데이터 보기를 편집합니다. 제품 프로필 관리자는 새 데이터 보기를 만들 수 없습니다.

### 사용자 수준 액세스

아래 표에는 관련 사용자에 대해 구성할 수 있는 다양한 Customer Journey Analytics 기능에 대한 기본 액세스 권한이 요약되어 있습니다. 제품 프로필을 통해 다양한 수준의 사용자 액세스를 관리 할 수 있습니다. 제품 프로필은 여러 권한을 결합하여 개별 사용자 또는 사용자 그룹에 할당할 수 있습니다.
...
| [!UICONTROL 보고 도구] | [!UICONTROL Analysis Workspace 액세스] | 사용자가 Analysis Workspace Analysis Workspace에[&#x200B; 액세스할 &#x200B;](/help/analysis-workspace/home.md)수 있도록 합니다. |
| [!UICONTROL 보고 도구] | [!UICONTROL 분석 액세스 안내] | 사용자가 안내식 분석[에 액세스할 수 &#x200B;](/help/guided-analysis/overview.md)있도록 합니다. |
| [!UICONTROL 보고 도구] | [!UICONTROL 계산된 지표 만들기] | 사용자가 계산된 지표를 만들 [수 있도록 합니다](/help/components/calc-metrics/calc-metr-overview.md). 사용자는 자신이 만든 계산된 지표 또는 자신과 공유된 계산된 지표만 태그, 공유, 삭제, 이름 변경, 승인, 승인 취소할 수 있습니다. |
| [!UICONTROL 보고 도구] | [!UICONTROL 세그먼트 만들기] | 사용자가 세그먼트를 만들 [수 있도록 합니다](/help/components/segments/seg-overview.md). 사용자는 자신이 만든 세그먼트 또는 자신과 공유된 세그먼트만 태그, 공유, 삭제, 이름 변경, 승인, 승인 취소할 수 있습니다. |
| [!UICONTROL 보고 도구] | [!UICONTROL Labs 액세스] | 사용자가 Customer Journey Analytics 에서 Labs[&#x200B; 탭 액세스할 &#x200B;](/help/labs/labs.md)수 있도록 합니다. |
...
| [!UICONTROL 데이터 보기 도구] | [!UICONTROL 전체 테이블 내보내기] | 사용자가 [전체 테이블을 클라우드](/help/analysis-workspace/export/export-cloud.md) 내보내도록 합니다. |
...
Customer Journey Analytics에서는 기존의 Adobe Analytics에서와 같이 개별 지표 또는 차원에 대한 권한을 부여하거나 거부할 수 없습니다. 지표 및 차원은 데이터 보기[에서 &#x200B;](/help/data-views/data-views.md)수정할 수 있으므로 Customer Journey Analytics 에서 변경될 수 있습니다. 소급하여 변경하면 보고도 변경됩니다.
...
귀사가 근무하는 서드파티의 팀장에게 제품 프로필 관리 액세스 권한을 제공할 수 있습니다. 이 관리자는 회사 팀의 사용자를 이 제품 프로필에 추가할 수 있습니다. 이 제품 프로필 관리자는 특정 데이터 보기에 대한 액세스 권한을 부여하고 타사 내의 다른 사용자를 이 제품 프로필에 추가할 수 있습니다. 제품 프로필 관리자는 타사 팀 요구 사항에 맞게 데이터 보기를 수정할 수 있습니다.

### 행 수준 액세스 제어

사용자에게 하루의 데이터에만 액세스할 수 있는 권한을 부여하려고 합니다. 이러한 특정 행에 대한 액세스를 제한하는 방법은 다음과 같습니다.
...
1. Admin Console에서 새 [제품 프로필을](#product-profile-admin-role) 만들기하고, 제품 프로필에 사용자를 추가하고, 이 특정 데이터 보기만 제품 프로필에 포함합니다.

### 가치 수준 액세스 제어

데이터 보기에 액세스할 수 있는 사용자는 관리자가 이 데이터 보기에 포함한 측정항목 및 측정기준만 사용할 수 있습니다. 관리자는 데이터 보기에서 [포함/제외 기능](/help/data-views/component-settings/include-exclude-values.md) 또는 [값 버킷팅](../data-views/component-settings/value-bucketing.md) 구성 요소 설정을 사용하여 데이터 보기에서 특정 차원 값을 제외하거나 합계할 수 있습니다.

예: 데이터 세트의 개별 환자 데이터가 포함된 구성 요소에서 데이터 보기에 Hypertension *이라는 지표*&#x200B;를 만듭니다. 값 버킷팅을 사용하여 버킷 값에 대한 액세스만 제공하므로 데이터 사용자는 개별 환자 데이터를 볼 수 없습니다.
