...
| **인터넷 지연 시간** | 성능 페이지가 열릴 때 Adobe에서 10개의 테스트 호출을 보냅니다. 이는 각 요청이 Adobe로 이동하고 반환되는 데 평균적으로 소요되는 시간을 나타냅니다. 즉, 현재 위치와 Adobe 사이의 인터넷 속도 측정입니다. 지침은 1초 미만입니다. | 로컬 네트워크 문제, 열려 있는 많은 브라우저 탭 또는 Adobe 문제가 이 요소에 영향을 줍니다. | 알려진 서비스 문제가 있는지 확인하려면 status.adobe.com을 점검합니다. 그런 다음, 로컬 네트워크 연결을 확인하고 사용하지 않는 탭을 닫습니다. |
