...
>[!NOTE]
>
>사이트에서 수집한 데이터를 Adobe Experience Platform으로 수집하고 Customer Journey Analytics에서 사용하는 방법에 대한 간단한 안내서입니다. 추가 정보를 참고하는 경우 연구하는 것이 좋습니다.
...
2. **[!UICONTROL 스키마 만들기]**&#x200B;를 선택합니다. 옵션 목록에서 **[!UICONTROL XDM ExperienceEvent]**&#x200B;를 선택합니다.
   ...
   >[!INFO]
   >
   >    Experience Event 스키마는 프로필의 _비헤이비어_(예: 페이지 조회수, 장바구니에 추가)를 모델링하는 데 사용됩니다. 개별 프로필 스키마는 프로필 _속성_(예: 이름, 이메일, 성별)을 모델링하는 데 사용됩니다.
...
3. [!UICONTROL 제목 없는 스키마] 화면에서:
   ...
   3. [!UICONTROL 필드 그룹 추가] 대화 상자의 목록에서 **[!UICONTROL AEP Web SDK ExperienceEvent]** 필드 그룹을 선택합니다.
...
5. [!UICONTROL 필드 속성] 패널에서 `Identification`을 이름으로, **[!UICONTROL 식별]**&#x200B;을 [!UICONTROL 디스플레이 이름]으로 입력하고 **[!UICONTROL 오브젝트]**&#x200B;를 [!UICONTROL 유형]으로 **[!UICONTROL ExperienceEvent 코어 v2.1]**&#x200B;을 [!UICONTROL 필드 그룹]으로 선택합니다.
...
사이트에서 코드를 구현하는 Adobe Experience Platform의 태그 기능을 사용하여 실제로 데이터를 수집합니다. 이는 다른 태그 지정 요구 사항과 함께 코드를 배포할 수 있도록 해 주는 태그 관리 솔루션입니다. 태그는 Adobe Experience Platform Web SDK 확장을 사용하여 Adobe Experience Platform과 원활하게 통합할 수 있도록 해 줍니다.
...
(데이터스트림을 통해) 데이터를 Adobe Experience Platform에 전송할 수 있도록 Adobe Platform Web SDK 확장을 태그에 추가합니다.
...
3. [!UICONTROL 라이브러리 만들기] 대화 상자에서:
   ...
   이렇게 하면 개발 환경에 대한 태그가 저장되고 빌드됩니다. 녹색 점은 개발 환경에서 태그의 빌드가 완료되었음을 나타냅니다.
...
마지막으로 추적할 웹 사이트에 태그를 설치해야 합니다. 이는 코드를 웹 사이트 템플릿의 헤더 태그에 배치함을 의미합니다.
