...
이 빠른 시작 안내서에서는 임시 데이터를 Adobe Experience Platform으로 수집한 다음 Customer Journey Analytics에서 해당 데이터를 사용하는 방법을 설명합니다.

이를 구현하려면 다음 작업을 수행해야 합니다.

- **CSV 파일로 데이터 세트를 만듭니다**. Experience Platform에서 수집하려는 데이터의 모델(스키마)과 데이터(데이터 세트)를 수집할 위치를 정의합니다.

- Experience Platform에서 **소스 커넥터를 사용**&#x200B;하여 구성된 데이터 세트로 데이터를 가져옵니다.
...
>[!NOTE]
>
>이 빠른 시작 안내서는 를 사용하여 애드혹 데이터를 Adobe Experience Platform에 수집하고 Customer Journey Analytics에서 사용하는 방법에 대한 간단한 안내서입니다. 추가 정보를 참고하는 경우 연구하는 것이 좋습니다.
...
임시 데이터에 대해 XDM 스키마를 만들 필요가 없습니다. Adobe Experience Platform에서는 CSV 파일의 데이터를 기반으로 다음과 같은 워크플로우를 지원합니다.

1. CSV 파일의 열을 준수하는 임시 스키마를 만듭니다.
1. CSV 파일의 데이터를 포함하는 임시 스키마를 기반으로 데이터 세트를 만듭니다.

워크플로우를 시작하려면 다음을 수행하십시오.

1. Adobe Experience Platform UI의 왼쪽 레일에서 **[!UICONTROL 워크플로]**&#x200B;를 선택합니다.
...
데이터가 준비되고 업로드됩니다. 데이터가 성공적으로 업로드되면 Adobe Experience Platform UI에서 **[!UICONTROL 데이터 세트]**(으)로 리디렉션됩니다.<br/> 상태가 **[!UICONTROL StatusOrange]** **[!UICONTROL 처리]**&#x200B;인 ![CSV의 샘플 데이터](/help/assets/icons/StatusOrange.svg) 데이터 집합에 대해 **[!UICONTROL 데이터 집합 활동]**&#x200B;이 표시됩니다.

![임시 데이터에 대한 데이터 집합 활동](assets/datasets-dataset-activity.png)

임시 데이터를 검사하려면:

1. Adobe Experience Platform UI의 왼쪽 레일에서 **[!UICONTROL 데이터 세트]**&#x200B;를 선택합니다.
...
Customer Journey Analytics에서 Adobe Experience Platform 데이터 세트를 사용하려면 워크플로우에서 발생하는 임시 데이터를 포함하는 연결을 만듭니다.

연결을 통해 Adobe Experience Platform의 데이터 세트를 Workspace에 통합할 수 있습니다. 이러한 데이터 세트에 대해 보고하려면 먼저 Adobe Experience Platform과 Workspace의 데이터 세트 간에 연결을 설정해야 합니다.
...
1. **[!UICONTROL 데이터 세트 추가]**&#x200B;의 **[!UICONTROL 데이터 세트 선택]** 단계에서:
   ...
   1. 이전에 만든 데이터 세트(예: **[!UICONTROL CSV의 샘플 데이터]**)와 연결에 포함할 다른 데이터 세트를 선택하십시오.
...
1. **[!UICONTROL 구성 요소]** 단계에서:
   ...
   1. 포함할 임시 스키마 필드 및/또는 표준 구성 요소를 **[!UICONTROL 지표]** 또는 **[!UICONTROL 차원]** 구성 요소 상자에 추가합니다.
...
1. 첫 번째 보고서를 만들려면 [!UICONTROL 패널]의 [!UICONTROL 자유 형식 테이블]에서 차원 및 지표를 끌어서 놓습니다. Ad Hoc 데이터를 기반으로 이러한 지표 또는 차원을 포함합니다.
