...
description: Customer Journey Analytics에서 스트리밍 데이터를 수집 및 사용하는 방법을 설명합니다.
...
- **HTTP API 소스 커넥터 사용** 데이터를 Adobe Experience Platform에 구성된 데이터 세트에 쉽게 스트리밍할 수 있습니다.
...
>[!NOTE]
>
>이 안내서는 스트리밍 데이터를 Adobe Experience Platform에 수집하고 Customer Journey Analytics에서 사용하는 방법에 대한 간단한 안내서입니다. 참조할 때는 추가 정보를 학습하는 것이 좋습니다.
...
소스 응용 프로그램은 사용자가 생성한 스키마를 따르며, 표시되는 데이터를 스트리밍합니다.
...
이 데이터를 만든 데이터 세트에 스트리밍하려면 해당 데이터를에 보낼 스트리밍 끝점을 정의해야 합니다. HTTP API 소스 커넥터를 정의하여 이를 수행합니다.

HTTP API 소스 커넥터를 만들려면 다음을 수행하십시오.

1. Experience Platform UI에서 **[!UICONTROL 소스]** 아래에 [!UICONTROL 연결] 왼쪽 레일에 있습니다.

2. 선택 **[!UICONTROL 스트리밍]** 목록 [!UICONTROL 카테고리].

3. 선택 **설정** 에서 [!UICONTROL HTTP API] 타일.

   ![HTTP API 타일 설정](./assets/httpapi-tile.png)

4. 에서 [!UICONTROL 인증] 의 단계 [!UICONTROL 데이터 추가] 화면:

   HTTP API 연결의 이름과 설명을 입력합니다.

   선택 **[!UICONTROL XDM 호환 가능]** 스트리밍하는 데이터가 기존 XDM 스키마와 호환되는지를 나타냅니다.

   선택 **[!UICONTROL 소스에 연결]**. 연결에 성공하면 [!UICONTROL 연결됨].

   ![HTTP API 인증](./assets/httpapi-authentication.png)

   선택 **[!UICONTROL 다음]** 계속하십시오.

5. 에서 [!UICONTROL 데이터 흐름 세부 정보] 의 단계 [!UICONTROL 데이터 추가] 화면:
...
6. 다음 [!UICONTROL 검토] 의 단계 [!UICONTROL 데이터 추가] 화면에서는 HTTP API 연결에 대한 개요를 제공합니다.

   ![데이터 흐름 세부 정보](./assets/httpapi-review.png)

   선택 **[!UICONTROL 완료]**.

7. HTTP API 스트리밍 끝점에 대한 최종 정의가 표시됩니다.

   ![HTTP API 스트리밍 끝점](./assets/httpapi-streamingendpoint.png)

스트리밍 끝점 URL을 복사하여 해당 URL을 사용하여 데이터를 Adobe Experience Platform 충성도 데이터 세트에 스트리밍하도록 충성도 애플리케이션을 구성할 수 있습니다.

자세한 내용은 [UI를 사용하여 HTTP API 스트리밍 연결 만들기](https://experienceleague.adobe.com/docs/experience-platform/sources/ui-tutorials/create/streaming/http.html?lang=en) 인증 사용 방법, 수신 데이터가 XDM 스키마와 호환되지 않을 때 데이터를 매핑하는 방법 및 스트리밍 커넥터 설정의 일부로 새 데이터 세트를 만드는 방법을 설명하는 보다 포괄적인 자습서를 제공합니다.
...
>[!SUCCESS]
>
>모든 단계를 완료했습니다. 수집하려는 충성도 데이터(스키마)와 Adobe Experience Platform에 저장할 위치(데이터 세트)를 정의하여, 충성도 데이터를 데이터 세트에 직접 스트리밍하도록 HTTP API 소스 커넥터를 구성했습니다. 데이터 보기 정의를 사용하여 사용할 차원 및 지표를 지정하고 최종적으로 데이터를 시각화하고 분석하는 첫 번째 프로젝트를 만들 수 있습니다.
