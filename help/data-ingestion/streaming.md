...
>[!NOTE]
>
>스트리밍 데이터를 Adobe Experience Platform으로 수집하고 Customer Journey Analytics에서 사용하는 방법에 대한 간단한 안내서입니다. 추가 정보를 참고하는 경우 연구하는 것이 좋습니다.
...
이 데이터를 생성 데이터 세트로 스트리밍하려면 해당 데이터를 보낼 스트리밍 엔드포인트를 정의해야 합니다. HTTP API 소스 커넥터를 정의하여 이 작업을 수행합니다.
...
4. [!UICONTROL 데이터 추가] 화면의 [!UICONTROL 인증] 단계에서:
...
인증을 사용하는 방법, 수신 데이터가 XDM 스키마와 호환되지 않을 경우 데이터를 매핑하는 방법과 스트리밍 커넥터 설정의 일부로 새 데이터 세트를 만드는 방법을 설명하는 포괄적인 튜토리얼은 [UI를 사용하여 HTTP API 스트리밍 연결 만들기](https://experienceleague.adobe.com/docs/experience-platform/sources/ui-tutorials/create/streaming/http.html?lang=ko)를 참조하십시오.
