...
- **워크플로를 사용**&#x200B;하여 배치 데이터를 Adobe Experience Platform에서 구성된 데이터 세트에 쉽게 업로드할 수 있습니다.
...
>[!NOTE]
>
>배치 데이터를 Adobe Experience Platform으로 수집하고 Customer Journey Analytics에서 사용하는 방법에 대한 간단한 안내서입니다. 추가 정보를 참고하는 경우 연구하는 것이 좋습니다.

## 스키마 및 데이터 세트 설정

데이터를 Adobe Experience Platform으로 수집하려면 먼저 수집하려는 데이터를 정의해야 합니다. Adobe Experience Platform으로 수집된 모든 데이터는 다운스트림 기능에서 인식 및 활성화될 수 있도록 비정규화된 표준 구조를 준수해야 합니다. 경험 데이터 모델(XDM)은 스키마 형태로 이러한 구조를 제공하는 표준 프레임워크입니다.

스키마를 정의한 후에는 하나 이상의 데이터 세트를 사용하여 데이터 컬렉션을 저장하고 관리합니다. 데이터 세트는 스키마(열) 및 필드(행)를 포함하는 데이터 수집을 위한 저장소 및 관리 구조입니다.
...
이를 수집하려면 먼저 이 데이터를 모델링하는 스키마를 정의해야 합니다.
...
2. **[!UICONTROL 스키마 만들기]**&#x200B;를 선택합니다. 옵션 목록에서 **[!UICONTROL XDM 개별 프로필]**&#x200B;을 선택합니다.
   ...
   >[!INFO]
   >
   >    개별 프로필 스키마는 프로필 _속성_(예: 이메일, 로열티 상태, 로열티 포인트)을 모델링하는 데 사용됩니다. Experience Event 스키마는 프로필의 _비헤이비어_(예: 페이지 조회수, 장바구니에 추가)를 모델링하는 데 사용됩니다.
...
5. [!UICONTROL 필드 속성] 패널에서 `Identification`을 이름으로, **[!UICONTROL 식별]**&#x200B;을 [!UICONTROL 디스플레이 이름]으로 입력하고 **[!UICONTROL 오브젝트]**&#x200B;를 [!UICONTROL 유형]으로 **[!UICONTROL 프로필 코어 v2]**&#x200B;를 [!UICONTROL 필드 그룹]으로 선택합니다.
   ...
   이러면 식별 기능이 스키마에 추가됩니다. 이 경우 배치 데이터의 이메일 주소를 사용하여 로열티 정보를 식별하려고 합니다.
...
스키마를 통해 데이터 모델은 정의됩니다. 이제 해당 데이터를 저장하고 관리하는 구조를 정의해야 합니다. 이 작업은 데이터 세트에서 수행합니다.
...
워크플로 기능을 사용하여 배치 데이터를 Adobe Experience Platform에 업로드합니다. 사용 중인 예제 배치 파일은 다음 콘텐츠가 포함된 CSV 파일입니다.
...
1. 플랫폼 UI의 왼쪽 레일에서 **[!UICONTROL 워크플로]**&#x200B;를 선택합니다.

2. **[!UICONTROL XDM 스키마에 CSV 매핑]**&#x200B;을 선택합니다. **[!UICONTROL 실행]**&#x200B;을 선택합니다.

   ![XDN에 CSV 매핑](./assets/workflow-mapcsvtoxdm.png)

3. [!UICONTROL XDM 스키마에 CSV 매핑] 화면의 [!UICONTROL 데이터 흐름 세부 정보] 단계에서:
...
4. [!UICONTROL 데이터 선택] 단계에서:

   **[!UICONTROL 파일 선택]**&#x200B;을 드래그 앤 드롭하거나 선택하여 로열티 데이터가 있는 CSV 파일을 선택합니다. 로열티 데이터의 미리보기가 표시됩니다.
...
5. [!UICONTROL 매핑] 단계에서:
   ...
   CSV 파일의 데이터를 스키마의 데이터에 매핑합니다. AI를 사용하여 워크플로 기능은 배치 데이터 필드를 스키마 필드에 자동으로 매핑하려고 합니다.

   ![데이터 매핑](./assets/workflow-dataflow-mapping.png)

   **[!UICONTROL 미리보기 데이터]**&#x200B;를 사용하여 매핑된 데이터의 미리보기를 볼 수 있습니다.

   ![매핑 미리보기](./assets/workflow-dataflow-mapping-preview.png)

6. **[!UICONTROL 마침]**&#x200B;을 선택하여 배치 데이터를 Adobe Experience Platform으로 수집하기 시작합니다.

수신 데이터가 XDM 스키마와 호환되지 않을 경우 데이터를 매핑하는 방법, 매핑 템플릿을 사용하는 방법과 계산된 필드를 사용하는 방법에 대한 자세한 내용은 [기존 XDM 스키마에 CSV 파일 매핑](https://experienceleague.adobe.com/docs/experience-platform/ingestion/tutorials/map-csv/existing-schema.html)을 참조하여 배치 데이터가 스키마의 기대치에 부합하는지 확인합니다.
...
5. [!UICONTROL 패널]의 [!UICONTROL 자유 형식 테이블]에서 차원 및 지표를 드래그 앤 드롭하여 첫 번째 보고서를 만듭니다. 예를 들어 `Program Points Balance` 및 `Page View`를 지표로 `email`을 차원으로 드래그하여 웹 사이트를 방문한 적이 있고, 로열티 포인트를 수집하는 로열티 프로그램의 일부인 프로필에 대한 간단한 개요를 살펴봅니다.
...
>[!SUCCESS]
>
>모든 단계가 완료되었습니다. Adobe Experience Platform에서 수집할 로열티 데이터(스키마)와 이를 저장할 위치(데이터 세트) 정의를 시작으로 로열티 데이터를 데이터 세트로 일괄 업로드하는 워크플로를 구성했습니다. Customer Journey Analytics에서 연결을 정의하여 수집된 로열티 데이터 및 기타 데이터를 사용했습니다. 데이터 보기 정의를 통해 사용할 차원 및 지표를 지정했고 최종적으로 데이터를 시각화 및 분석하는 첫 번째 프로젝트를 제작했습니다.
