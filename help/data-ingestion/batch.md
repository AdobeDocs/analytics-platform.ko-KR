...
>[!SUCCESS]
>
>모든 단계가 완료되었습니다. Adobe Experience Platform에서 수집할 로열티 데이터(스키마)와 저장 위치(데이터 세트)를 정의한 후, 로열티 데이터를 데이터 세트에 배치 업로드하는 워크플로를 구성했습니다. Customer Journey Analytics에서 연결을 정의하여 수집된 로열티 데이터 및 기타 데이터를 사용했습니다. 데이터 보기 정의를 통해 사용할 차원 및 지표를 지정했고 최종적으로 데이터를 시각화 및 분석하는 첫 번째 프로젝트를 제작했습니다.
